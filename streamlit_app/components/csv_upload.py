...
def _run_pipeline(df: pd.DataFrame, filename: str):
    ...
    try:
        ...
        transform_result = transformation.execute(
            raw_data_path=run_dir / "raw_data.parquet",
            run_dir=run_dir
        )
        ...
        calc_result = calculation.execute(
            clean_data_path=run_dir / "clean_data.parquet",
            run_dir=run_dir
        )
...
if __name__ == "__main__":
    ...
    st.set_page_config(
        page_title="CSV Upload - ABACO Analytics",
        page_icon="📤",
        layout="wide"
    )
    render_csv_upload()
