...
def render_advanced_intelligence(dashboard_metrics):
    ...
    with adv_tabs[0]:
        ...
        if seg_data:
            ...
            with c1:
                fig_seg_aum = px.bar(
                    df_seg_display, x="Segment", y="Portfolio Value ($)",
                    title="AUM by Segment", text_auto='.2s'
                )
                st.plotly_chart(apply_theme(fig_seg_aum), use_container_width=True)
            with c2:
                fig_seg_risk = px.bar(
                    df_seg_display, x="Segment", y="Delinquency (%)",
                    title="Risk by Segment", color="Delinquency (%)", color_continuous_scale="Reds"
                )
    ...
    with adv_tabs[1]:
        ...
        if churn_data:
            ...
            if "month" in df_churn.columns:
                ...
                fig_churn = px.line(df_churn, x="month", y=["churn90d_pct"], title="90-Day Churn Rate Trend", markers=True)
    ...
    with adv_tabs[2]:
        ...
        if ue_data:
            ...
            if "month" in df_ue.columns:
                ...
                fig_ue = px.line(df_ue, x="month", y="ltv_cac_ratio", title="LTV/CAC Ratio Trend", markers=True)
