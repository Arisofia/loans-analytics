...
class HubSpotSyncService {
    ...
    try {
      ...
      for (const contact of contacts) {
        ...
        const kpiResponse = await axios.get(
          `${this.kpiServiceUrl}/customer/${contactData.email}`,
          {
  ...
  async syncDealsWithKPIs(): Promise<void> {
    try {
      ...
      for (const deal of deals) {
        ...
        const impactAnalysis = await axios.post(
          `${this.kpiServiceUrl}/analysis/deal-impact`,
          dealData,
          {
            headers: { Authorization: `Bearer ${process.env.API_KEY}` },
          }
  ...
  private async updateContactWithKPIs(
    contactId: string,
    data: Contact
  ): Promise<void> {
    try {
  ...
  private async updateDealWithKPIImpact(
    dealId: string,
    data: Deal
  ): Promise<void> {
    try {
