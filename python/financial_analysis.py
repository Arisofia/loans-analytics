...
def resolve_column(candidates: List[str], fallback: Optional[str] = None):
    ...
    def decorator(func: Callable) -> Callable:
        @wraps(func)
        def wrapper(self, df: pd.DataFrame, col_param: str = None, **kwargs):
            if col_param and col_param in df.columns:
                ...
                if not resolved_col:
                    logger.warning(f"Column not found for {func.__name__}, tried: {candidates}")
                    return df
...
class FinancialAnalyzer:
    ...
    def segment_clients_by_exposure(self, df: pd.DataFrame, exposure_col: str) -> pd.DataFrame:
        ...
        try:
            ...
            result['exposure_segment'] = result[exposure_col].apply(Classification.exposure_segment_rules)
        except ValueError as e:
            logger.error(f"Exposure segmentation failed: {e}")
    ...
    def classify_client_type(
        ...
        if not id_col:
            logger.error(f"Customer ID column not found")
            return result
        ...
        if count_col and active_col:
            try:
                ...
                logger.error(f"Client type classification failed: {e}")
    ...
    def calculate_weighted_stats(
        ...
        weight_col = find_column(loan_df, [weight_field, 'outstanding_balance', 'olb', 'current_balance', 'saldo_actual', 'balance'])
        if not weight_col:
            logger.error(f"Weight field not found, tried: {[weight_field]}")
            return pd.DataFrame()
        ...
        for stat in metrics:
            ...
            if stat_col:
                ...
                if not filtered_df.empty and filtered_df[weight_col].sum() > 0:
                    weighted_avg = np.average(filtered_df[stat_col], weights=filtered_df[weight_col])
                    result[f'weighted_{stat}'] = weighted_avg
                else:
                    logger.warning(f"No valid data for weighted {stat}")
    ...
    def calculate_line_utilization(
        ...
        credit_col = find_column(result_df, [credit_line_field, 'credit_line', 'line_limit', 'limite_credito'])
        loan_col = find_column(result_df, [loan_amount_field, 'outstanding_balance', 'olb', 'current_balance', 'loan_amount'])
        
        if not credit_col or not loan_col:
            logger.warning(f"Line utilization columns not found")
            return result_df
