...
IGNORED_WORDS = {
    'sum', 'count', 'min', 'max', 'mean', 'avg', 'if', 'else', 'and', 'or', 'not', 'null', 'None', 'True', 'False',
    'int', 'float', 'str', 'abs', 'round', 'len', 'in', 'for', 'return', 'def', 'lambda', 'import', 'as', 'from',
}
...
def main(yaml_path):
    ...
    for kpi_key, kpi in config.items():
        ...
        if missing:
            errors.append(f"KPI '{kpi_key}' references missing fields: {sorted(missing)} (sources: {data_sources})")
