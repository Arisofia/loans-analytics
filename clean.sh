...
delete_item() {
	...
	if [[ ${DRY_RUN} == true ]]; then
		...
		if [[ -f ${item} ]]; then
			rm "${item}" && echo "  ✓ Deleted: ${item}" && ((TOTAL_DELETED += 1))
		elif [[ -d ${item} ]]; then
			rm -rf "${item}" && echo "  ✓ Deleted directory: ${item}" && ((TOTAL_DELETED += 1))
		fi
	fi
}
...
if [[ ${MODE} == "full" ]]; then
	...
	if [[ -f "projects/Q1-2026.md" ]]; then
		if [[ ${DRY_RUN} == false ]]; then
			...
			rmdir projects 2>/dev/null && echo "  ✓ Removed: projects/" && ((TOTAL_DELETED += 1))
		else
	...
	if [[ -f "models/loan_risk_model.pkl" ]]; then
		if [[ ${DRY_RUN} == false ]]; then
			...
			rmdir models 2>/dev/null && echo "  ✓ Removed: models/" && ((TOTAL_DELETED += 1))
		else
	...
	if [[ -d "fi-analytics" ]]; then
		if [[ ${DRY_RUN} == false ]]; then
			...
			rm -rf fi-analytics && echo "  ✓ Deleted: fi-analytics/" && ((TOTAL_DELETED += 1))
		else
