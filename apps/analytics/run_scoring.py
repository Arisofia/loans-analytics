...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(falls back to AZURE_STORAGE_ACCOUNT_URL)."
        ),
    )
    parser.add_argument(
        "--ltv-threshold",
...
def summarize_results(metrics: Dict[str, Any], alerts_rows: int) -> None:
    """
    Display a summary of loan portfolio KPIs and risk alerts in a professional,
    human-readable format.


    Args:
        metrics (Dict[str, Any]): Dictionary of KPI metric names and their
            values. Numeric values are formatted to two decimal places; string
            values are displayed as-is.
        alerts_rows (int): The number of risk alerts flagged in the portfolio
            analysis.
    ...
    for key, value in metrics.items():
        ...
        print(
            f"{label}: {value:.2f}" if isinstance(value, (int, float))
            else f"{label}: {value}"
        )
...
def main() -> None:
    """Main entry point for KPI and risk alert computation workflow."""
    args = parse_args()
    ...
    if args.container_name:
        if not args.connection_string and not args.account_url:
            raise SystemExit(
                "Azure export requested but no connection string or account "
                "URL was provided. Pass --connection-string/--account-url or "
        ...
        blob_path = engine.export_kpis_to_blob(
            exporter, blob_name=args.blob_name
        )
