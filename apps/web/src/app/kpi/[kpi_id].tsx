...
const ProvenanceTooltip: FC<{ kpi: KPI }> = ({ kpi }) => (
  <div className="text-xs text-gray-500 mt-2">
    ...
    <span className="mr-2">
      Source:{' '}
      <a
        href={`https://github.com/Abaco-Technol/abaco-loans-analytics/blob/main/config/kpis/${kpi.source}`}
        target="_blank"
    ...
    <span className="mr-2">
      SQL:{' '}
      <a
        href={`https://github.com/Abaco-Technol/abaco-loans-analytics/blob/main/sql/calculations/${kpi.sql}`}
        target="_blank"
        rel="noopener noreferrer"
      >
        {kpi.sql}
...
const KPIDrilldownPage: FC = () => {
  ...
  const kpiId =
    typeof params === 'object' && params !== null ? (params['kpi_id'] as string) : 'par_90'
  ...
  return (
    <div className="max-w-2xl mx-auto p-8">
      ...
      <div className="mb-2">
        <b>Formula:</b>{' '}
        <span className="font-mono bg-gray-100 px-2 py-1 rounded">{kpi.formula}</span>
      </div>
      <div className="mb-2">
        <b>Thresholds:</b>{' '}
        <span className="text-yellow-600">Warning: {kpi.thresholds.warning}</span>,{' '}
        <span className="text-red-600">Critical: {kpi.thresholds.critical}</span>
      </div>
      <div className="mb-2">
        <b>Owner:</b> {kpi.owner}
      </div>
      <div className="mb-2">
        <b>Last Refresh:</b> {kpi.lastRefresh}
      </div>
