...
class AppSettings(BaseSettings):
    ...
    aum_target_2026: float = Field(
        default=16.3e6, description="AUM Target for 2026 in USD"
    )
    ...
    top10_concentration_max: float = Field(
        default=0.30, description="Maximum Top-10 concentration"
    )
    single_obligor_max: float = Field(
        default=0.04, description="Maximum single-obligor concentration"
    )
    apr_target_min: float = Field(
        default=0.34, description="Minimum target weighted APR"
    )
    apr_target_max: float = Field(
        default=0.40, description="Maximum target weighted APR"
    )
    cost_of_debt_max: float = Field(default=0.13, description="Maximum cost of debt")
    dscr_min: float = Field(
        default=1.2, description="Minimum Debt Service Coverage Ratio"
    )
    ...
    ce_6m_min: float = Field(
        default=0.96, description="6-month collection efficiency minimum"
    )

    # --- Business Logic Thresholds ---
    churn_90d_threshold_days: int = Field(
        default=90, description="Threshold for 90-day churn"
    )
    churn_120d_threshold_days: int = Field(
        default=120, description="Threshold for 120-day churn"
    )
    reactivation_threshold_days: int = Field(
        default=180, description="Threshold for reactivation"
    )
    intensity_low_max_loans: int = Field(
        default=1, description="Max loans for low intensity"
    )
    intensity_medium_max_loans: int = Field(
        default=3, description="Max loans for medium intensity"
    )
