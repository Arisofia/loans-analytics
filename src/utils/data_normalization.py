...
COL_OUTSTANDING_AMOUNT = "outstanding_amount"
...
COLUMN_MAPPING = {
    ...
    COL_OUTSTANDING_AMOUNT: ["Outstanding Amount", "total_receivable_usd", "outstanding", "balance"],
    COL_APPROVED_AMOUNT: ["Approved Amount", "approved_limit", "loan_limit"],
...
def normalize_columns(df: pd.DataFrame, extra_mapping: Optional[Dict[str, List[str]]] = None) -> pd.DataFrame:
    """
...
def ensure_columns(df: pd.DataFrame, required: List[str], defaults: Optional[Dict[str, Any]] = None) -> pd.DataFrame:
    """
