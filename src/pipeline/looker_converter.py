...
class LookerConverter:
    ...
    def load_financials(
        ...
        if path.is_dir():
            files = sorted(
                [*path.glob("*.csv"), *path.glob("*.xlsx"), *path.glob("*.xls")],
        ...
        default_date_strategy = str(
            self.config.get("financials_default_date_strategy", "file_mtime")
        ).lower()
        ...
        for file_path in files:
            ...
            is_long = (
                bool(metric_col and value_col) if format_mode == "auto" else format_mode == "long"
            )
            ...
            if is_long:
                ...
                for idx, metric_name in financials_df[metric_col].items():
                    ...
                    if metric_key and pd.notna(values[idx]) and pd.notna(date_series[idx]):
                        financials_by_date.setdefault(str(date_series[idx]), {})[metric_key] = (
                            float(values[idx])
                        )
            else:
        ...
        return financials_by_date, {
            ...
            "metrics": sorted({k for v in financials_by_date.values() for k in v}),
        }
    ...
    def apply_financials(
        ...
        if "cash_available_usd" not in df.columns:
            df["cash_available_usd"] = df["measurement_date"].map(
                lambda date: financials_by_date.get(str(date), {}).get("cash_balance_usd", 0.0)
            )

        df["cash_available_usd"] = (
            df["cash_available_usd"].fillna(df.get("cash_balance_usd", 0.0)).fillna(0.0)
        )
        return df
    ...
    def convert_par_balances(
        ...
        res = pd.DataFrame(
            {
                "measurement_date": pd.to_datetime(
                    df[col_map["reporting_date"]], errors="coerce"
                ).dt.strftime("%Y-%m-%d"),
                "total_receivable_usd": pd.to_numeric(
                    df[col_map["outstanding_balance_usd"]], errors="coerce"
                ),
                "dpd_90_plus_usd": pd.to_numeric(
                    df[col_map["par_90_balance_usd"]], errors="coerce"
                ),
                "dpd_60_90_usd": (
                    pd.to_numeric(df[col_map["par_60_balance_usd"]], errors="coerce")
                    - pd.to_numeric(df[col_map["par_90_balance_usd"]], errors="coerce")
                ).clip(lower=0),
                "dpd_30_60_usd": (
                    pd.to_numeric(df[col_map["par_30_balance_usd"]], errors="coerce")
                    - pd.to_numeric(df[col_map["par_60_balance_usd"]], errors="coerce")
                ).clip(lower=0),
                "dpd_7_30_usd": (
                    pd.to_numeric(df[col_map["par_7_balance_usd"]], errors="coerce")
                    - pd.to_numeric(df[col_map["par_30_balance_usd"]], errors="coerce")
                ).clip(lower=0),
                "dpd_0_7_usd": (
                    pd.to_numeric(df[col_map["outstanding_balance_usd"]], errors="coerce")
                    - pd.to_numeric(df[col_map["par_7_balance_usd"]], errors="coerce")
                ).clip(lower=0),
            }
        ...
        grouped["loan_id"] = grouped["measurement_date"].apply(
            lambda d: f"looker_snapshot_{str(d).replace('-', '')}"
        )
    ...
    def convert_dpd_loans(
        ...
        balance, dpd = pd.to_numeric(df[bal_col], errors="coerce").fillna(0.0), pd.to_numeric(
            df[dpd_col], errors="coerce"
        ).fillna(0.0)
