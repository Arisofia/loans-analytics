...
- **Validation & compliance utilities**: `src.validation` enforces required
  numeric/date columns, while `src.compliance` masks detected PII, records
  access, and serializes the report (owner: data engineering, reviewer: data
  privacy) so that workflow context is versioned in `logs/runs`.
