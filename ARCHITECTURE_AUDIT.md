...
|--------|-----------|-------|
| `ingestion.py` | `/python/ingestion.py` + `/python/pipeline/ingestion.py` | Two separate ingestion implementations |
| `transformation.py` | `/python/transformation.py` + `/python/pipeline/transformation.py` | Two separate transformation implementations |
| `kpi_engine` | `kpi_engine.py` + `kpi_engine_v2.py` | Old vs new KPI calculation interface |
| `validation.py` | Root `/python/validation.py` | No clear module organization |
...
### Phase 1: Ingestion
- **File**: `/python/pipeline/ingestion.py`
- **Purpose**: Read data from Cascade API
- **Dependencies**: Not analyzed yet

### Phase 2: Transformation
- **File**: `/python/pipeline/transformation.py`
- **Purpose**: Clean and enrich raw data
- **Dependencies**: Not analyzed yet

### Phase 3: Calculation
- **Files**: `/python/pipeline/calculation_v2.py` (main) + `/python/kpi_engine_v2.py`
- **Purpose**: Calculate KPIs and metrics
- **Dependencies**: KPI modules in `/python/kpis/`

### Phase 4: Output
- **File**: `/python/pipeline/output.py`
- **Purpose**: Export results to databases/files
- **Dependencies**: Not analyzed yet

### Orchestrator
- **File**: `/python/pipeline/orchestrator.py`
- **Purpose**: Coordinate all 4 phases
- **Type**: Primary entry point

## 4. KPI Calculation Architecture

### KPI Modules (in `/python/kpis/`)
...
### AI Agent Framework (in `/python/agents/`)
...
1. **Duplicate Ingestion/Transformation Modules**
   - `/python/ingestion.py` vs `/python/pipeline/ingestion.py`
...
1. **Consolidate to `/python/pipeline/` as primary**
   - Keep `/python/pipeline/ingestion.py` as canonical
   - Remove `/python/ingestion.py`
