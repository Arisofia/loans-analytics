...
- **Engineering (PR Assignee)**: Implements changes, ships tests, and updates observability; assigns PRs to `@codex`, requests review from `@coderabbit`, and ensures SonarCloud gates are green.
...
3. Assign and reviewers: set assignee to `@codex`; request review from `@coderabbit`; confirm SonarCloud/SAST gates are green.
