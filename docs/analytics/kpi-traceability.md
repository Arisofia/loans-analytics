...
3. Assign and reviewers: set assignee to `@codex`; request reviews from `@sonarqube`, `@coderabbit`, and `@sourcery` to enforce SAST/quality automation. The workflow `.github/workflows/pr-auto-assign.yml` applies this automatically on open/sync; rerun the workflow if the state drifts.
