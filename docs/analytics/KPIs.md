...
| CAC / LTV           | Acquisition cost vs lifetime value                | Growth           | GrowthCoach   | Marketing + repayment    | Red if CAC/LTV >0.3                 | Channel table, cohort profitability           | `runbooks/kpi-breach.md`        | Slack `#growth`             || SegmentationSummary   | Per-segment client counts, portfolio value, avg loan (2025) — primary metric is total unique clients (2025). See `config/pipeline.yml` (calculation: `src.kpis.segmentation_summary.calculate_segmentation_summary`) for implementation | Commercial      | KPIAnalytics  | Loans fact + customer join | N/A                                  | Segment breakdown table, delinquency rates     | `runbooks/segmentation.md`      | Slack `#commercial`         || Data Quality        | Freshness, completeness, duplicates, schema drift | Data Engineering | Integrator    | Pipelines + monitors     | Red if freshness >1h or null %>1%   | Failed checks, offending rows                 | `runbooks/schema-drift.md`      | Slack `#data`, email        |
