...
class RiskParameters(BaseModel):
    ...
    loss_given_default: float = Field(default=0.1, ge=0, le=1)

class SLASettings(BaseModel):
    decision_sla_hours: int = Field(default=24)
    funding_sla_hours: int = Field(default=48)
    sla_compliance_target: float = Field(default=0.9)
    min_slo: float = Field(default=0.995)

class AnalyticsSettings(BaseModel):
    required_columns: list[str] = Field(default=['loan_amount', 'appraised_value', 'borrower_income', 'monthly_debt', 'loan_status', 'interest_rate', 'principal_balance'])
    numeric_columns: list[str] = Field(default=['loan_amount', 'appraised_value', 'borrower_income', 'monthly_debt', 'interest_rate', 'principal_balance'])
    delinquent_statuses: set[str] = Field(default={'30-59 days past due', '60-89 days past due', '90+ days past due', 'delinquent'})
    ingestion_numeric_columns: list[str] = Field(default=['dpd_0_7_usd', 'dpd_7_30_usd', 'dpd_30_60_usd', 'dpd_60_90_usd', 'dpd_90_plus_usd', 'total_receivable_usd', 'total_eligible_usd', 'discounted_balance_usd', 'cash_available_usd'])
...
class EnvironmentSettings(BaseModel):
    environment: str = Field(default_factory=lambda: os.getenv('ENVIRONMENT', 'dev'), description='Current environment: dev, staging, or prod')
    prod_data_path: Optional[str] = Field(default_factory=lambda: os.getenv('PROD_DATA_PATH'), description='Production data mount path')
    staging_data_path: Optional[str] = Field(default_factory=lambda: os.getenv('STAGING_DATA_PATH'), description='Staging data mount path')
    ...
    def validate_required_env_vars(self) -> None:
        if self.environment == 'prod':
            ...
            has_supabase_auth = bool(os.getenv('SUPABASE_ANON_KEY') or os.getenv('SUPABASE_SERVICE_ROLE_KEY'))
...
class Settings(BaseSettings):
    ...
    def _candidate_config_paths(cls, project_root: str | Path | None=None) -> list[Path]:
        ...
        return [config_dir / 'business_rules.yaml', config_dir / 'business_parameters.yml']
    ...
    @classmethod
    def _apply_yaml_overrides(cls, app_settings: 'Settings', yaml_data: dict[str, Any]) -> 'Settings':
        updates: dict[str, Any] = {}
        if (financial_payload := cls._map_guardrails_payload(yaml_data)):
        ...
        if 'analytics_weights' in yaml_data:
            ...
            current_analytics['dq_null_weight'] = weights.get('dq_null_weight', current_analytics['dq_null_weight'])
            current_analytics['dq_duplicate_weight'] = weights.get('dq_duplicate_weight', current_analytics['dq_duplicate_weight'])
            current_analytics['dq_invalid_numeric_weight'] = weights.get('dq_invalid_numeric_weight', current_analytics['dq_invalid_numeric_weight'])
        ...
        if isinstance(portfolio_targets, dict):
            updates['portfolio_targets_2026'] = {str(month): int(str(value).replace('_', '')) for month, value in portfolio_targets.items() if len(str(month)) == 7 and str(month)[:4].isdigit() and (str(month)[4] == '-')}
