...
import pandas as pd
from backend.python.kpis.strategic_modules import build_compliance_dashboard, build_next_steps_plan, build_pd_model, predict_kpis

class TestStrategicModules(unittest.TestCase):

    def _sample_loans(self) -> pd.DataFrame:
        return pd.DataFrame({'loan_id': ['L1', 'L2', 'L3', 'L4', 'L5', 'L6'], 'outstanding_loan_value': [1000, 1500, 1200, 800, 700, 900], 'interest_rate_apr': [0.32, 0.38, 0.41, 0.29, 0.35, 0.31], 'term': [45, 60, 75, 40, 55, 65], 'disbursement_amount': [1200, 1700, 1400, 1000, 900, 1100], 'tpv': [3000, 5200, 4100, 2800, 2600, 3300], 'days_in_default': [0, 35, 95, 10, 190, 0], 'disbursement_date': ['2025-10-05', '2025-11-08', '2025-12-10', '2026-01-05', '2026-02-01', '2026-03-01'], 'pagador': ['P1', 'P1', 'P2', 'P3', 'P3', 'P4'], 'loan_status': ['Current', 'Default', 'Default', 'Current', 'Default', 'Current']})
    ...
    def test_next_steps_plan_includes_apr_breach_action(self):
        compliance = {'metrics': [{'metric': 'apr_pct_ann', 'status': 'breach', 'variance': 5.4}], 'variance_decomposition': {'apr_pct_ann': {'driver': 'Pricing above target ceiling', 'explanation': 'Weighted APR exceeds pricing corridor'}}}
    ...
    def test_next_steps_plan_includes_dscr_no_data_action(self):
        compliance = {'metrics': [{'metric': 'dscr', 'status': 'no_data', 'variance': None}], 'variance_decomposition': {'dscr': {'driver': 'DSCR inputs unavailable', 'explanation': 'NOI/debt_service fields missing'}}}
        plan = build_next_steps_plan(forecast={}, compliance=compliance)
    ...
    def test_next_steps_plan_consolidates_duplicate_metric_actions(self):
        compliance = {'metrics': [{'metric': 'apr_pct_ann', 'status': 'breach', 'variance': 5.4}, {'metric': 'apr_pct_ann', 'status': 'breach', 'variance': 2.1}], 'variance_decomposition': {'apr_pct_ann': {'driver': 'Pricing above target ceiling', 'explanation': 'Weighted APR exceeds pricing corridor'}}}
        plan = build_next_steps_plan(forecast={}, compliance=compliance)
        apr_actions = [a for a in plan['actions'] if a['metric'] == 'apr_pct_ann']
        self.assertEqual(len(apr_actions), 1)
    ...
    def test_next_steps_plan_consolidates_semantic_duplicates_across_sources(self):
        compliance = {'metrics': [{'metric': 'par30_pct', 'status': 'breach', 'variance': 3.0}], 'variance_decomposition': {'par30_pct': {'driver': 'Rising delinquency in key segment', 'explanation': 'PAR30 above threshold'}}}
        forecast = {'par30_forecast': [{'month': 'M+1', 'value': 20.0}, {'month': 'M+2', 'value': 24.0}]}
    ...
    def test_next_steps_plan_supports_alias_metric_for_apr(self):
        compliance = {'metrics': [{'metric': 'apr', 'status': 'breach', 'variance': 4.2}], 'variance_decomposition': {'apr': {'driver': 'Pricing above target ceiling', 'explanation': 'Alias metric received from upstream'}}}
        plan = build_next_steps_plan(forecast={}, compliance=compliance)
    ...
    def test_next_steps_plan_consolidates_revenue_alias_across_sources(self):
        compliance = {'metrics': [{'metric': 'revenue_usd', 'status': 'warning', 'variance': -1200.0}], 'variance_decomposition': {'revenue_usd': {'driver': 'Pipeline conversion slowdown', 'explanation': 'Monthly revenue below target'}}}
        forecast = {'revenue_forecast': [{'month': 'M+1', 'value': 10000.0}, {'month': 'M+2', 'value': 8500.0}]}
        plan = build_next_steps_plan(forecast=forecast, compliance=compliance)
