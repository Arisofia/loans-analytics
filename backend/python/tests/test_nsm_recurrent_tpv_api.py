"""API tests for GET /analytics/nsm endpoint."""
...
def test_nsm_endpoint_returns_empty_response_when_no_runs_dir(tmp_path, monkeypatch):
    """When logs/runs dir is absent, endpoint returns 200 with empty NSM response."""
    # Point CWD to a temp dir that has no logs/runs directory
...
def test_nsm_endpoint_returns_empty_response_when_no_artifact(tmp_path, monkeypatch):
    """When a run dir exists but has no nsm_recurrent_tpv.json, returns empty response."""
    run_dir = tmp_path / "logs" / "runs" / "20260101_abc12345"
    run_dir.mkdir(parents=True)
    # No nsm_recurrent_tpv.json written — service must return empty response
    monkeypatch.chdir(tmp_path)
    response = client.get("/analytics/nsm")
    assert response.status_code == 200
    body = response.json()
...
def test_nsm_endpoint_happy_path(tmp_path, monkeypatch):
    """Returns full NSM breakdown when a valid nsm_recurrent_tpv.json artifact exists."""
    ...
    nsm_payload = {
        ...
        "latest": {
            "tpv_total": 10000.0,
            "tpv_recurrent": 6000.0,
            "tpv_new": 3000.0,
            "tpv_recovered": 1000.0,
            "active_clients": 20,
            "recurrent_clients": 12,
            "new_clients": 6,
            "recovered_clients": 2,
        },
        "by_period": {
...
def test_nsm_endpoint_selects_most_recently_modified_run(tmp_path, monkeypatch):
    """When multiple run dirs exist, the service picks the most recently modified one."""
    runs_root = tmp_path / "logs" / "runs"
    runs_root.mkdir(parents=True)

    # Older run — has an artifact with period "2025-12"
    old_run = runs_root / "20260101_aaa00000"
    old_run.mkdir()
    (old_run / "nsm_recurrent_tpv.json").write_text(
        json.dumps({"latest_period": "2025-12", "latest": None, "by_period": {}})
    )
    # Ensure newer run has a strictly later mtime
    time.sleep(0.1)

    # Newer run — has an artifact with period "2026-01"
    new_run = runs_root / "20260201_bbb11111"
    new_run.mkdir()
    (new_run / "nsm_recurrent_tpv.json").write_text(
        json.dumps({"latest_period": "2026-01", "latest": None, "by_period": {}})
    )
