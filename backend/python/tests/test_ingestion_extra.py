import pandas as pd
import pytest
from backend.src.pipeline.ingestion import IngestionPhase

@pytest.fixture
def base_config():
    return {'pipeline': {'phases': {'ingestion': {'validation': {'strict': False}}}}}

@pytest.mark.skip(reason='IngestionPhase implementation changed - _archive_raw method no longer exists')
def test_archive_raw_and_log(tmp_path, base_config):
    ui = IngestionPhase(base_config)
    src = tmp_path / 'data.csv'
    src.write_text('loan_id,total_receivable_usd\n1,100\n')
    archive_dir = tmp_path / 'archive'
    archived = ui._archive_raw(src, archive_dir)
    assert archived is not None
    assert archived.exists()
    assert any((e.get('event') == 'archive' and e.get('status') == 'success' for e in ui.audit_log))

@pytest.mark.skip(reason='IngestionPhase implementation changed - _apply_deduplication method no longer exists')
def test_apply_deduplication_config(tmp_path, base_config):
    cfg = base_config.copy()
    cfg['pipeline']['phases']['ingestion']['deduplication'] = {'enabled': True, 'key_columns': ['loan_id']}
    ui = IngestionPhase(cfg)
    df = pd.DataFrame({'loan_id': ['a', 'a', 'b'], 'total_receivable_usd': [10, 10, 20]})
    deduped, removed = ui._apply_deduplication(df)
    assert removed == 1
    assert len(deduped) == 2

@pytest.mark.skip(reason='IngestionPhase implementation changed - _validate_dataframe method no longer exists')
def test_validation_strict_raises(tmp_path):
    cfg = {'pipeline': {'phases': {'ingestion': {'validation': {'required_columns': ['loan_id', 'total_receivable_usd'], 'strict': True}}}}}
    ui = IngestionPhase(cfg)
    df = pd.DataFrame({'loan_id': ['a']})
    try:
        ui._validate_dataframe(df)
        raise AssertionError('Expected ValueError')
    except ValueError:
        pass
