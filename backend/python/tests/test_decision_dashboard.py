...
def _sample_loans_payload() -> dict:
    ...
    return {'loans': [{'id': 'L1', 'borrower_id': 'B1', 'loan_amount': 1000.0, 'principal_balance': 1000.0, 'interest_rate': 0.1, 'loan_status': 'current', 'days_past_due': 0.0, 'total_scheduled': 100.0, 'last_payment_amount': 90.0, 'origination_date': month_start.isoformat(), 'tpv': 1000.0}, {'id': 'L2', 'borrower_id': 'B2', 'loan_amount': 1500.0, 'principal_balance': 1200.0, 'interest_rate': 0.18, 'loan_status': '90+ days past due', 'days_past_due': 120.0, 'total_scheduled': 180.0, 'last_payment_amount': 20.0, 'origination_date': month_start.isoformat(), 'tpv': 1200.0}]}

def test_portfolio_health_score_traffic_light_levels():
    healthy = calculate_portfolio_health_score(par30=1.0, collection_rate=98.0, npl=1.0, cost_of_risk=0.5, default_rate=0.5)
    critical = calculate_portfolio_health_score(par30=25.0, collection_rate=60.0, npl=20.0, cost_of_risk=12.0, default_rate=12.0)
