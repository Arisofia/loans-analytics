"""Cross-module parity tests for SSOT-backed asset quality metrics."""
...
def _loans_snapshot() -> pd.DataFrame:
    return pd.DataFrame(
        {
            "loan_id": ["L1", "L2", "L3", "L4"],
            "outstanding_loan_value": [100.0, 200.0, 300.0, 400.0],
            "principal_balance": [100.0, 200.0, 300.0, 400.0],
            "days_past_due": [10, 35, 95, 210],
            "days_in_default": [10, 35, 95, 210],
            "loan_status": ["current", "current", "delinquent", "defaulted"],
            "pagador": ["P1", "P2", "P2", "P3"],
            "categorialineacredito": ["A", "A", "A", "A"],
            "disbursement_amount": [120.0, 220.0, 320.0, 420.0],
            "interest_rate_apr": [0.30, 0.32, 0.34, 0.36],
            "term": [45, 55, 65, 75],
        }
    )
    ...
    return pd.DataFrame(
        {
            ...
            "FechaPagoProgramado": due_dates.dt.strftime("%Y-%m-%d"),
        }
    )
...
def test_asset_quality_metrics_parity_across_consumers() -> None:
    ...
    canonical = calculate_asset_quality_metrics(
        balance,
        dpd,
        actor="parity_test",
        metric_aliases=("par30", "par60", "par90", "npl90", "npl180"),
        status=status,
    )
