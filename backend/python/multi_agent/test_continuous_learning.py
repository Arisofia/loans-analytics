"""Tests for agent commentary guarantees and continuous learning hooks."""
...
class TestContinuousLearningLoop(unittest.TestCase):
    ...
    def test_run_agent_rejects_empty_commentary(self):
        """Agents must return a non-empty comment/analysis."""
        role = AgentRole.RISK_ANALYST
        self.orchestrator.agents[role].process = MagicMock(
            return_value=self._response(role=role, trace_id="trace_empty", content="   ")
        )

        with self.assertRaises(ValueError):
            self.orchestrator.run_agent(
                role=role,
                messages=[Message(role=MessageRole.USER, content="Analyze risk")],
                trace_id="trace_empty",
            )

    def test_run_agent_tracks_learning_event(self):
        """Every successful agent response should feed learning telemetry."""
        role = AgentRole.COMPLIANCE
        self.orchestrator.agents[role].process = MagicMock(
            return_value=self._response(
                role=role,
                trace_id="trace_ok",
                content="Compliance review complete. No material issues found.",
            )
        )

        response = self.orchestrator.run_agent(
            role=role,
            messages=[Message(role=MessageRole.USER, content="Review compliance")],
            trace_id="trace_ok",
        )
        ...
        self.assertTrue(
            any(
                call.kwargs.get("feature_name") == "agent_response"
                and call.kwargs.get("action") == role.value
                for call in self.usage_tracker.track.call_args_list
            )
        )

    def test_run_scenario_collects_agent_comments(self):
        """Scenario execution should expose each agent's commentary explicitly."""
        ...
        def make_process(role: AgentRole):
            def _process(request):
                return self._response(
                    ...
                    content=f"{role.value} commentary generated.",
                )
        ...
        results = self.orchestrator.run_scenario(
            "loan_risk_review",
            initial_context={"portfolio_data": "sample portfolio"},
            trace_id="trace_scenario",
        )
        ...
        self.assertTrue(
            any(
                call.kwargs.get("feature_name") == "scenario_execution"
                and call.kwargs.get("action") == "loan_risk_review"
                for call in self.usage_tracker.track.call_args_list
            )
        )

    def test_feedback_updates_learning_summary(self):
        """Feedback must be aggregated for continuous learning summaries."""
        self.orchestrator.record_feedback(
            trace_id="trace_feedback_1",
            rating=5,
            comments="Strong risk rationale and actionable recommendations.",
            agent_role=AgentRole.RISK_ANALYST,
        )
        self.orchestrator.record_feedback(
            trace_id="trace_feedback_2",
            rating=3,
            comments="Needs clearer next-step prioritization.",
            agent_role=AgentRole.RISK_ANALYST,
        )
        ...
        self.assertTrue(
            any(
                call.kwargs.get("feature_name") == "agent_feedback"
                and call.kwargs.get("action") == "feedback_received"
                for call in self.usage_tracker.track.call_args_list
            )
        )
