"""Engine for computing Weight of Evidence (WoE) and Information Value (IV)."""
...
class WoEIVEngine:
    ...
    def analyze_all(self, df: pd.DataFrame, feature_cols: List[str]) -> Dict[str, float]:
        """Compute IV for all provided features and return a ranked dictionary."""
        iv_results = {}
    ...
    def compute_iv(self, df: pd.DataFrame, feat: str) -> Optional[float]:
        ...
        try:
            ...
            # Sum IV from all bins (excluding the totals row)
            # The binning table has a "Totals" row at the end
    ...
    def interpret_iv(self, iv: float) -> str:
        """Provide a human-readable interpretation of the IV value."""
        if iv < 0.02:
