...
class SegmentationModel:
    ...
    def _extract_late_flags(payment_df: pd.DataFrame, loan_id_col: str) -> pd.DataFrame:
        ...
        """
        status_col = SegmentationModel._find_col(
            payment_df, 'status', 'estado', 'estatus'
        )
        if status_col is None:
            empty_idx = payment_df[loan_id_col].unique()
            return pd.DataFrame(
                {'late_payment_rate': np.nan, 'mora_months': np.nan, 'mora_month_concentration': np.nan},
                index=empty_idx,
            )
        ...
        date_col = next(
            (c for c in payment_df.columns if 'date' in c or 'fecha' in c), None
        )
        if date_col:
        ...
        if '_month' in payment_df.columns:
            ...
            if late_by_month.empty:
                ...
                mora_months = (
                    late_by_month.groupby(loan_id_col)['_month']
                    .nunique()
                    .rename('mora_months')
                )
    ...
    @staticmethod
    def _extract_payment_volatility(
        payment_df: pd.DataFrame, loan_id_col: str
    ) -> pd.Series:
    ...
    def build_behavioral_features(
        ...
        dpd_col = self._find_col(
            loan_df,
            'days_past_due', 'dpd', 'dias_mora', 'dias_en_mora', 'days_in_default',
        )
    ...
    def fit(
        ...
        logger.info(
            'Scaling %d observations with %d features...', len(feat_matrix), len(available)
        )
        self.scaler = RobustScaler()
        x_scaled = self.scaler.fit_transform(feat_matrix.values)
        self._train_scaled: Optional[np.ndarray] = None  # stored for DBSCAN predict
    ...
    def predict(
        self, loan_df: pd.DataFrame, payment_df: pd.DataFrame
    ) -> pd.Series:
        """
