...
_METRIC_ALIAS_TO_ID: dict[str, str] = {'par30': 'par_30', 'par60': 'par_60', 'par90': 'par_90', 'npl': 'par_30', 'npl90': 'npl_90_proxy', 'npl180': 'npl_180_proxy'}
_ASSET_QUALITY_REGISTRY = {'version': 'asset-quality-ssot-1.1', 'asset_quality_kpis': {'par_30': {'formula': "SUM(outstanding_balance WHERE dpd >= 30 OR status IN ['delinquent', 'defaulted']) / SUM(outstanding_balance) * 100", 'unit': 'percentage'}, 'par_60': {'formula': "SUM(outstanding_balance WHERE dpd >= 60 OR status IN ['delinquent', 'defaulted']) / SUM(outstanding_balance) * 100", 'unit': 'percentage'}, 'par_90': {'formula': 'SUM(outstanding_balance WHERE dpd >= 90) / SUM(outstanding_balance) * 100', 'unit': 'percentage'}, 'npl_90_proxy': {'formula': "SUM(outstanding_balance WHERE dpd >= 90 OR status = 'defaulted') / SUM(outstanding_balance) * 100", 'unit': 'percentage'}, 'npl_180_proxy': {'formula': "SUM(outstanding_balance WHERE dpd >= 180 OR status = 'defaulted') / SUM(outstanding_balance) * 100", 'unit': 'percentage'}}}
