...
class KPIFormulaEngine:
    ...
    def __init__(self, df: pd.DataFrame, *, actor: str='system', run_id: Optional[str]=None, registry_path: Optional[str]=None, registry_data: Optional[dict]=None):
        self.df = df
    ...
    def calculate_kpi(self, kpi_name: str, *, strict_comparison_errors: bool=False) -> dict:
        ...
        result = {'value': value, 'unit': str(kpi_definition.get('unit', 'unknown')), 'formula_version': formula_version, 'execution_time_ms': execution_time_ms, 'data_rows': int(len(self.df)), 'actor': self.actor, 'timestamp': end.isoformat(), 'kpi_name': kpi_name, 'run_id': self.run_id, 'thresholds': thresholds, 'threshold_status': threshold_status, 'status': 'success', 'columns_used': columns_used}
    ...
    def _get_registry_data(self) -> dict:
        ...
        path = Path(self._registry_path) if self._registry_path else Path(__file__).resolve().parents[3] / 'config' / 'kpis' / 'kpi_definitions.yaml'
    ...
    def _record_audit(self, *, kpi_name: str, formula: str, result: dict) -> None:
        self._audit_records.append({'timestamp': result['timestamp'], 'kpi_name': kpi_name, 'formula': formula, 'formula_version': result['formula_version'], 'actor': result['actor'], 'run_id': result['run_id'], 'data_rows': result['data_rows'], 'execution_time_ms': result['execution_time_ms'], 'result': str(result['value']), 'unit': result['unit'], 'threshold_status': result.get('threshold_status', 'not_configured'), 'status': result.get('status', 'success'), 'columns_used': result.get('columns_used', [])})
    ...
    def _evaluate_threshold_status(value: Decimal, thresholds: dict) -> str:
        ...
        if two_level is not None:
            return two_level
        critical_only = KPIFormulaEngine._evaluate_single_threshold(value=value, level_name='critical', level_value=critical, reference=target)
        if critical_only is not None:
            return critical_only
        warning_only = KPIFormulaEngine._evaluate_single_threshold(value=value, level_name='warning', level_value=warning, reference=target)
        if warning_only is not None:
            return warning_only
        return 'not_configured'

    @staticmethod
    def _evaluate_two_level_thresholds(value: Decimal, warning: Optional[Decimal], critical: Optional[Decimal]) -> Optional[str]:
    ...
    @staticmethod
    def _evaluate_single_threshold(*, value: Decimal, level_name: str, level_value: Optional[Decimal], reference: Optional[Decimal]) -> Optional[str]:
    ...
    def calculate(self, formula: str, *, strict_comparison_errors: bool=False) -> Decimal:
        ...
        try:
            if self._is_comparison_formula(formula):
                return self._execute_comparison_formula(formula, strict_errors=strict_comparison_errors)
    ...
    @staticmethod
    def _eval_binary_operation(operator: ast.AST, left: Decimal, right: Decimal, strict: bool=False) -> Decimal:
    ...
    def _build_comparison_context(self) -> Dict[str, Decimal]:
        ...
        return {'current_month_balance': current_balance, 'previous_month_balance': previous_balance}

    def _resolve_monthly_balances(self) -> tuple[Decimal, Decimal]:
        if 'outstanding_balance' not in self.df.columns:
            return (Decimal('0.0'), Decimal('0.0'))
        snapshot_date_candidates = ['measurement_date', 'snapshot_date', 'as_of_date', 'reporting_date', 'data_ingest_ts']
        date_column = next((col for col in snapshot_date_candidates if col in self.df.columns), None)
        if date_column is None and os.getenv('KPI_ENGINE_ALLOW_ORIGINATION_FALLBACK', '0') == '1':
            fallback_candidates = ['origination_date', 'disbursement_date', 'last_payment_date', 'maturity_date']
            date_column = next((col for col in fallback_candidates if col in self.df.columns), None)
        if date_column is None:
            logger.info('Monthly comparison skipped: no snapshot date column found (set KPI_ENGINE_ALLOW_ORIGINATION_FALLBACK=1 to enable legacy fallback).')
            return (Decimal('0.0'), Decimal('0.0'))
        if self._polars_enabled and len(self.df) >= 100000:
            try:
                import polars as pl
                pl_df = pl.from_pandas(self.df[[date_column, 'outstanding_balance']].copy()).with_columns(pl.col(date_column).cast(pl.Datetime, strict=False).alias('date'), pl.col('outstanding_balance').cast(pl.Float64, strict=False).fill_null(0.0).alias('balance'))
                pl_df = pl_df.select(['date', 'balance']).filter(pl.col('date').is_not_null())
                if pl_df.is_empty():
                    return (Decimal('0.0'), Decimal('0.0'))
                monthly = pl_df.with_columns(pl.col('date').dt.truncate('1mo').alias('month')).group_by('month').agg(pl.col('balance').sum().alias('balance_sum')).sort('month')
                if monthly.height == 0:
                    return (Decimal('0.0'), Decimal('0.0'))
                month_to_balance = {pd.Timestamp(row['month']).to_period('M'): float(row['balance_sum']) for row in monthly.select(['month', 'balance_sum']).to_dicts()}
        ...
        period_df = pd.DataFrame({'date': self._get_datetime_series(date_column), 'balance': self._get_numeric_series('outstanding_balance').fillna(0.0)}).dropna(subset=['date'])
        if period_df.empty:
            return (Decimal('0.0'), Decimal('0.0'))
        period_df['period'] = period_df['date'].dt.to_period('M')
        current_period = period_df['period'].max()
        previous_period = current_period - 1
        current_balance = Decimal(str(period_df.loc[period_df['period'] == current_period, 'balance'].sum()))
        previous_balance = Decimal(str(period_df.loc[period_df['period'] == previous_period, 'balance'].sum()))
        return (current_balance, previous_balance)

    def _execute_arithmetic_formula(self, formula: str) -> Decimal:
        expression = re.sub('(SUM|AVG|COUNT)\\([^)]+\\)', self._replace_aggregation_match, formula, flags=re.IGNORECASE)
        try:
    ...
    def _execute_simple_formula(self, formula: str) -> Decimal:
        ...
        if agg_func == 'SUM':
            ...
            result = Decimal(str(filtered_df[column].nunique() if distinct else filtered_df[column].count()))
        return result
