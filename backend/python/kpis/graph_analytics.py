...
try:
    ...
    nx = cast(Any, None)
    _NX = False
    logger.warning('networkx not installed — graph modules will return empty results. Install with: pip install networkx')
try:
...
def _calculate_ssot_npl_metrics(balance: pd.Series, dpd: pd.Series) -> tuple[float, float]:
    values = calculate_asset_quality_metrics(balance, dpd, actor='graph_analytics', metric_aliases=('npl90', 'npl180'))
...
def _build_dpd_lookup(intermedia_df: pd.DataFrame) -> tuple[dict[str, float], dict[str, float]]:
    ...
    if dpd_col:
        ...
        if fpp_col:
            ...
            df['_dpd'] = 0.0
    active = df[df['_bal'] > 0]
    return (active.groupby('_cod')['_dpd'].mean().to_dict(), active.groupby('_cod')['_bal'].sum().to_dict())
...
def _compute_hhi(series: pd.Series, df: pd.DataFrame, total: float) -> dict:
    ...
    return {'hhi': round(hhi_val, 1), 'equivalent_n': round(1 / (hhi_val / 10000), 1) if hhi_val > 0 else 0, 'concentration_level': _concentration_level(hhi_val), 'top1_pct': round(float(top.iloc[0] / total * 100), 1) if len(top) > 0 else 0, 'top3_pct': round(float(top.head(3).sum() / total * 100), 1), 'top10_pct': round(float(top.head(10).sum() / total * 100), 1), 'top1_name': str(top.index[0]) if len(top) > 0 else '', 'top1_balance_usd': round(float(top.iloc[0]), 2) if len(top) > 0 else 0}
...
def _k_factor_from_channel(loans_df: pd.DataFrame, cust_col: str, ch_col: str, conversion_rate: float | None) -> dict:
    ...
    return {'k_factor': round(k, 3), 'avg_invitations': avg_invitations, 'conversion_rate': round(conv, 3), 'organic_clients': n_organic, 'total_clients': n_total, 'organic_rate_pct': round(referral_rate * 100, 1), 'confidence': 'medium', 'data_source': f'sales_channel column ({ch_col})', 'growth_status': 'exponential' if k >= 1.0 else 'sub-viral', 'to_reach_k1': f'Need conversion rate >= {1 / avg_invitations:.1%} OR avg invites >= {1 / max(conv, 0.001):.1f} per client'}

def _score_community(comm: dict, dpd_by_client: dict[str, float], bal_by_client: dict[str, float], dpd_threshold: int, avg_density: float, graph: Any) -> dict | None:
    ...
    sub_nodes = [f'C:{cid}' for cid in client_ids] + [f'D:{did}' for did in comm.get('debtor_ids', [])]
    ...
    return {'community_id': comm['community_id'], 'flag_types': flags, 'size': comm['size'], 'client_count': comm['client_count'], 'client_ids': client_ids[:15], 'avg_dpd': round(avg_dpd, 1), 'total_balance_usd': round(total_bal, 2), 'risk_score': risk_score}
...
def _compute_loan_tape_hhi(loans_df: pd.DataFrame) -> dict | None:
    ...
    return {'hhi': round(hhi_lt, 1), 'top1_name': str(top_lt.index[0]) if len(top_lt) > 0 else '', 'top1_pct': round(float(top_lt.iloc[0] / total_lt * 100), 1), 'top10_pct': round(float(top_lt.head(10).sum() / total_lt * 100), 1)}

def _k_factor_from_repeat(loans_df: pd.DataFrame, cust_col: str, conversion_rate: float | None) -> dict:
    ...
    return {'k_factor': round(proxy_k, 3), 'conversion_rate': conv, 'repeat_clients': repeat, 'total_clients': total, 'confidence': 'low', 'data_source': 'repeat_borrower_rate proxy (no referral col)', 'growth_status': 'exponential' if proxy_k >= 1.0 else 'sub-viral', 'note': "Low confidence — add referral/invite tracking to CRM to get accurate K. Recommended: tag sales_channel='referral' in HubSpot for organic clients."}

def _build_intermedia_edges(graph: Any, intermedia_df: pd.DataFrame, active_only: bool=True) -> None:
...
def _build_loan_edges(graph: Any, loans_df: pd.DataFrame) -> None:
...
def build_transaction_graph(intermedia_df: pd.DataFrame, loans_df: pd.DataFrame | None=None, active_only: bool=True) -> Any | None:
    if not _NX:
        return None
    graph = nx.Graph()
    ...
    logger.info('build_transaction_graph: %d nodes (%d clients, %d debtors), %d edges', graph.number_of_nodes(), sum((d.get('node_type') == 'client' for n, d in graph.nodes(data=True))), sum((d.get('node_type') == 'debtor' for n, d in graph.nodes(data=True))), graph.number_of_edges())
    return graph

def pagerank_scores(graph: Any | None, alpha: float=0.85, weight: str='weight') -> dict[str, Any]:
    ...
    for node, score in pr.items():
        ...
        if ntype == 'client':
            client_rows.append({'id': nid, 'pagerank': round(score, 6), 'degree': deg, 'total_exposure_usd': round(exp, 2)})
        elif ntype == 'debtor':
            debtor_rows.append({'id': nid, 'pagerank': round(score, 6), 'client_count': deg, 'total_exposure_usd': round(exp, 2)})
    if client_rows:
    ...
    return {'status': 'ok', 'client_scores': client_rows, 'debtor_scores': debtor_rows[:50], 'high_trust_threshold': high_thresh, 'graph_stats': {'nodes': graph.number_of_nodes(), 'edges': graph.number_of_edges(), 'density': round(nx.density(graph), 6), 'avg_degree': round(sum((d for _, d in graph.degree())) / max(graph.number_of_nodes(), 1), 2)}, 'top_clients': client_rows[:10], 'top_debtors': debtor_rows[:10]}

def detect_communities(graph: Any | None) -> dict[str, Any]:
    if graph is None or not _NX or graph.number_of_nodes() == 0:
        return {'status': 'unavailable', 'reason': 'networkx not installed or empty graph'}
    if _LOUVAIN:
    ...
    for comm_id, nodes in community_map.items():
        ...
        rows.append({'community_id': comm_id, 'size': len(nodes), 'client_count': len(clients), 'debtor_count': len(debtors), 'client_ids': [graph.nodes[n].get('id', n) for n in clients][:20], 'debtor_ids': [graph.nodes[n].get('id', n) for n in debtors][:10], 'flag_review': len(clients) >= 3 and len(debtors) >= 2})
    rows.sort(key=lambda x: int(cast(int, x['size'])), reverse=True)
    return {'status': 'ok', 'num_communities': len(rows), 'modularity': round(modularity, 4) if modularity is not None else None, 'algorithm': algo, 'communities': rows[:50], 'flagged_for_review': sum((r['flag_review'] for r in rows))}

def detect_fraud_rings(graph: Any | None, intermedia_df: pd.DataFrame | None=None, dpd_threshold: int=30, min_community_size: int=3) -> dict[str, Any]:
    ...
    return {'status': 'ok', 'fraud_rings': rings, 'total_rings_flagged': len(rings), 'flagged_balance_usd': round(flagged_bal, 2)}

def concentration_hhi(intermedia_df: pd.DataFrame, loans_df: pd.DataFrame | None=None) -> dict[str, Any]:
    bal_col = _col(intermedia_df, ['TotalSaldoVigente', 'outstanding_balance'])
...
def calc_unit_economics(loans_df: pd.DataFrame, payments_df: pd.DataFrame, marketing_spend_usd: float | None=None, trailing_months: int=12) -> dict[str, Any]:
    ...
    if marketing_val is None:
        if amt_col:
            ...
            marketing_val = float(total_disb * 0.01)
        else:
            marketing_val = 0.0
    cac = marketing_val / max(new_clients, 1)
    ltv, avg_lifetime_months = _estimate_ltv(loans_df, payments_df, cust_col, disb_col, pay_date, pay_amt, window_start, trailing_months)
...
def viral_k_factor(loans_df: pd.DataFrame, conversion_rate: float | None=None) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {'status': 'ok', 'k_target': 1.0, 'interpretation': 'K > 1 = exponential growth; K < 1 = decays without paid acquisition'}
...
def npl_benchmarks(intermedia_df: pd.DataFrame) -> dict[str, Any]:
    bal_col = _col(intermedia_df, ['TotalSaldoVigente'])
...
def build_graph_kpi_report(intermedia_df: pd.DataFrame, loans_df: pd.DataFrame, payments_df: pd.DataFrame) -> dict[str, Any]:
    for col in ['TotalSaldoVigente', 'MontoDesembolsado', 'ValorAprobado']:
        if col in intermedia_df.columns:
            intermedia_df[col] = pd.to_numeric(intermedia_df[col].astype(str).str.replace('[\\$,]', '', regex=True), errors='coerce').fillna(0)
    ...
    def graph_adjusted_score(base_score: float, pagerank: float, community_risk: str) -> dict:
        ...
        if community_risk == 'HIGH':
            adjusted *= 0.5
        return {'base_score': base_score, 'pagerank': pagerank, 'community_risk': community_risk, 'adjusted_score': round(adjusted, 1), 'decision': 'APPROVE' if adjusted > 700 else 'REVIEW'}
    return {'generated_at': datetime.now(timezone.utc).isoformat(), 'graph': {'nodes': G.number_of_nodes() if G else 0, 'edges': G.number_of_edges() if G else 0, 'networkx_available': _NX}, 'pagerank': pr, 'communities': comms, 'fraud_rings': rings, 'concentration_hhi': hhi, 'unit_economics': ue, 'viral_k_factor': k_factor, 'npl_benchmarks': npl, 'decision_logic_example': graph_adjusted_score(750, 0.9, 'LOW'), 'copilot_instructions': {'to_add_referral_data': "Add 'sales_channel=referral' in HubSpot → export to customer.csv", 'to_improve_pd_auc': 'Join Equifax score + payment cadence features to build_pd_model()', 'to_activate_graph_score': "Use pagerank['client_scores'] in decision_logic() above", 'to_connect_neo4j': 'Replace build_transaction_graph() with Neo4j GDS PageRank query'}}
