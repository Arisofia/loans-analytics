...
def detect_exposure_weighted_outliers(loans_df: pd.DataFrame, z_threshold: float=2.58) -> dict[str, Any]:
    ...
    for var_name, series in variables.items():
        ...
        if w_std == 0:
            summary[var_name] = {'mean_w': w_mean, 'std_w': 0.0, 'outlier_count': 0, 'outlier_balance_usd': 0.0}
            continue
        z_scores = (series - w_mean) / w_std
        flag_mask = z_scores.abs() > z_threshold
        flag_bal = float(bal[flag_mask].sum())
        summary[var_name] = {'mean_w': round(w_mean, 4), 'std_w': round(w_std, 4), 'outlier_count': int(flag_mask.sum()), 'outlier_balance_usd': round(flag_bal, 2)}
        if flag_mask.any():
...
def _run_pd_cross_validation(clf: Any, X_scaled: Any, y: pd.Series, weights: pd.Series, cv_folds: int) -> Any:
    ...
    try:
        return cross_val_score(clf, X_scaled, y, cv=skf, scoring='roc_auc', params={'sample_weight': weights.values})
    except TypeError:
...
def predict_kpis(loans_df: pd.DataFrame, payments_df: pd.DataFrame, horizon_months: int=6) -> dict[str, Any]:
    ...
    if disb_col and disb_amt_col:
        ...
        aum_series = loans_df.groupby('_month')[disb_amt_col].apply(lambda s: pd.to_numeric(s, errors='coerce').fillna(0).sum()).tail(12)
    ...
    if pay_date_col and pay_amt_col:
        payments_df['_month'] = pd.to_datetime(payments_df[pay_date_col], errors='coerce').dt.to_period('M')
        rev_series = payments_df.groupby('_month')[pay_amt_col].apply(lambda s: pd.to_numeric(s, errors='coerce').fillna(0).sum()).tail(12)
    ...
    if dpd_col and disb_col and bal_col:
        ...
        for month, month_df in df_par.groupby('_month'):
            ...
            try:
                month_metrics = _calculate_ssot_asset_quality_metrics(balance=month_df['_bal'], dpd=month_df['_dpd'], status=month_status)
    ...
    return {'horizon_months': horizon_months, 'generated_at': datetime.now(timezone.utc).isoformat(), 'aum_forecast': _linear_forecast(aum_series, horizon_months), 'revenue_forecast': _linear_forecast(rev_series, horizon_months), 'par30_forecast': _linear_forecast(par30_series, horizon_months), 'par90_forecast': _linear_forecast(par90_series, horizon_months), 'methodology': 'linear_trend_1.5sigma_bands', 'data_notes': {'aum': 'monthly disbursement_amount by origination month (bullet-loan proxy)', 'par': 'vintage approximation from single DPD snapshot — no historical PAR archives', 'revenue': 'true_total_payment per payment month from real_payment.csv'}, 'training_months': {'aum': int(len(aum_series)), 'revenue': int(len(rev_series)), 'par30': int(len(par30_series))}}
...
def build_compliance_dashboard(loans_df: pd.DataFrame, payments_df: pd.DataFrame, guardrails: dict | None=None, owners: dict | None=None) -> dict[str, Any]:
    ...
    status_col = _col(loans_df, ['loan_status', 'status', 'current_status'])
    status_series = loans_df[status_col].astype(str) if status_col and status_col in loans_df.columns else pd.Series(['active'] * len(loans_df), index=loans_df.index)
    try:
    ...
    if util_col:
        ...
        util_source = 'NO_DATA'
    data_sources = {'par': f'loan.{dpd_col}' if dpd_col else 'NO_DATA', 'concentration': f'loan.{deb_col}' if deb_col else 'NO_DATA', 'ce_6m': 'payments.true_total_payment / loan.disbursement_amount (6m proxy)' if not sched else f'payments.true_total_payment / loan.{sched}', 'utilization': util_source, 'apr': f'loan.{apr_col} (annual decimal ×100)' if apr_col else 'NO_DATA', 'dscr': f'loan.{noi_col} / loan.{debt_service_col}' if noi_col and debt_service_col else 'NO_DATA'}
    metric_defs = {'rotation_x': ('rotation_x', 4.5, False, 0.3), 'top1_concentration_pct': ('top1_concentration_pct', 4.0, True, 1.0), 'top10_concentration_pct': ('top10_concentration_pct', 30.0, True, 3.0), 'par30_pct': ('par30_pct', 5.0, True, 1.0), 'par90_pct': ('par90_pct', 2.0, True, 0.5), 'npl_180_pct': ('npl_180_pct', 4.0, True, 0.5), 'utilization_pct': ('utilization_pct', 75.0, False, 5.0), 'dscr': ('dscr', float(guardrails.get('dscr', 1.2)) if guardrails else 1.2, False, 0.1), 'ce_6m_pct': ('ce_6m_pct', 96.0, False, 2.0)}
    ...
    for metric, (act_key, target, lower_is_better, warn_band) in metric_defs.items():
        ...
        if lower_is_better:
            ...
            stat = 'breach'
        counts[stat] += 1
        rows.append({'metric': metric, 'actual': actual, 'target': target, 'variance': round(var, 2), 'variance_pct': var_pct, 'status': stat, 'lower_is_better': lower_is_better, 'owner': owners.get(metric, '—')})
    apr_actual = actuals.get('apr_pct_ann')
    if apr_actual is None:
        counts['no_data'] += 1
        rows.append({'metric': 'apr_pct_ann', 'actual': 'NO_DATA', 'target': f'{apr_min:.1f}-{apr_max:.1f}', 'variance': None, 'variance_pct': None, 'status': 'no_data', 'lower_is_better': False, 'owner': owners.get('apr_pct_ann', owners.get('apr_pct_min', HEAD_OF_PRICING))})
    else:
        apr_warn_band = 2.0
        if apr_actual < apr_min:
            ...
            apr_stat = 'ok'
        counts[apr_stat] += 1
        rows.append({'metric': 'apr_pct_ann', 'actual': apr_actual, 'target': f'{apr_min:.1f}-{apr_max:.1f}', 'variance': apr_var, 'variance_pct': apr_var_pct, 'status': apr_stat, 'lower_is_better': False, 'owner': owners.get('apr_pct_ann', owners.get('apr_pct_min', HEAD_OF_PRICING))})
    variance_decomp = {'top1_concentration_pct': {'driver': 'La Constancia LTDA — single pagador dominates', 'magnitude': round(top1_pct - 4.0, 1), 'explanation': f'La Constancia represents ~{top1_pct:.0f}% of AUM vs 4% guardrail. Reduce exposure by originating with competing debtors or applying per-debtor cap.'}, 'par30_pct': {'driver': 'Plazo extension + 2026 payment recording lag', 'magnitude': round(par30 - 5.0, 1), 'explanation': 'PAR30 elevated; partial cause is INTERMEDIA DPD calculated from FechaPagoProgramado without 2026 payment records. Verify with real collections.'}, 'par90_pct': {'driver': 'Structural: factoring ops with extended terms breach 90-day bucket', 'magnitude': round(par90 - 2.0, 1), 'explanation': 'PAR90 guardrail (2%) is aggressive for a 46-day median term book. Recommend reviewing guardrail calibration against peer benchmarks.'}, 'rotation_x': {'driver': 'AUM growth outpacing disbursement velocity in trailing 12m', 'magnitude': round(rotation_actual - 4.5, 2), 'explanation': f'Rotation {rotation_actual:.1f}x vs target 4.5x. Accelerate collections cycle or reduce average term to recover velocity.'}}
    if apr_actual is None:
        variance_decomp['apr_pct_ann'] = {'driver': 'APR source unavailable', 'magnitude': None, 'explanation': 'APR cannot be computed because no APR column was found in the loan tape.'}
    elif apr_actual < apr_min:
        variance_decomp['apr_pct_ann'] = {'driver': 'Pricing below target floor', 'magnitude': round(apr_actual - apr_min, 1), 'explanation': f'Weighted APR {apr_actual:.1f}% is below minimum {apr_min:.1f}%. Review pricing floors and discount approvals by segment.'}
    elif apr_actual > apr_max:
        variance_decomp['apr_pct_ann'] = {'driver': 'Pricing above target ceiling', 'magnitude': round(apr_actual - apr_max, 1), 'explanation': f'Weighted APR {apr_actual:.1f}% exceeds maximum {apr_max:.1f}%. Review competitiveness and channel mix impacts.'}
    else:
        variance_decomp['apr_pct_ann'] = {'driver': 'APR within policy corridor', 'magnitude': 0.0, 'explanation': f'Weighted APR {apr_actual:.1f}% is inside the configured range {apr_min:.1f}% to {apr_max:.1f}%.'}
    dscr_target = float(guardrails.get('dscr', 1.2)) if guardrails else 1.2
    if dscr_actual is None:
        variance_decomp['dscr'] = {'driver': 'DSCR inputs unavailable', 'magnitude': None, 'explanation': 'DSCR cannot be computed because NOI and debt service fields are not present.'}
    else:
        variance_decomp['dscr'] = {'driver': 'Income coverage vs debt service', 'magnitude': round(dscr_actual - dscr_target, 2), 'explanation': f'DSCR {dscr_actual:.2f}x vs target {dscr_target:.2f}x. Improve cash generation or reduce scheduled debt service to lift coverage.'}
...
def _load_compliance_actions(compliance: dict[str, Any], variance_decomp: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in compliance.get('metrics', []):
        ...
        if row['status'] in ('breach', 'warning'):
            ...
            actions.append({'area': area, 'action': action_text, 'impact': impact, 'effort': effort, 'source': 'compliance', 'metric': row['metric'], 'variance': row.get('variance'), '_sort_key': (0 if impact == 'high' else 1, row.get('variance', 0))})
            ...
            if vd_driver:
                ...
                action_text = f'{action_text} Context: {vd_expl}'
            actions.append({'area': area, 'action': action_text, 'impact': impact, 'effort': effort, 'source': 'compliance', 'metric': row['metric'], 'variance': None, '_sort_key': (0 if impact == 'high' else 1, 0)})
    return actions
...
def _load_pd_model_actions(pd_result: dict[str, Any] | None) -> list[dict[str, Any]]:
    if not (pd_result and pd_result.get('status') == 'ok'):
        return []
    top_pd = [r for r in pd_result.get('pd_scores', []) if r['pd_score'] >= 0.5]
    if not top_pd:
        return []
