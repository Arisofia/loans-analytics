...
import pandas as pd
KPI_LABEL_OVERRIDES = {'total_aum_usd': 'Total AUM (USD)', 'ltv_cac_ratio': 'LTV / CAC', 'par_90_ratio_pct': 'PAR 90 Ratio', 'mom_growth_pct': 'MoM Growth', 'yoy_growth_pct': 'YoY Growth'}
...
def format_kpi_value(name: str, value: Any) -> str:
    ...
    currency_hints = ('usd', 'revenue', 'outstanding', 'disbursement', 'fee', 'interest', 'aum', 'capital', 'payment', 'received', 'sched')
...
def compute_cat_agg(frame: pd.DataFrame) -> pd.DataFrame:
    if 'categoria' not in frame.columns or 'outstanding_loan_value' not in frame.columns:
        return pd.DataFrame()
    category_totals = frame.groupby('categoria', dropna=False)['outstanding_loan_value'].sum().reset_index()
