...
def dollars_to_cents(amount_dollars: Union[float, str, Decimal]) -> int:
    ...
    cents = int((decimal_amount * Decimal(CENTS_PER_DOLLAR)).quantize(
...
def interest_rate_to_basis_points(rate_decimal: Union[float, Decimal]) -> int:
    ...
    basis_points = int((decimal_rate * Decimal(INTEREST_RATE_SCALE)).quantize(
        Decimal('1'), rounding=ROUNDING_MODE
    ))
...
def safe_decimal_divide(numerator: Union[int, Decimal], 
                       denominator: Union[int, Decimal],
