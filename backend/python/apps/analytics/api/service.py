...
from backend.python.apps.analytics.api.models import AdvancedRiskResponse, AnalysisLayer, CohortAnalyticsResponse, CohortAnalyticsSummary, CohortMetrics, CostOfRiskMetrics, CureRateMetrics, DataQualityResponse, DecisionDashboardResponse, DPDBucketWithAction, DecisionFlag, KpiContext, KpiSingleResponse, LgdMetrics, LoanRecord, NimMetrics, NplMetrics, NSMPeriodMetrics, NSMRecurrentTPVResponse, PaybackMetrics, PortfolioHealthComponent, PortfolioHealthScore, RiskHeatmapResponse, RiskStratificationResponse, RollRateAnalyticsResponse, RollRateAnalyticsSummary, RollRateBucketSummary, RollRateTransition, SegmentAnalyticsResponse, SegmentAnalyticsSummary, SegmentMetrics, StressTestAssumptions, StressTestMetrics, StressTestResponse, UnitEconomicsResponse, ValidationResponse, VintageCurvePoint, VintageCurveResponse
...
_PORTFOLIO_HEALTH_FORMULA = 'PAR30(25pts) + CollectionRate(25pts) + NPL(20pts) + ' + 'CostOfRisk(15pts) + DefaultRate(15pts)'
try:
...
def _extract_kpi_metadata(kpi_def: dict[str, Any]) -> dict[str, Any]:
    ...
    return {'formula': str(kpi_def.get('formula', '')), 'definition': str(kpi_def.get('description', '')), 'implications': f'Use trend and segment context when interpreting this KPI.{threshold_note}', 'thresholds': thresholds, 'benchmark': thresholds.get('target')}
...
class KPIService:
    ...
    def get_exposed_aliases(self) -> dict[str, list[str]]:
        return {'PAR30': ['PAR30', 'par_30'], 'PAR60': ['PAR60', 'par_60'], 'PAR90': ['PAR90', 'par_90'], 'DPD1_30': ['DPD_1_30', 'dpd_1_30'], 'DPD31_60': ['DPD_31_60', 'dpd_31_60'], 'DPD61_90': ['DPD_61_90', 'dpd_61_90'], 'DPD90Plus': ['DPD_90_PLUS', 'dpd_90_plus'], 'CollectionRate': ['COLLECTION_RATE', 'collections_rate'], 'DefaultRate': ['DEFAULT_RATE', 'default_rate'], 'TotalLoansCount': ['TOTAL_LOANS_COUNT', 'total_loans_count'], 'LossRate': ['LOSS_RATE', 'loss_rate'], 'RecoveryRate': ['RECOVERY_RATE', 'recovery_rate'], 'CashOnHand': ['CASH_ON_HAND', 'cash_on_hand'], 'PortfolioHealth': ['AUM', 'portfolio_growth_rate'], 'CustomerLifetimeValue': ['CUSTOMER_LIFETIME_VALUE', 'customer_lifetime_value'], 'ActiveBorrowers': ['ACTIVE_BORROWERS', 'active_borrowers'], 'RepeatBorrowerRate': ['REPEAT_BORROWER_RATE', 'repeat_borrower_rate'], 'AutomationRate': ['AUTOMATION_RATE', 'automation_rate'], 'AverageLoanSize': ['AVERAGE_LOAN_SIZE', 'average_loan_size'], 'ProcessingTimeAvg': ['PROCESSING_TIME_AVG', 'processing_time_avg'], 'DisbursementVolumeMTD': ['DISBURSEMENT_VOLUME_MTD', 'disbursement_volume_mtd'], 'NewLoansCountMTD': ['NEW_LOANS_COUNT_MTD', 'new_loans_count_mtd'], 'CAC': ['CAC', 'cac'], 'GrossMarginPct': ['GROSS_MARGIN_PCT', 'gross_margin_pct'], 'RevenueForecast6M': ['REVENUE_FORECAST_6M', 'revenue_forecast_6m'], 'Churn90D': ['CHURN_90D', 'churn_90d'], 'LTV': ['AVG_LTV', 'avg_ltv'], 'DTI': ['AVG_DTI', 'avg_dti'], 'PortfolioYield': ['PORTFOLIO_YIELD', 'portfolio_yield'], 'NPL': ['NPL', 'npl_ratio'], 'LGD': ['LGD', 'lgd_pct'], 'CoR': ['COR', 'cost_of_risk_pct'], 'NIM': ['NIM', 'nim_pct'], 'CureRate': ['CURERATE', 'cure_rate_pct']}
    ...
    def _get_kpi_metadata(self, kpi_id: str, kpi_name: str | None=None) -> dict[str, Any]:
        ...
        return {'formula': default_metadata.get('formula') or catalog_metadata.get('formula') or 'Not available', 'definition': default_metadata.get('definition') or catalog_metadata.get('definition') or f'KPI metric for {definition_fallback}', 'implications': default_metadata.get('implications') or catalog_metadata.get('implications') or 'Interpret with trend, segmentation, and risk appetite context.', 'thresholds': thresholds, 'benchmark': benchmark}

    @staticmethod
    def _eval_higher_is_riskier(value: float, critical: float, warning: float, target: float | None) -> Literal['below_target', 'on_target', 'warning', 'critical']:
        if value >= critical:
            return 'critical'
        if value >= warning:
            return 'warning'
    ...
    @staticmethod
    def _eval_lower_is_riskier(value: float, critical: float, warning: float, target: float | None) -> Literal['below_target', 'on_target', 'warning', 'critical']:
        if value <= critical:
            return 'critical'
        if value <= warning:
            return 'warning'
    ...
    @staticmethod
    def _evaluate_kpi_status(value: float, thresholds: dict[str, float]) -> Literal['below_target', 'on_target', 'warning', 'critical', 'unknown']:
        if not thresholds:
            return 'unknown'
    ...
    def _build_kpi_single_response(self, *, kpi_id: str, name: str, value: float, unit: str, timestamp: datetime, sample_size: int, period: str, filters: dict[str, Any]) -> KpiSingleResponse:
        ...
        return KpiSingleResponse(id=kpi_id, name=name, value=rounded_value, unit=unit, status=status, threshold_status=self._to_threshold_status(status), benchmark=benchmark_value, thresholds=thresholds or None, formula=formula, definition=metadata['definition'], implications=metadata['implications'], context=KpiContext(metric=name, timestamp=timestamp, formula=formula, sample_size=sample_size, period=period, calculation_date=timestamp, filters=filters))

    @staticmethod
    def _to_threshold_status(status: Literal['below_target', 'on_target', 'warning', 'critical', 'unknown']) -> Literal['normal', 'warning', 'critical', 'not_configured']:
        if status == 'critical':
            return 'critical'
        if status == 'warning':
            return 'warning'
    ...
    async def get_latest_kpis(self, kpi_keys: list[str] | None=None) -> list[KpiSingleResponse]:
        ...
        try:
            ...
            for rec in records:
                ...
                responses.append(self._build_kpi_single_response(kpi_id=kpi_id, name=kpi_name, value=float(rec['value']), unit=rec['unit'], timestamp=rec['created_at'], sample_size=0, period='latest', filters={'as_of_date': as_of_date_str}))
            return responses
    ...
    async def get_risk_alerts(self, loans: list[LoanRecord] | None, ltv_threshold: float=80.0, dti_threshold: float=50.0) -> list[dict]:
        try:
            if loans is None:
                return []
    ...
    def _calculate_loan_risk_metrics(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'days_past_due' not in df.columns:
            ...
            if null_mask.any():
                df.loc[null_mask, 'days_past_due'] = df.loc[null_mask, 'loan_status'].apply(self._dpd_from_status)
        return df

    def _build_loan_risk_alert(self, rec: pd.Series, ltv_threshold: float, dti_threshold: float) -> dict:
        ltv = rec['ltv']
        dti = rec['dti']
    ...
    def _calculate_executive_analytics_sync(self, loans: list[LoanRecord], payments: list[dict] | None=None, customers: list[dict] | None=None, schedule: list[dict] | None=None) -> dict:
        loans_df = self._convert_loan_records_to_dataframe(loans)
        payments_df = self._convert_dict_records_to_dataframe(payments)
        customers_df = self._convert_dict_records_to_dataframe(customers)
        schedule_df = self._convert_dict_records_to_dataframe(schedule)
        ...
        if loans_df.empty:
            ...
            portfolio_health = PortfolioHealthScore(score=DATA_MISSING_SCORE, traffic_light='critical', components=[], formula=_PORTFOLIO_HEALTH_FORMULA, interpretation=DATA_MISSING_INTERPRETATION)
        else:
            metrics = self._calculate_portfolio_performance_metrics(loans_df.copy())
            risk_kpis = self._build_risk_kpi_snapshot_from_metrics(metrics, sample_size=len(loans_df), timestamp=now)
            portfolio_health = self._build_portfolio_health_score_from_metrics(metrics)
        return {'strategic_confirmations': extended_kpis.get('strategic_confirmations', {}), 'executive_strip': extended_kpis.get('executive_strip', {}), 'nsm_customer_types': extended_kpis.get('nsm_customer_types', {}), 'dpd_buckets': extended_kpis.get('dpd_buckets', {}), 'concentration': extended_kpis.get('concentration', {}), 'portfolio_rotation': extended_kpis.get('portfolio_rotation', {}), 'monthly_pricing': extended_kpis.get('monthly_pricing', {}), 'weighted_apr': float(extended_kpis.get('weighted_apr', 0.0) or 0.0), 'weighted_fee_rate': float(extended_kpis.get('weighted_fee_rate', 0.0) or 0.0), 'churn_90d_metrics': extended_kpis.get('churn_90d_metrics', []), 'unit_economics': extended_kpis.get('unit_economics', []), 'pricing_analytics': extended_kpis.get('pricing_analytics', {}), 'revenue_forecast_6m': extended_kpis.get('revenue_forecast_6m', []), 'opportunity_prioritization': extended_kpis.get('opportunity_prioritization', []), 'data_governance': extended_kpis.get('data_governance', {}), 'graph_analytics': extended_kpis.get('graph_analytics', {}), 'portfolio_analytics': extended_kpis.get('portfolio_analytics', {}), 'lending_kpis': extended_kpis.get('lending_kpis', {}), 'risk_kpis': risk_kpis, 'portfolio_health': portfolio_health}

    @staticmethod
    def _empty_portfolio_health_score() -> PortfolioHealthScore:
        return PortfolioHealthScore(score=DATA_MISSING_SCORE, traffic_light='critical', components=[], formula=_PORTFOLIO_HEALTH_FORMULA, interpretation=DATA_MISSING_INTERPRETATION)

    def _build_risk_kpi_snapshot_from_metrics(self, metrics: dict[str, float], *, sample_size: int, timestamp: datetime) -> list[KpiSingleResponse]:
        snapshot_defs = [('PAR30', 'Portfolio at Risk (30+ days)', metrics['par30'], '%'), ('PAR60', 'Portfolio at Risk (60+ days)', metrics['par60'], '%'), ('PAR90', 'Portfolio at Risk (90+ days)', metrics['par90'], '%'), ('DEFAULT_RATE', 'Default Rate', metrics['default_rate'], '%'), ('NPL', 'Non-Performing Loans', metrics['npl'], '%'), ('LGD', 'Loss Given Default', metrics['lgd'], '%'), ('COR', 'Cost of Risk', metrics['cor'], '%'), ('COLLECTION_RATE', 'Collection Rate', metrics['collection_rate'], '%')]
        return [self._build_kpi_single_response(kpi_id=kpi_id, name=name, value=value, unit=unit, timestamp=timestamp, sample_size=sample_size, period='on-demand', filters={'loan_count': sample_size}) for kpi_id, name, value, unit in snapshot_defs]

    def _build_portfolio_health_score_from_metrics(self, metrics: dict[str, float]) -> PortfolioHealthScore:
        ...
        payload = calculate_portfolio_health_score(par30=float(metrics['par30']), collection_rate=float(metrics['collection_rate']), npl=float(metrics['npl']), cost_of_risk=float(metrics['cor']), default_rate=float(metrics['default_rate']))
        ...
        return PortfolioHealthScore(score=float(payload.get('score', 0.0)), traffic_light=cast(Literal['healthy', 'at_risk', 'warning', 'critical'], traffic_light_value), components=components, formula=str(payload.get('formula', '')), interpretation=str(payload.get('interpretation', '')))

    async def get_portfolio_health_score(self, loans: list[LoanRecord] | None) -> PortfolioHealthScore:
        if not loans:
    ...
    async def get_decision_dashboard(self, loans: list[LoanRecord] | None) -> DecisionDashboardResponse:
        if not loans:
        ...
        if health.traffic_light in {'critical', 'warning'}:
            recommendations.append('Prioritize remediation on the weakest health-score components this week.')
        if (red_flags := [flag.flag for flag in risk_stratification.decision_flags if flag.status == 'red']):
            recommendations.append(f"Escalate red risk flags to committee: {', '.join(red_flags)}.")
        if not recommendations:
            recommendations.append('Maintain current strategy and monitor trend deltas in PAR and collections.')
        return DecisionDashboardResponse(generated_at=datetime.now(), portfolio_health=health, risk_stratification=risk_stratification, risk_heatmap=risk_heatmap, unit_economics=unit_economics, kpis=kpis, recommendations=recommendations)

    def _normalize_loans_for_catalog(self, loans_df: pd.DataFrame, payments_df: pd.DataFrame, customers_df: pd.DataFrame) -> pd.DataFrame:
        if loans_df.empty:
            return loans_df
    ...
    def _apply_balance_aliases(self, df: pd.DataFrame) -> pd.DataFrame:
        if 'principal_balance' in df.columns and 'outstanding_balance' not in df.columns:
            df['outstanding_balance'] = pd.to_numeric(df['principal_balance'], errors='coerce').fillna(0)
    ...
    def _apply_customer_mapping(self, df: pd.DataFrame, payments_df: pd.DataFrame, customers_df: pd.DataFrame) -> pd.DataFrame:
    ...
    async def get_data_quality_profile(self, loans: list[LoanRecord] | None) -> DataQualityResponse:
        try:
            if loans is None:
                return DataQualityResponse(duplicate_ratio=0.0, average_null_ratio=0.0, invalid_numeric_ratio=0.0, data_quality_score=0.0, issues=['No data provided for real-time profiling. Quality score unavailable.'])
            df = await run_in_threadpool(self._convert_loan_records_to_dataframe, loans)
            if df.empty:
                return DataQualityResponse(duplicate_ratio=0.0, average_null_ratio=0.0, invalid_numeric_ratio=0.0, data_quality_score=0.0, issues=['Input dataset is empty. Quality score unavailable.'])
            metrics = self._calculate_data_quality_metrics(df)
            return DataQualityResponse(**metrics)
        except Exception as e:
            logger.error('Error calculating data quality profile for actor %s: %s', self.actor, e, exc_info=True)
            raise
    ...
    def _calculate_data_quality_metrics(self, df: pd.DataFrame) -> dict:
        ...
        return {'duplicate_ratio': round(duplicate_ratio, 2), 'average_null_ratio': round(average_null_ratio, 2), 'invalid_numeric_ratio': round(invalid_numeric_ratio, 2), 'data_quality_score': round(data_quality_score, 2), 'issues': issues}
    ...
    def _field_type_error(field_name: str, field_info: Any, series: pd.Series) -> str | None:
        validators = {float: KPIService._float_field_error, str: KPIService._str_field_error, datetime: KPIService._datetime_field_error}
    ...
    async def validate_loan_portfolio_schema(self, loans: list[LoanRecord] | None) -> ValidationResponse:
        try:
            if loans is None:
            ...
            logger.error('Error validating loan portfolio schema for actor %s: %s', self.actor, e, exc_info=True)
            raise

    async def calculate_kpis_for_portfolio(self, loans: list[LoanRecord] | None) -> list[KpiSingleResponse]:
        try:
            if loans is None:
                return []
            df = await run_in_threadpool(self._convert_loan_records_to_dataframe, loans)
            ...
            def build_kpi_response(kpi_id: str, name: str, value: float, unit: str) -> KpiSingleResponse:
                return self._build_kpi_single_response(kpi_id=kpi_id, name=name, value=value, unit=unit, timestamp=now, sample_size=len(loans), period='on-demand', filters={'loan_count': len(loans)})
            return [build_kpi_response('PAR30', 'Portfolio at Risk (30+ days)', results['par30'], '%'), build_kpi_response('PAR60', 'Portfolio at Risk (60+ days)', results['par60'], '%'), build_kpi_response('PAR90', 'Portfolio at Risk (90+ days)', results['par90'], '%'), build_kpi_response('DPD_1_30', 'DPD Bucket (1-30 days)', results['dpd_1_30'], '%'), build_kpi_response('DPD_31_60', 'DPD Bucket (31-60 days)', results['dpd_31_60'], '%'), build_kpi_response('DPD_61_90', 'DPD Bucket (61-90 days)', results['dpd_61_90'], '%'), build_kpi_response('DPD_90_PLUS', 'DPD Bucket (90+ days)', results['dpd_90_plus'], '%'), build_kpi_response('COLLECTION_RATE', 'Collection Rate', results['collection_rate'], '%'), build_kpi_response('LOSS_RATE', 'Loss Rate', results['loss_rate'], '%'), build_kpi_response('RECOVERY_RATE', 'Recovery Rate', results['recovery_rate'], '%'), build_kpi_response('NPL', 'Non-Performing Loans', npl_data['npl_ratio'], '%'), build_kpi_response('LGD', 'Loss Given Default', lgd_data['lgd_pct'], '%'), build_kpi_response('COR', 'Cost of Risk', cor_data['cost_of_risk_pct'], '%'), build_kpi_response('CURERATE', 'Cure Rate', cure_rate, '%'), build_kpi_response('NIM', 'Net Interest Margin', nim_data['nim_pct'], '%'), build_kpi_response('collections_eligible_rate', 'Collections Eligible Rate', results['collections_eligible_rate'], '%'), build_kpi_response('government_sector_exposure_rate', 'Government Sector Exposure Rate', results['government_sector_exposure_rate'], '%'), build_kpi_response('avg_credit_line_utilization', 'Average Credit Line Utilization', results['avg_credit_line_utilization'], '%'), build_kpi_response('capital_collection_rate', 'Capital Collection Rate', results['capital_collection_rate'], '%'), build_kpi_response('mdsc_posted_rate', 'MDSC Posted Rate', results['mdsc_posted_rate'], '%'), build_kpi_response('CASH_ON_HAND', 'Cash on Hand', results['cash_on_hand'], 'USD'), build_kpi_response('PORTFOLIO_YIELD', 'Weighted Average Interest Rate', results['yield'] * 100, '%'), build_kpi_response('AUM', 'Assets Under Management', results['aum'], 'USD'), build_kpi_response('AVERAGE_LOAN_SIZE', 'Average Loan Size', results['average_loan_size'], 'USD'), build_kpi_response('DISBURSEMENT_VOLUME_MTD', 'Disbursement Volume MTD', results['disbursement_volume_mtd'], 'USD'), build_kpi_response('NEW_LOANS_COUNT_MTD', 'New Loans Count MTD', results['new_loans_count_mtd'], 'count'), build_kpi_response('CUSTOMER_LIFETIME_VALUE', 'Customer Lifetime Value', results['customer_lifetime_value'], 'USD'), build_kpi_response('CAC', 'Customer Acquisition Cost', results['cac'], 'USD'), build_kpi_response('GROSS_MARGIN_PCT', 'Gross Margin %', results['gross_margin_pct'], '%'), build_kpi_response('REVENUE_FORECAST_6M', 'Revenue Forecast (6M)', results['revenue_forecast_6m'], 'USD'), build_kpi_response('CHURN_90D', '90-Day Churn Rate', results['churn_90d'], '%'), build_kpi_response('AVG_LTV', 'Average Loan-to-Value', results['avg_ltv'], '%'), build_kpi_response('AVG_DTI', 'Average Debt-to-Income', results['avg_dti'], '%'), build_kpi_response('DEFAULT_RATE', 'Default Rate', results['default_rate'], '%'), build_kpi_response('ACTIVE_BORROWERS', 'Active Borrowers', results['active_borrowers'], 'count'), build_kpi_response('REPEAT_BORROWER_RATE', 'Repeat Borrower Rate', results['repeat_borrower_rate'], '%'), build_kpi_response('AUTOMATION_RATE', 'Automation Rate', results['automation_rate'], '%'), build_kpi_response('PROCESSING_TIME_AVG', 'Average Processing Time', results['processing_time_avg'], 'months'), build_kpi_response('TOTAL_LOANS_COUNT', 'Total Loans Count', results['count'], 'count')]
        except Exception as e:
            logger.error('Error calculating real-time KPIs for actor %s: %s', self.actor, e, exc_info=True)
            raise
    ...
    async def calculate_advanced_risk(self, loans: list[LoanRecord] | None) -> AdvancedRiskResponse:
        try:
            ...
            logger.error('Error calculating advanced risk metrics for actor %s: %s', self.actor, e, exc_info=True)
            raise

    async def calculate_unit_economics(self, loans: list[LoanRecord] | None, funding_cost_rate: float=0.08, cac: float=0.0, monthly_arpu: float=0.0) -> UnitEconomicsResponse:
        try:
            if loans is None:
                loans = []
            return await run_in_threadpool(self._calculate_unit_economics_sync, loans, funding_cost_rate, cac, monthly_arpu)
        except Exception as e:
            logger.error('Error calculating unit economics for actor %s: %s', self.actor, e, exc_info=True)
            raise

    def _calculate_unit_economics_sync(self, loans: list[LoanRecord], funding_cost_rate: float, cac: float, monthly_arpu: float) -> UnitEconomicsResponse:
        ...
        def _normalize_dpd_bucket_dict(bucket: Dict[str, Any]) -> Dict[str, Any]:
            ...
            return normalized
        dpd_migration = [DPDBucketWithAction(**_normalize_dpd_bucket_dict(bucket)) for bucket in raw['dpd_migration']]
        logger.info('Unit economics calculated for actor=%s, loans=%d, npl=%.4f%%', self.actor, len(loans), raw['npl'].get('npl_ratio', 0.0))
        return UnitEconomicsResponse(generated_at=datetime.now(), npl=NplMetrics(**raw['npl']), lgd=LgdMetrics(**raw['lgd']), cost_of_risk=CostOfRiskMetrics(**raw['cost_of_risk']), nim=NimMetrics(**raw['nim']), payback=PaybackMetrics(**raw['payback']), cure_rate=CureRateMetrics(**raw['cure_rate']), dpd_migration=dpd_migration)

    async def calculate_cohort_analytics(self, loans: list[LoanRecord] | None) -> CohortAnalyticsResponse:
        try:
            if loans is None:
                loans = []
            return await run_in_threadpool(self._calculate_cohort_analytics_sync, loans)
        except Exception as e:
            logger.error('Error calculating cohort analytics for actor %s: %s', self.actor, e, exc_info=True)
            raise

    def _calculate_cohort_analytics_sync(self, loans: list[LoanRecord]) -> CohortAnalyticsResponse:
        df = self._convert_loan_records_to_dataframe(loans)
        if df.empty:
            return CohortAnalyticsResponse(generated_at=datetime.now(), cohorts=[], summary=CohortAnalyticsSummary(cohort_count=0, total_loans=0, weighted_par30_pct=0.0, highest_risk_cohort=None, strongest_collection_cohort=None))
        df = df.copy()
        df['loan_amount'] = pd.to_numeric(df['loan_amount'], errors='coerce').fillna(0.0)
        df['principal_balance'] = pd.to_numeric(df['principal_balance'], errors='coerce').fillna(0.0)
        origination_raw = pd.to_datetime(df['origination_date'], errors='coerce', utc=True).dt.tz_convert(None) if 'origination_date' in df.columns else pd.Series([pd.Timestamp.now()] * len(df), index=df.index)
        ...
        collected = pd.to_numeric(df['last_payment_amount'], errors='coerce').fillna(0.0) if 'last_payment_amount' in df.columns else pd.Series([0.0] * len(df), index=df.index)
        scheduled = pd.to_numeric(df['total_scheduled'], errors='coerce').fillna(0.0) if 'total_scheduled' in df.columns else pd.Series([0.0] * len(df), index=df.index)
        ...
        for cohort_month, group_idx in df.groupby('cohort_month').groups.items():
            ...
            group_loans = len(idx)
            par30 = self._safe_pct(df.loc[idx, 'principal_balance'][dpd.loc[idx] > 30].sum(), group_balance)
            par90 = self._safe_pct(df.loc[idx, 'principal_balance'][dpd.loc[idx] > 90].sum(), group_balance)
            default_rate = self._safe_pct(float(default_mask.loc[idx].sum()), group_loans)
            collection_rate = self._safe_pct(float(collected.loc[idx].sum()), float(scheduled.loc[idx].sum()))
            cohort_rows.append(CohortMetrics(cohort_month=str(cohort_month), loan_count=group_loans, originated_amount_usd=round(group_originated, 2), outstanding_amount_usd=round(group_balance, 2), par30_pct=round(par30, 2), par90_pct=round(par90, 2), default_rate_pct=round(default_rate, 2), collection_rate_pct=round(collection_rate, 2)))
        cohort_rows.sort(key=lambda row: row.cohort_month)
        total_outstanding = sum((row.outstanding_amount_usd for row in cohort_rows))
        weighted_par30 = sum((row.par30_pct * row.outstanding_amount_usd for row in cohort_rows)) / total_outstanding if total_outstanding > 0 else 0.0
        highest_risk = max(cohort_rows, key=lambda row: row.par90_pct).cohort_month if cohort_rows else None
        strongest_collection = max(cohort_rows, key=lambda row: row.collection_rate_pct).cohort_month if cohort_rows else None
        summary = CohortAnalyticsSummary(cohort_count=len(cohort_rows), total_loans=len(df), weighted_par30_pct=round(weighted_par30, 2), highest_risk_cohort=highest_risk, strongest_collection_cohort=strongest_collection)
        return CohortAnalyticsResponse(generated_at=datetime.now(), cohorts=cohort_rows, summary=summary)
    ...
    def _build_vintage_curve_point(group: pd.DataFrame, mob_val: int) -> VintageCurvePoint:
        ...
        return VintageCurvePoint(months_on_book=mob_val, cumulative_default_rate=round(cum_default, 2), npl_ratio=round(npl_ratio, 2), loan_count=loan_count)

    def _calculate_vintage_curves_sync(self, loans: list[LoanRecord]) -> VintageCurveResponse:
        df = self._convert_loan_records_to_dataframe(loans)
        if df.empty:
            return VintageCurveResponse(generated_at=datetime.now(), curves={}, portfolio_average_curve=[])
        df = df.copy()
        ...
        if origination.isna().any():
            ...
            logger.warning('Vintage curves: dropping %d loans missing origination_date', missing_count)
        ...
        df['mob'] = ((now.year - df['origination'].dt.year) * 12 + (now.month - df['origination'].dt.month)).clip(lower=0)
        dpd = self._resolve_vintage_dpd(df)
        status_source = df['loan_status'] if 'loan_status' in df.columns else pd.Series([''] * len(df), index=df.index)
        ...
        curves: Dict[str, List[VintageCurvePoint]] = {str(cohort): [self._build_vintage_curve_point(group, self._safe_int(group['mob'].iloc[0]))] for cohort, group in df.groupby('cohort')}
        avg_curve: List[VintageCurvePoint] = [self._build_vintage_curve_point(group, self._safe_int(mob)) for mob, group in df.groupby('mob')]
        avg_curve.sort(key=lambda x: x.months_on_book)
        return VintageCurveResponse(generated_at=datetime.now(), curves=curves, portfolio_average_curve=avg_curve)

    async def calculate_stress_test(self, loans: list[LoanRecord] | None, par_deterioration_pct: float=25.0, collection_efficiency_pct: float=-10.0, recovery_efficiency_pct: float=-15.0, funding_cost_bps: float=150.0) -> StressTestResponse:
        try:
            if loans is None:
                loans = []
            return await run_in_threadpool(self._calculate_stress_test_sync, loans, par_deterioration_pct, collection_efficiency_pct, recovery_efficiency_pct, funding_cost_bps)
        except Exception as e:
            logger.error('Error calculating stress test for actor %s: %s', self.actor, e, exc_info=True)
            raise

    async def calculate_segment_analytics(self, loans: list[LoanRecord] | None, dimension: str='risk_band', top_n: int=20) -> SegmentAnalyticsResponse:
        try:
            return await run_in_threadpool(self._calculate_segment_analytics_sync, loans or [], dimension, top_n)
        except Exception as e:
            logger.error('Error calculating segment analytics for actor %s: %s', self.actor, e, exc_info=True)
            raise

    def _calculate_segment_analytics_sync(self, loans: list[LoanRecord], dimension: str, top_n: int) -> SegmentAnalyticsResponse:
        df = self._convert_loan_records_to_dataframe(loans)
        if df.empty:
            return SegmentAnalyticsResponse(generated_at=datetime.now(), segments=[], summary=SegmentAnalyticsSummary(dimension=dimension, segment_count=0, total_loans=0, largest_segment=None, riskiest_segment=None))
        df = df.copy()
        df['principal_balance'] = pd.to_numeric(df['principal_balance'], errors='coerce').fillna(0.0)
        ...
        if 'days_past_due' in df.columns:
            ...
            dpd = dpd.mask(status.str.contains(STATUS_PATTERN_60_89, regex=True, na=False), 75.0)
            dpd = dpd.mask(status.str.contains(STATUS_PATTERN_30_59, regex=True, na=False), 45.0)
        default_mask = df.get('loan_status', pd.Series([''] * len(df))).astype(str).str.contains(STATUS_PATTERN_DEFAULT, regex=True, case=False, na=False)
        collected = pd.to_numeric(df['last_payment_amount'], errors='coerce').fillna(0.0) if 'last_payment_amount' in df.columns else pd.Series([0.0] * len(df), index=df.index)
        scheduled = pd.to_numeric(df['total_scheduled'], errors='coerce').fillna(0.0) if 'total_scheduled' in df.columns else pd.Series([0.0] * len(df), index=df.index)
        ...
        for segment_value, group_idx in df.groupby('segment_value').groups.items():
            ...
            loan_count = len(idx)
            par30 = self._safe_pct(df.loc[idx, 'principal_balance'][dpd.loc[idx] >= 30].sum(), outstanding)
            par60 = self._safe_pct(df.loc[idx, 'principal_balance'][dpd.loc[idx] >= 60].sum(), outstanding)
            par90 = self._safe_pct(df.loc[idx, 'principal_balance'][dpd.loc[idx] >= 90].sum(), outstanding)
            default_rate = self._safe_pct(float(default_mask.loc[idx].sum()), loan_count)
            collection_rate = self._safe_pct(float(collected.loc[idx].sum()), float(scheduled.loc[idx].sum()))
            avg_rate = float(df.loc[idx, 'interest_rate'].mean()) * 100.0
            segment_rows.append(SegmentMetrics(segment=str(segment_value), loan_count=loan_count, outstanding_usd=round(outstanding, 2), par30_pct=round(par30, 2), par60_pct=round(par60, 2), par90_pct=round(par90, 2), default_rate_pct=round(default_rate, 2), avg_interest_rate_pct=round(avg_rate, 2), collection_rate_pct=round(collection_rate, 2)))
        ...
        riskiest_segment = max(segment_rows, key=lambda row: row.par90_pct).segment if segment_rows else None
        summary = SegmentAnalyticsSummary(dimension=dimension, segment_count=len(segment_rows), total_loans=len(df), largest_segment=largest_segment, riskiest_segment=riskiest_segment)
        return SegmentAnalyticsResponse(generated_at=datetime.now(), segments=segment_rows, summary=summary)

    async def calculate_roll_rate_analytics(self, loans: list[LoanRecord] | None) -> RollRateAnalyticsResponse:
        try:
            if loans is None:
                loans = []
            return await run_in_threadpool(self._calculate_roll_rate_analytics_sync, loans)
        except Exception as e:
            logger.error('Error calculating roll-rate analytics for actor %s: %s', self.actor, e, exc_info=True)
            raise

    def _calculate_roll_rate_analytics_sync(self, loans: list[LoanRecord]) -> RollRateAnalyticsResponse:
        df = self._convert_loan_records_to_dataframe(loans)
        if df.empty:
        ...
        summary = self._build_roll_rate_summary(prepared_df, matrix_df, bucket_summaries, explicit_previous_signal)
        return RollRateAnalyticsResponse(generated_at=datetime.now(), transition_matrix=transition_matrix, bucket_summaries=bucket_summaries, summary=summary)

    def _empty_roll_rate_analytics_response(self) -> RollRateAnalyticsResponse:
        return RollRateAnalyticsResponse(generated_at=datetime.now(), transition_matrix=[], bucket_summaries=[], summary=RollRateAnalyticsSummary(total_loans=0, historical_coverage_pct=0.0, portfolio_cure_rate_pct=0.0, portfolio_roll_forward_rate_pct=0.0, worst_migration_path=None, best_cure_source=None))

    def _build_roll_rate_base_frames(self, df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.Series]:
        work = df.copy()
        current_dpd = self._derive_dpd_series(df=work, dpd_column='days_past_due', status_column='loan_status')
        previous_dpd_raw = self._derive_dpd_series(df=work, dpd_column='previous_days_past_due', status_column='previous_loan_status')
        ...
        if 'previous_principal_balance' in work.columns:
            from_balance = pd.to_numeric(work['previous_principal_balance'], errors='coerce').fillna(to_balance).astype(float)
        else:
        ...
        matrix_df = work.groupby(['from_bucket', 'to_bucket'], dropna=False).agg(loan_count=('from_bucket', 'size'), exposure_usd=('from_exposure_usd', 'sum')).reset_index()
    ...
    def _resolve_explicit_previous_signal(df: pd.DataFrame) -> pd.Series:
        ...
        if 'previous_loan_status' in df.columns:
            previous_status_present = df['previous_loan_status'].fillna('').astype(str).str.strip() != ''
            explicit_previous_signal = explicit_previous_signal | previous_status_present
        return explicit_previous_signal

    def _build_roll_rate_transition_matrix(self, matrix_df: pd.DataFrame) -> list[RollRateTransition]:
        ...
        return [RollRateTransition(**row) for row in matrix_df[['from_bucket', 'to_bucket', 'loan_count', 'exposure_usd', 'loan_share_pct', 'exposure_share_pct']].to_dict('records')]

    def _build_roll_rate_bucket_summaries(self, df: pd.DataFrame) -> list[RollRateBucketSummary]:
        if 'loan_count' not in df.columns:
            df = df.copy()
            df['loan_count'] = 1
        bucket_stats = df.groupby('from_bucket').agg(loan_count=('loan_count', 'sum'), exposure_usd=('from_exposure_usd', 'sum'))
        cure_counts = df[(df['to_bucket'] == 'current') & (df['from_bucket'] != 'current')].groupby('from_bucket')['loan_count'].sum()
        roll_forward_counts = df[df['to_severity'] > df['from_severity']].groupby('from_bucket')['loan_count'].sum()
        stable_counts = df[df['to_bucket'] == df['from_bucket']].groupby('from_bucket')['loan_count'].sum()
        ...
        for bucket in self._bucket_order():
            ...
            summaries.append(RollRateBucketSummary(from_bucket=bucket, loan_count=count, exposure_usd=round(exposure, 2), cure_rate_pct=round(self._safe_pct(cure_c, float(count)), 2), roll_forward_rate_pct=round(self._safe_pct(roll_f_c, float(count)), 2), stability_rate_pct=round(self._safe_pct(stable_c, float(count)), 2)))
        return summaries

    def _build_roll_rate_summary(self, df: pd.DataFrame, matrix_df: pd.DataFrame, bucket_summaries: list[RollRateBucketSummary], explicit_previous_signal: pd.Series) -> RollRateAnalyticsSummary:
        ...
        roll_forward_portfolio = int((roll_eligible_mask & (df['to_severity'] > df['from_severity'])).sum())
        roll_eligible_total = int(roll_eligible_mask.sum())
        migration_rows = matrix_df[matrix_df['to_order'] > matrix_df['from_order']].sort_values(['loan_count', 'exposure_usd'], ascending=False)
        worst_migration_path = f"{migration_rows.iloc[0]['from_bucket']}->{migration_rows.iloc[0]['to_bucket']}" if not migration_rows.empty and int(migration_rows.iloc[0]['loan_count']) > 0 else None
        cure_candidates = [row for row in bucket_summaries if row.from_bucket != 'current' and row.loan_count > 0]
        if cure_candidates and (best_cure_row := max(cure_candidates, key=lambda row: (row.cure_rate_pct, row.loan_count))):
            best_cure_source = best_cure_row.from_bucket if best_cure_row.cure_rate_pct > 0 else None
        else:
            best_cure_source = None
        return RollRateAnalyticsSummary(total_loans=len(df), historical_coverage_pct=round(self._safe_pct(explicit_previous_signal.sum(), len(df)), 2), portfolio_cure_rate_pct=round(self._safe_pct(cured_portfolio, delinquent_total), 2), portfolio_roll_forward_rate_pct=round(self._safe_pct(roll_forward_portfolio, roll_eligible_total), 2), worst_migration_path=worst_migration_path, best_cure_source=best_cure_source)

    def _calculate_stress_test_sync(self, loans: list[LoanRecord], par_deterioration_pct: float, collection_efficiency_pct: float, recovery_efficiency_pct: float, funding_cost_bps: float) -> StressTestResponse:
        df = self._convert_loan_records_to_dataframe(loans)
        if df.empty:
            zero_metrics = StressTestMetrics(par30_pct=0.0, par90_pct=0.0, default_rate_pct=0.0, loss_rate_pct=0.0, collection_rate_pct=0.0, recovery_rate_pct=0.0, gross_margin_pct=0.0, revenue_forecast_6m_usd=0.0, expected_credit_loss_usd=0.0, expected_collections_usd=0.0)
            assumptions = StressTestAssumptions(par_deterioration_pct=par_deterioration_pct, collection_efficiency_pct=collection_efficiency_pct, recovery_efficiency_pct=recovery_efficiency_pct, funding_cost_bps=funding_cost_bps)
            return StressTestResponse(scenario_id=str(uuid.uuid4()), generated_at=datetime.now(), assumptions=assumptions, baseline=zero_metrics, stressed=zero_metrics, deltas=zero_metrics, alerts=[])
        ...
        total_scheduled = float(pd.to_numeric(df['total_scheduled'], errors='coerce').fillna(0).sum()) if 'total_scheduled' in df.columns else 0.0
        baseline = StressTestMetrics(par30_pct=round(float(baseline_raw['par30']), 2), par90_pct=round(float(baseline_raw['par90']), 2), default_rate_pct=round(float(baseline_raw['default_rate']), 2), loss_rate_pct=round(float(baseline_raw['loss_rate']), 2), collection_rate_pct=round(float(baseline_raw['collection_rate']), 2), recovery_rate_pct=round(float(baseline_raw['recovery_rate']), 2), gross_margin_pct=round(float(baseline_raw['gross_margin_pct']), 2), revenue_forecast_6m_usd=round(float(baseline_raw['revenue_forecast_6m']), 2), expected_credit_loss_usd=round(total_originated * float(baseline_raw['loss_rate']) / 100.0, 2), expected_collections_usd=round(total_scheduled * float(baseline_raw['collection_rate']) / 100.0, 2))
        ...
        stressed_par90 = self._clamp_pct(baseline.par90_pct * (1.0 + par_deterioration_pct * 1.2 / 100.0))
        stressed_default = self._clamp_pct(baseline.default_rate_pct * (1.0 + par_deterioration_pct * 0.8 / 100.0))
        stressed_loss = self._clamp_pct(baseline.loss_rate_pct * (1.0 + par_deterioration_pct * 1.1 / 100.0))
        ...
        stressed_margin = round(baseline.gross_margin_pct - funding_cost_pct - loss_drag - recovery_drag - par_drag - collection_drag, 2)
        forecast_drag = max(0.0, stressed_default - baseline.default_rate_pct) * 0.5 + max(0.0, baseline.collection_rate_pct - stressed_collection) * 0.3
        stressed_forecast = max(0.0, baseline.revenue_forecast_6m_usd * (1.0 - forecast_drag / 100.0))
        stressed = StressTestMetrics(par30_pct=round(stressed_par30, 2), par90_pct=round(stressed_par90, 2), default_rate_pct=round(stressed_default, 2), loss_rate_pct=round(stressed_loss, 2), collection_rate_pct=round(stressed_collection, 2), recovery_rate_pct=round(stressed_recovery, 2), gross_margin_pct=round(stressed_margin, 2), revenue_forecast_6m_usd=round(stressed_forecast, 2), expected_credit_loss_usd=round(total_originated * stressed_loss / 100.0, 2), expected_collections_usd=round(total_scheduled * stressed_collection / 100.0, 2))
        deltas = StressTestMetrics(par30_pct=round(stressed.par30_pct - baseline.par30_pct, 2), par90_pct=round(stressed.par90_pct - baseline.par90_pct, 2), default_rate_pct=round(stressed.default_rate_pct - baseline.default_rate_pct, 2), loss_rate_pct=round(stressed.loss_rate_pct - baseline.loss_rate_pct, 2), collection_rate_pct=round(stressed.collection_rate_pct - baseline.collection_rate_pct, 2), recovery_rate_pct=round(stressed.recovery_rate_pct - baseline.recovery_rate_pct, 2), gross_margin_pct=round(stressed.gross_margin_pct - baseline.gross_margin_pct, 2), revenue_forecast_6m_usd=round(stressed.revenue_forecast_6m_usd - baseline.revenue_forecast_6m_usd, 2), expected_credit_loss_usd=round(stressed.expected_credit_loss_usd - baseline.expected_credit_loss_usd, 2), expected_collections_usd=round(stressed.expected_collections_usd - baseline.expected_collections_usd, 2))
        alerts: list[str] = []
        ...
        if baseline.expected_credit_loss_usd > 0 and stressed.expected_credit_loss_usd >= baseline.expected_credit_loss_usd * 1.25:
            alerts.append('Capital stress: expected credit loss increases by at least 25%.')
        assumptions = StressTestAssumptions(par_deterioration_pct=round(float(par_deterioration_pct), 2), collection_efficiency_pct=round(float(collection_efficiency_pct), 2), recovery_efficiency_pct=round(float(recovery_efficiency_pct), 2), funding_cost_bps=round(float(funding_cost_bps), 2))
        return StressTestResponse(scenario_id=str(uuid.uuid4()), generated_at=datetime.now(), assumptions=assumptions, baseline=baseline, stressed=stressed, deltas=deltas, alerts=alerts)

    async def get_risk_stratification(self, loans: list[LoanRecord] | None) -> RiskStratificationResponse:
        if not loans:
            return RiskStratificationResponse(buckets=[], decision_flags=[], summary='No loans provided for stratification.')
        ...
        return RiskStratificationResponse(buckets=advanced_risk.dpd_buckets, decision_flags=decision_flags, summary=summary)
    ...
    async def get_layered_insights(self, loans: list[LoanRecord] | None) -> list[AnalysisLayer]:
        ...
        critical_buckets = [bucket['label'] for bucket in heatmap_summary['heatmap'] if bucket['risk_intensity'] == 'high']
        cure_rate = (await self.calculate_roll_rate_analytics(loans)).summary.portfolio_cure_rate_pct
        return [self._build_portfolio_risk_layer(metrics, critical_buckets), self._build_growth_profitability_layer(metrics), self._build_operational_efficiency_layer(metrics, cure_rate)]
    ...
    async def get_risk_heatmap_summary(self, loans: list[LoanRecord] | None) -> dict[str, Any]:
        ...
        return {'status': 'success', 'heatmap': heatmap, 'critical_buckets': critical_buckets, 'narrative': narrative, 'overall_par30': round(metrics['par30'], 2)}

    @staticmethod
    def _build_portfolio_risk_layer(metrics: dict[str, float], critical_buckets: list[str]) -> AnalysisLayer:
        ...
        return AnalysisLayer(layer='Portfolio Risk', what=f"Portfolio risk is currently {risk_status} with PAR30 at {metrics['par30']:.1f}%.", why=f'Driver: {risk_driver}.', so_what=f"Impact: Potential {metrics['cor']:.1f}% hit to margin from expected credit losses.", now_what='Action: Trigger intensive collections for high-intensity buckets and review underwriting for toxic cohorts.')
    ...
    def _build_growth_profitability_layer(metrics: dict[str, float]) -> AnalysisLayer:
        ...
        return AnalysisLayer(layer='Growth & Profitability', what=f'Unit economics show a {clv_cac:.1f}x CLV/CAC ratio and {nim:.1f}% NIM.', why='Driver: High repeat borrower rate and stable yield spread.', so_what='Impact: Growth is sustainable and provides positive risk-adjusted returns.', now_what='Action: Maintain acquisition cadence while monitoring for any margin compression.')

    @staticmethod
    def _build_operational_efficiency_layer(metrics: dict[str, float], cure_rate: float) -> AnalysisLayer:
        return AnalysisLayer(layer='Operational Efficiency', what=f"Collections efficiency is at {metrics['collection_rate']:.1f}% with a {cure_rate:.1f}% cure rate.", why='Driver: Effective early-stage intervention and automated PTP tracking.', so_what='Impact: High cash conversion speed reduces funding pressure.', now_what='Action: Optimize SMS/Call timing based on DPD migration patterns to further improve cure rates.')

    @staticmethod
    def _build_heatmap_buckets(metrics: dict[str, float]) -> list[dict[str, Any]]:
        return [{'id': '1_30', 'label': 'Early (1-30 DPD)', 'value': metrics['dpd_1_30'], 'threshold': 8.0}, {'id': '31_60', 'label': 'Warning (31-60 DPD)', 'value': metrics['dpd_31_60'], 'threshold': 4.0}, {'id': '61_90', 'label': 'Severe (61-90 DPD)', 'value': metrics['dpd_61_90'], 'threshold': 2.0}, {'id': '90_plus', 'label': 'NPL (90+ DPD)', 'value': metrics['dpd_90_plus'], 'threshold': 1.0}]
    ...
    def _build_heatmap_rows(self, buckets: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[str]]:
        ...
        for bucket in buckets:
            ...
            heatmap.append({'bucket': bucket['id'], 'label': bucket['label'], 'exposure_pct': round(float(bucket['value']), 2), 'risk_intensity': intensity})
    ...
    def _calculate_portfolio_performance_metrics(self, df: pd.DataFrame) -> dict:
        ...
        default_mask = status_series.str.contains('default|charged.off|90\\+', case=False, na=False) | (df['dpd'] > 90)
        ...
        status_defaulted_mask = status_series.str.contains(STATUS_PATTERN_DEFAULT, case=False, na=False)
        ...
        collection_rate, recovery_rate = self._calculate_collection_recovery_metrics(df, status_defaulted_mask)
        ...
        return {'par30': par_metrics['par30'], 'par60': par_metrics['par60'], 'par90': par_metrics['par90'], 'dpd_1_30': par_metrics['dpd_1_30'], 'dpd_31_60': par_metrics['dpd_31_60'], 'dpd_61_90': par_metrics['dpd_61_90'], 'dpd_90_plus': par_metrics['dpd_90_plus'], 'default_rate': default_rate_pct, 'collection_rate': collection_rate, 'loss_rate': loss_rate, 'recovery_rate': recovery_rate, 'npl': npl_pct, 'npl_90': npl_90_pct, 'lgd': lgd_pct, 'cor': loss_rate, 'cash_on_hand': self._calculate_cash_on_hand(df), 'yield': avg_interest_rate, 'aum': total_outstanding, 'average_loan_size': float(df['loan_amount'].mean()) if len(df) > 0 else 0.0, 'disbursement_volume_mtd': disbursement_volume_mtd, 'new_loans_count_mtd': new_loans_count_mtd, 'customer_lifetime_value': self._calculate_customer_lifetime_value(df), 'cac': executive_metrics['cac'], 'gross_margin_pct': gross_margin_pct, 'revenue_forecast_6m': executive_metrics['revenue_forecast_6m'], 'churn_90d': executive_metrics['churn_90d'], 'avg_ltv': df['ltv_ratio'].mean(), 'avg_dti': df['dti_ratio'].mean(), 'active_borrowers': active_borrowers, 'repeat_borrower_rate': repeat_borrower_rate, 'automation_rate': self._calculate_automation_rate(df), 'processing_time_avg': self._calculate_processing_time_avg(df), 'count': self._calculate_total_loans_count(df, status_series), 'collections_eligible_rate': enriched_metrics['collections_eligible_rate'], 'government_sector_exposure_rate': enriched_metrics['government_sector_exposure_rate'], 'avg_credit_line_utilization': enriched_metrics['avg_credit_line_utilization'], 'capital_collection_rate': enriched_metrics['capital_collection_rate'], 'mdsc_posted_rate': enriched_metrics['mdsc_posted_rate']}
    ...
    def _calculate_realtime_enriched_metrics(self, df: pd.DataFrame, total_outstanding: float) -> dict[str, float]:
        metrics = {'collections_eligible_rate': 0.0, 'government_sector_exposure_rate': 0.0, 'avg_credit_line_utilization': 0.0, 'capital_collection_rate': 0.0, 'mdsc_posted_rate': 0.0}
        if df.empty:
            return metrics
        balance_col = self._first_present_column(df, 'principal_balance', 'outstanding_balance', 'current_balance', 'loan_amount')
        ...
        if eligible_col is not None:
            eligible_mask = df[eligible_col].astype(str).str.strip().str.upper().isin({'Y', 'YES', 'SI', 'S', 'TRUE', '1'})
            metrics['collections_eligible_rate'] = self._safe_pct(balance[eligible_mask].sum(), denom)
        ...
        if govt_col is not None:
            ...
            metrics['government_sector_exposure_rate'] = self._safe_pct(float(balance[govt_mask].sum()), denom)
    ...
    def _calculate_cash_on_hand(df: pd.DataFrame) -> float:
        ...
        null_or_zero_ratio = (current_balance_series.isna() | (current_balance_series == 0)).mean() if len(current_balance_series) > 0 else 1.0
    ...
    def _calculate_mtd_metrics(df: pd.DataFrame) -> tuple[float, float]:
        if 'origination_date' not in df.columns:
            return (0.0, 0.0)
        origination_dates = pd.to_datetime(df['origination_date'], errors='coerce', utc=True).dt.tz_convert(None)
    ...
    def _calculate_customer_lifetime_value(df: pd.DataFrame) -> float:
        ...
        unique_borrowers = float(df['borrower_id'].dropna().astype(str).nunique()) if 'borrower_id' in df.columns else 0.0
    ...
    def _calculate_total_loans_count(df: pd.DataFrame, status_series: pd.Series) -> float:
        if 'id' in df.columns and df['id'].notna().any():
            non_closed_mask = ~status_series.str.contains('closed|complete|paid|settled', case=False, na=False)
    ...
    def _calculate_collection_recovery_metrics(self, df: pd.DataFrame, status_defaulted_mask: pd.Series) -> tuple[float, float]:
        ...
        collected = pd.to_numeric(df['last_payment_amount'], errors='coerce').fillna(0) if 'last_payment_amount' in df.columns else pd.Series([0.0] * len(df), index=df.index)
    ...
    def _calculate_borrower_metrics(self, df: pd.DataFrame) -> tuple[float, float]:
        ...
        active_mask = ~df['loan_status'].str.contains('closed|complete|paid|settled', case=False, na=False)
    ...
    def _calculate_automation_rate(df: pd.DataFrame) -> float:
        if 'payment_frequency' not in df.columns:
            return 0.0
        automated_mask = df['payment_frequency'].fillna('').astype(str).str.contains('bullet|auto', case=False, na=False)
    ...
    def _last_realtime_metric(rows: list[dict], key: str) -> float | None:
        value = next((row.get(key) for row in reversed(rows) if row.get(key) is not None and (not pd.isna(row.get(key)))), None)
        return float(value) if value is not None else None

    def _calculate_realtime_executive_metrics(self, df: pd.DataFrame) -> dict[str, float]:
        try:
            normalized = self._normalize_loans_for_catalog(df.copy(), pd.DataFrame(), pd.DataFrame())
            processor = KPICatalogProcessor(loans_df=normalized, payments_df=pd.DataFrame(), customers_df=pd.DataFrame(), schedule_df=pd.DataFrame())
        ...
        cac_value = self._safe_float(executive_strip.get('cac_usd'), default=self._last_realtime_metric(unit_economics, 'cac_usd'))
        margin_ratio = self._safe_float(executive_strip.get('gross_margin_pct'), default=self._last_realtime_metric(unit_economics, 'gross_margin_pct'))
        churn_ratio = self._safe_float(self._last_realtime_metric(churn_rows, 'churn90d_pct'))
        forecast_sum = float(sum((self._safe_float(row.get('forecast_revenue_usd')) for row in forecast_rows if isinstance(row, dict))))
        if forecast_sum <= 0 and isinstance(revenue_df, pd.DataFrame) and (not revenue_df.empty) and ('recv_revenue_for_month' in revenue_df.columns):
            revenue_series = pd.to_numeric(revenue_df['recv_revenue_for_month'], errors='coerce').fillna(0)
            if len(revenue_series) > 0:
                forecast_sum = max(0.0, float(revenue_series.iloc[-1]) * 6.0)
        return {'cac': round(cac_value, 4), 'gross_margin_pct': round(self._ratio_to_percent(margin_ratio), 4), 'revenue_forecast_6m': round(forecast_sum, 4), 'churn_90d': round(self._ratio_to_percent(churn_ratio), 4)}
    ...
    @staticmethod
    def _series_from_aliases(df: pd.DataFrame, aliases: list[str], default: str='unknown') -> pd.Series:
        lower_map = {col.lower(): col for col in df.columns}
        matched = next((lower_map.get(alias.lower()) for alias in aliases), None)
        return df[matched].fillna(default).astype(str) if matched is not None else pd.Series([default] * len(df), index=df.index, dtype=object)
    ...
    def _build_ticket_size_band_series(df: pd.DataFrame) -> pd.Series:
        amount_source = df['loan_amount'] if 'loan_amount' in df.columns else pd.Series([0] * len(df), index=df.index)
    ...
    def _build_month_period_series(df: pd.DataFrame, source_column: str) -> pd.Series:
        source = df[source_column] if source_column in df.columns else pd.Series([None] * len(df), index=df.index)
    ...
    def _build_utilization_band_series(df: pd.DataFrame) -> pd.Series:
        ...
        if (util <= 1.0).all():
            util = util * 100.0
        return pd.cut(util, bins=[-0.001, 25, 50, 75, 100, float('inf')], labels=['0_25', '25_50', '50_75', '75_100', '100_plus']).astype(str)

    @staticmethod
    def _segment_aliases() -> dict[str, list[str]]:
        return {'company': ['company'], 'credit_line': ['credit_line', 'lineacredito', 'linea_credito'], 'client_code': ['client_code', 'codcliente'], 'issuer': ['issuer_name', 'issuer'], 'kam_hunter': ['kam_hunter', 'cod_kam_hunter'], 'kam_farmer': ['kam_farmer', 'cod_kam_farmer'], 'advisory_channel': ['advisory_channel', 'asesoriadigital'], 'gov': ['gov', 'ministry', 'ministerio'], 'industry': ['industry', 'industria', 'giro'], 'doc_type': ['doc_type'], 'government_sector': ['government_sector', 'goes'], 'collections_eligible': ['collections_eligible', 'procede_a_cobrar']}
    ...
    def _build_segment_dimension(df: pd.DataFrame, dpd: pd.Series, dimension: str) -> pd.Series:
        ...
        if normalized == 'payment_frequency':
            return df.get('payment_frequency', pd.Series(['unknown'] * len(df), index=df.index)).fillna('unknown')
        if normalized == 'loan_status':
            return df.get('loan_status', pd.Series(['unknown'] * len(df), index=df.index)).fillna('unknown')
    ...
    def _derive_dpd_series(df: pd.DataFrame, dpd_column: str, status_column: str) -> pd.Series:
        if dpd_column in df.columns and pd.to_numeric(df[dpd_column], errors='coerce').notna().any():
            return pd.to_numeric(df[dpd_column], errors='coerce').fillna(0.0).clip(lower=0.0)
        status = df.get(status_column, pd.Series([''] * len(df), index=df.index)).astype(str).str.lower()
        dpd = pd.Series([0.0] * len(df), index=df.index, dtype=float)
        dpd = dpd.mask(status.str.contains(STATUS_PATTERN_90_PLUS, regex=True, na=False), 100.0)
        dpd = dpd.mask(status.str.contains(STATUS_PATTERN_60_89, regex=True, na=False), 75.0)
        dpd = dpd.mask(status.str.contains(STATUS_PATTERN_30_59, regex=True, na=False), 45.0)
    ...
    def _load_nsm_recurrent_tpv_sync(self) -> NSMRecurrentTPVResponse:
        ...
        with open(nsm_path) as f:
            data = json.load(f)
        by_period = {period: NSMPeriodMetrics(period=period, **metrics) for period, metrics in data.get('by_period', {}).items()}
        latest_period = data.get('latest_period')
        latest_raw = data.get('latest')
        latest = NSMPeriodMetrics(period=latest_period, **latest_raw) if latest_period and latest_raw else None
        return NSMRecurrentTPVResponse(latest_period=latest_period, latest=latest, by_period=by_period)
