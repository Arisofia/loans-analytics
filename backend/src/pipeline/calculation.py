...
def _assign_cluster_cohorts(
    risk_scores: Dict[int, float], centroids: Dict[int, Dict[str, Any]]
) -> Dict[int, str]:
...
class CalculationPhase:
    ...
    def execute(
        self, clean_data_path: Optional[Path] = None, df: Optional[pd.DataFrame] = None
    ) -> Dict[str, Any]:
        if df is None:
            if clean_data_path and Path(clean_data_path).exists():
                ...
                raise ValueError(
                    "CRITICAL: CalculationPhase.execute requires either a DataFrame or a valid clean_data_path."
                )
    ...
    def _calculate_recurrent_tpv(timeseries: pd.DataFrame) -> Dict[str, Any]:
        ...
        for period in periods:
            ...
            tpv_new = round(
                sum((float(tpv_by_client.get(client, 0.0)) for client in new_clients)), 2
            )
            tpv_recurrent = round(
                sum((float(tpv_by_client.get(client, 0.0)) for client in recurrent_clients)), 2
            )
            tpv_recovered = round(
                sum((float(tpv_by_client.get(client, 0.0)) for client in recovered_clients)), 2
            )
    ...
    def _build_anomaly_record(
        kpi_name: str, kpi_value: Any, normal_ranges: Dict[str, tuple[float, float]]
    ...
    def _calculate_segment_group_kpis(
        self, grp: pd.DataFrame, balance_col: str, dpd_col: Optional[str], status_col: Optional[str]
    ) -> Optional[Dict[str, Any]]:
        ...
        if dpd_col:
            seg_kpis.update(
                self._calculate_segment_par_metrics(
                    grp, balance_col, dpd_col, float(total_bal), status_col
                )
            )
        if status_col:
    ...
    def _build_canonical_marts(df: pd.DataFrame) -> Dict[str, pd.DataFrame]:
        ...
        if "outstanding_principal" not in work.columns:
            work["outstanding_principal"] = pd.to_numeric(
                _first_existing_series(
                    "outstanding_balance", "current_balance", "amount", default=0
                ),
                errors="coerce",
            ).fillna(0)
        if "days_past_due" not in work.columns:
            work["days_past_due"] = pd.to_numeric(
                _series_or_default("dpd", 0), errors="coerce"
            ).fillna(0)
        ...
        if "disbursement_amount" not in work.columns:
            work["disbursement_amount"] = pd.to_numeric(
                _series_or_default("amount", 0), errors="coerce"
            ).fillna(0)
        ...
        finance_mart = pd.DataFrame(
            {
                "interest_income": pd.to_numeric(
                    _series_or_default("interest_income", 0), errors="coerce"
                ).fillna(0),
                "fee_income": pd.to_numeric(
                    _series_or_default("fee_income", 0), errors="coerce"
                ).fillna(0),
                "funding_cost": pd.to_numeric(
                    _series_or_default("funding_cost", 0), errors="coerce"
                ).fillna(0),
                "operating_expense": pd.to_numeric(
                    _series_or_default("operating_expense", 0), errors="coerce"
                ).fillna(0),
                "provision_expense": pd.to_numeric(
                    _series_or_default("provision_expense", 0), errors="coerce"
                ).fillna(0),
                "balance_avg": pd.to_numeric(
                    _series_or_default("outstanding_principal", 0), errors="coerce"
                ).fillna(0),
        ...
        sales_mart = pd.DataFrame(
            {
                ...
                "approved_ticket": pd.to_numeric(
                    _first_existing_series("disbursement_amount", "amount", default=0),
                    errors="coerce",
                ).fillna(0),
                "funded_flag": ~work["status"]
                .astype(str)
                .str.lower()
    ...
    def _calculate_expected_loss(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        logger.info(
            "EL calculated: $%.2f (%.4f%% of EAD)",
            result["total_expected_loss_usd"],
            result["expected_loss_rate_pct"],
        )
        return result
    ...
    def _calculate_vintage_analysis(self, df: pd.DataFrame) -> Dict[str, Any]:
                    ...
                    float(
                        compute_default_rate_by_count(
                            pd.DataFrame(
                                {
                                    "status": grp[status_col],
                                    "outstanding_principal": pd.Series(
                                        1, index=grp.index, dtype="int64"
                                    ),
                                }
                            )
                        )
        ...
        result = {
            ...
            "worst_vintage": (
                max(vintage_data.items(), key=lambda x: x[1]["default_rate"])[0]
                if vintage_data
                else None
            ),
            "best_vintage": (
                min(vintage_data.items(), key=lambda x: x[1]["default_rate"])[0]
                if vintage_data
                else None
            ),
        }
    ...
    def _calculate_concentration_hhi(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        top_5_pct = (
            float(shares.iloc[:5].sum() * 100) if len(shares) >= 5 else float(shares.sum() * 100)
        )
        top_10_pct = (
            float(shares.iloc[:10].sum() * 100) if len(shares) >= 10 else float(shares.sum() * 100)
        )
        top_20_pct = (
            float(shares.iloc[:20].sum() * 100) if len(shares) >= 20 else float(shares.sum() * 100)
        )
        if hhi < 1000:
    ...
    def _run_advanced_clustering(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            if X.shape[0] < 10:
                logger.warning(
                    "Clustering skipped: insufficient non-opaque observations (%d < 10)", X.shape[0]
                )
                metrics["skipped"] = True
    ...
    def _build_feature_matrix(self, df: pd.DataFrame) -> tuple[np.ndarray, List[str], pd.Series]:
        ...
        if not feature_cols:
            raise ValueError(
                f"No numeric feature columns available for clustering (looked for: {_COHORT_FEATURE_COLS} + *_is_missing flags)"
            )
    ...
    def _map_cohorts(
        self, df: pd.DataFrame, labels: np.ndarray, opaque_mask: pd.Series
