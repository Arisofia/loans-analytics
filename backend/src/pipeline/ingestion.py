...
class IngestionPhase:
    ...
    def execute(
        ...
        if data_as_of_date:
            logger.info(
                "Detected data as-of date: %s (from column: %s)", data_as_of_date, data_as_of_column
            )
        if run_dir:
    ...
    def _normalize_control_mora_like_file(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not evidence_columns.intersection(slug_to_column):
            return df
        normalized = df.copy()
        loan_id_col = self._first_existing_column(
            slug_to_column, ["loan_id", "numerointerno", "numerodesembolso", "mfanumdoc"]
        )
        borrower_id_col = self._first_existing_column(
            slug_to_column, ["borrower_id", "clinumero", "codcliente", "client_id"]
        )
        ...
        as_of_col = self._first_existing_column(
            slug_to_column, ["as_of_date", "fechaactual", "fechacorte", "mfafecha"]
        )
        origination_col = self._first_existing_column(
            slug_to_column, ["origination_date", "fechadesembolso", "disbursement_date"]
        )
        existing_due_col = self._first_existing_column(
            slug_to_column,
            ["due_date", "fechapagoprogramado", "fechadevencimiento", "fechavencimiento"],
        )
        dpd_col = self._first_existing_column(
            slug_to_column, ["dpd", "dayspastdue", "diasmora", "diasvencidos", "diasvencido"]
        )
        status_col = self._first_existing_column(
            slug_to_column,
            ["status", "currentstatus", "loanstatus", "mdscposteado", "infoclientefinal"],
        )
        ...
        if origination_col and "origination_date" not in normalized.columns:
            normalized["origination_date"] = self._coerce_datetime_loose(
                normalized[origination_col]
            )
        ...
        existing_dpd = (
            self._coerce_numeric_loose(normalized[dpd_col])
            if dpd_col
        ...
        if (
            "status" not in normalized.columns
            or normalized["status"].isna().all()
            or (normalized["status"].astype("string").str.strip() == "").all()
        ):
    ...
    def _first_existing_column(
        slug_to_column: Dict[str, str], candidates: list[str]
    ) -> Optional[str]:
    ...
    def _build_due_date(
        self, df: pd.DataFrame, slug_to_column: Dict[str, str], existing_due_col: Optional[str]
    ) -> Optional[pd.Series]:
        if existing_due_col:
        ...
        due_date.loc[valid_mask] = pd.to_datetime(
            {
                "year": year.loc[valid_mask].astype(int),
                "month": month.loc[valid_mask].astype(int),
                "day": day.loc[valid_mask].astype(int),
            },
            errors="coerce",
        )
        return due_date
    ...
    def _load_from_api(self) -> pd.DataFrame:
        raise RuntimeError(
            "No input file provided. API ingestion was deprecated and dummy/sample fallback is disabled. Pass --input with a real dataset."
        )
    ...
    def _load_from_google_sheets(self, input_path: Optional[Path] = None) -> pd.DataFrame:
        ...
        if not credentials_path or not spreadsheet_id:
            raise ValueError(
                "CRITICAL: Google Sheets ingestion enabled but credentials_path/spreadsheet_id are missing. Set google_sheets config or environment variables."
            )
        adapter = ControlMoraSheetsAdapter(
            credentials_path=str(credentials_path), spreadsheet_id=str(spreadsheet_id)
        )
    ...
    def _enrich_with_intermedia(
        self, df: pd.DataFrame, adapter: ControlMoraSheetsAdapter
    ) -> pd.DataFrame:
        ...
        if bal_col is None:
            logger.warning(
                "INTERMEDIA tab has no TotalSaldoVigente column; skipping balance enrichment"
            )
            return df
        # Prepare merge key
        intermedia_df["_merge_id"] = intermedia_df[id_col].astype(str).str.strip()
        intermedia_df["outstanding_balance"] = pd.to_numeric(
            intermedia_df[bal_col], errors="coerce"
        )
        ...
        matched = (
            df["outstanding_balance"].notna().sum() if "outstanding_balance" in df.columns else 0
        )
        logger.info(
    ...
    def _coerce_google_sheets_schema(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "status" not in df.columns:
            if status_col:
                ...
                raise ValueError(
                    "CRITICAL: Google Sheets data cannot be normalized to canonical schema; missing status source column (expected one of: status, mdscPosteado, mdscposteado, infoclientefinal)."
                )
        missing = [
        ...
        if missing:
            raise ValueError(
                f"CRITICAL: Google Sheets data cannot be normalized to canonical schema; missing columns after mapping: {missing}. Available columns: {sorted(df.columns)}"
            )
        return df
    ...
    def _validate_schema(self, df: pd.DataFrame) -> Dict[str, Any]:
        try:
            ...
            logger.info(
                ...
                extra={"column_count": len(df.columns), "row_count": len(df)},
            )
            return {
                "schema_valid": True,
