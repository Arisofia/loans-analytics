...
_INTERMEDIA_REQUIRED_COLUMNS = {'CodCliente', 'Cliente', 'NumeroInterno', 'FechaDesembolso', 'TotalSaldoVigente'}
...
class ControlMoraSheetsAdapter:
    ...
    def fetch_desembolsos_raw(self) -> List[Dict[str, Any]]:
        return self.fetch_sheet_raw(self.DESEMBOLSOS_TAB, required_columns=_DESEMBOLSOS_REQUIRED_COLUMNS)

    def fetch_intermedia_raw(self) -> List[Dict[str, Any]]:
        return self.fetch_sheet_raw(self.INTERMEDIA_TAB, required_columns=_INTERMEDIA_REQUIRED_COLUMNS)

    def fetch_sheet_raw(self, tab_name: str, required_columns: set[str] | None=None) -> List[Dict[str, Any]]:
        self._ensure_dependencies()
        client = self._authenticate()
        try:
            spreadsheet = client.open_by_key(self._spreadsheet_id)
        except Exception as exc:
            raise ValueError(f"CRITICAL: Could not open spreadsheet '{self._spreadsheet_id}': {exc}") from exc
        ...
        try:
            ...
            raise ValueError(f"CRITICAL: Tab '{normalized_tab}' not found in spreadsheet '{self._spreadsheet_id}': {exc}") from exc
        try:
        ...
        if required_columns:
            ...
            if (missing := sorted(required_columns - present)):
                raise ValueError(f"CRITICAL: Tab '{normalized_tab}' missing required columns {missing}; present={sorted(present)}")
        return records
    ...
    def _ensure_dependencies(self) -> None:
        if gspread is None or ServiceAccountCredentials is None:
            raise ValueError('CRITICAL: gspread and google-auth are required for Google Sheets ingestion')

    def _authenticate(self):
        try:
            creds = ServiceAccountCredentials.from_service_account_file(self._credentials_path, scopes=_SCOPES)
            return gspread.authorize(creds)
        except Exception as exc:
            raise ValueError(f'CRITICAL: Google Sheets authentication failed ({self._credentials_path}): {exc}') from exc
