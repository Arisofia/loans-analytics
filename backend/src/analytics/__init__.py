...
def standardize_numeric(series: pd.Series) -> pd.Series:
    """Convert mixed string/currency/percentage values into numeric floats."""
...
def calculate_quality_score(df: pd.DataFrame) -> float:
    """Return data completeness score in [0, 100]."""
...
def portfolio_kpis(df: pd.DataFrame) -> tuple[dict[str, float], pd.DataFrame]:
    """Compute portfolio-level KPIs and return enriched dataframe."""
    required = [
        "loan_amount",
        "appraised_value",
        "borrower_income",
...
def project_growth(
    ...
    """Create a simple monthly linear projection path."""
    if periods < 2:
    ...
    projection = pd.DataFrame(
        {
            ...
            "loan_volume": np.linspace(float(start_loan_volume), float(end_loan_volume), periods),
        }
    )
