...
def xirr(cashflows: list[float], dates: list[_DateLike], guess: float = _INITIAL_GUESS) -> float:
    ...
    if not (np.any(cfs > 0) and np.any(cfs < 0)):
        raise ValueError(
            "cashflows must contain at least one positive and one negative value; XIRR has no solution when all flows have the same sign"
        )
    ...
    for g in [guess, 0.0, -0.05, 0.5, -0.5, 1.0]:
        try:
            ...
            logger.debug(
                "Newton-Raphson failed for starting guess %s due to numerical error: %s", g, exc
            )
    result = _bisect(_npv)
    if result is not None:
        return result
