...
from backend.loans_analytics.apps.analytics.api.models import (
...
class KPIService:
    ...
    def _build_loan_risk_alert(
        ...
        alerts = []
        if not ltv_valid:
            alerts.append("LTV data insufficient â€” collateral valuation unavailable")
        elif ltv > ltv_threshold:
            alerts.append(f"LTV {ltv:.1f}% exceeds threshold {ltv_threshold}%")
        if not dti_valid:
            alerts.append("DTI data insufficient â€” income data unavailable")
