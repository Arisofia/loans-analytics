...
class OperationalEventCreate(BaseModel):
    event_type: str = Field(..., description='Type of operational event', examples=['pipeline_complete', 'kpi_breach', 'risk_alert', 'sentry_issue'])
    severity: EventSeverity = Field(..., description='Event severity level')
    source: str = Field(..., description='System that emitted the event', examples=['pipeline', 'agent', 'sentry', 'cron'])
    correlation_id: Optional[UUID] = Field(None, description='Correlation ID linking to Sentry traces')
...
class CommandCreate(BaseModel):
    command_type: str = Field(..., description='Type of command to execute', examples=['rerun_pipeline', 'notify_team', 'scale_up', 'acknowledge_alert'])
    requested_by: str = Field(..., description='Who requested the command', examples=['n8n', 'operator', 'auto_rule'])
