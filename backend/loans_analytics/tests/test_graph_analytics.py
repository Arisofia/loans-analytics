...
import pandas as pd
from backend.loans_analytics.kpis.graph_analytics import npl_benchmarks

def _intermedia_df() -> pd.DataFrame:
    return pd.DataFrame({'TotalSaldoVigente': [100.0, 200.0, 300.0], 'FechaPagoProgramado': ['2025-01-01', '2025-12-01', '2026-02-01']})
...
def test_npl_benchmarks_fallback_when_ssot_errors() -> None:
    with patch('backend.loans_analytics.kpis.ssot_asset_quality.KPIFormulaEngine.calculate_kpi', side_effect=RuntimeError('engine error')):
        result = npl_benchmarks(_intermedia_df())
    assert result['status'] == 'ok'
