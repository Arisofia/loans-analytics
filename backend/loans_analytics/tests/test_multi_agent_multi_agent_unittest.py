...
from backend.loans_analytics.multi_agent.protocol import AgentRequest, AgentResponse, AgentRole, Message, MessageRole, Scenario, ScenarioStep, Tool
from backend.loans_analytics.multi_agent.tracing import AgentTracer
...
class TestProtocol(unittest.TestCase):
    ...
    def test_agent_request_validation(self):
        ...
        request = AgentRequest(trace_id='test_123', messages=[msg], context={'key': 'value'}, max_tokens=1000)
    ...
    def test_agent_response_validation(self):
        ...
        response = AgentResponse(trace_id='test_123', agent_role=AgentRole.RISK_ANALYST, message=msg, tokens_used=100, cost_usd=0.001, latency_ms=500.0)
    ...
    def test_scenario_validation(self):
        step = ScenarioStep(agent_role=AgentRole.RISK_ANALYST, prompt_template='Analyze {data}', context_keys=['data'], output_key='analysis')
        scenario = Scenario(name='test_scenario', description='Test scenario', steps=[step])
    ...
    def test_tool_validation(self):
        tool = Tool(name='calculate_risk', description='Calculate risk metrics', parameters={'portfolio': 'object'}, enabled=True)
...
class TestGuardrails(unittest.TestCase):
    ...
    def test_sanitize_context(self):
        context = {'user_email': 'john@example.com', 'ssn': '123-45-6789', 'nested': {'phone': '555-123-4567'}, 'numbers': [1, 2, 3]}
...
class TestTracing(unittest.TestCase):
    ...
    def test_generate_trace_id(self):
        ...
        self.assertEqual(len(trace_id), 22)
    ...
    def test_cost_tracking(self):
        ...
        response1 = AgentResponse(trace_id='test_123', agent_role=AgentRole.RISK_ANALYST, message=msg, tokens_used=100, cost_usd=0.001, latency_ms=500.0)
        response2 = AgentResponse(trace_id='test_123', agent_role=AgentRole.COMPLIANCE, message=msg, tokens_used=150, cost_usd=0.002, latency_ms=600.0)
...
class TestOrchestrator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.openai_patcher = patch('backend.loans_analytics.multi_agent.base_agent.OpenAI')
    ...
    def test_add_custom_scenario(self):
        orchestrator = MultiAgentOrchestrator()
        custom_scenario = Scenario(name='custom_test', description='Test scenario', steps=[ScenarioStep(agent_role=AgentRole.RISK_ANALYST, prompt_template='Test {data}', context_keys=['data'], output_key='result')])
