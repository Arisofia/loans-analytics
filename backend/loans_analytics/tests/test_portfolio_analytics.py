...
import pandas as pd
from backend.loans_analytics.kpis.portfolio_analytics import credit_line_category_kpis

def test_credit_line_category_kpis_uses_ssot_for_par_metrics() -> None:
    loans_df = pd.DataFrame({'loan_id': ['L1', 'L2', 'L3'], 'categorialineacredito': ['A', 'A', 'B'], 'outstanding_loan_value': [100.0, 300.0, 200.0], 'days_past_due': [0, 60, 100], 'disbursement_amount': [120.0, 320.0, 220.0], 'interest_rate_apr': [0.2, 0.3, 0.4], 'term': [90, 120, 150], 'loan_status': ['active', 'active', 'defaulted']})
    with patch('backend.loans_analytics.kpis.ssot_asset_quality.KPIFormulaEngine.calculate_kpi') as mock_calc:
        mock_calc.side_effect = [{'value': 12.3}, {'value': 4.5}, {'value': 67.8}, {'value': 45.6}]
