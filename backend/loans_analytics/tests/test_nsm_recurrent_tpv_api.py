...
def test_nsm_endpoint_returns_empty_response_when_no_runs_dir(tmp_path, monkeypatch):
    ...
    body = response.json()
    assert body['latest_period'] is None  # nosec B101
    assert body['latest'] is None  # nosec B101
    assert body['by_period'] == {}  # nosec B101
...
def test_nsm_endpoint_happy_path(tmp_path, monkeypatch):
    ...
    assert response.status_code == 200  # nosec B101
    body = response.json()
    assert body['latest_period'] == '2026-01'  # nosec B101
    assert body['latest']['tpv_recurrent'] == 6000.0  # nosec B101
    assert body['latest']['tpv_new'] == 3000.0  # nosec B101
    assert body['latest']['tpv_recovered'] == 1000.0  # nosec B101
    assert body['latest']['active_clients'] == 20  # nosec B101
    assert '2026-01' in body['by_period']  # nosec B101
    assert body['by_period']['2026-01']['recurrent_clients'] == 12  # nosec B101
...
def test_nsm_endpoint_supports_current_output_filename(tmp_path, monkeypatch):
    ...
    body = response.json()
    assert body['latest_period'] == '2026-01'  # nosec B101
    assert body['latest']['tpv_recurrent'] == 15000.0  # nosec B101
    assert body['by_period']['2026-01']['recovered_clients'] == 3  # nosec B101
...
def test_nsm_endpoint_schema_completeness(tmp_path, monkeypatch):
    ...
    assert response.status_code == 200  # nosec B101
    body = response.json()
    assert 'latest_period' in body  # nosec B101
    assert 'latest' in body  # nosec B101
    assert 'by_period' in body  # nosec B101
...
def test_nsm_endpoint_selects_most_recently_modified_run(tmp_path, monkeypatch):
    ...
    response = client.get('/analytics/nsm')
    assert response.status_code == 200  # nosec B101
    assert response.json()['latest_period'] == '2026-01'  # nosec B101
