...
def test_advanced_risk_endpoint_returns_expected_shape():
    ...
    body = response.json()
    expected_top_level_keys = {'par30', 'par60', 'par90', 'default_rate', 'collections_coverage', 'fee_yield', 'total_yield', 'recovery_rate', 'concentration_hhi', 'repeat_borrower_rate', 'credit_quality_index', 'total_loans', 'dpd_buckets'}
    assert expected_top_level_keys.issubset(body.keys())
    assert body['total_loans'] == 2
    assert abs(body['par90'] - 52.63) < 1e-09
    assert len(body['dpd_buckets']) == 5
    assert {bucket['bucket'] for bucket in body['dpd_buckets']} == {'current', '1_30', '31_60', '61_90', '90_plus'}
...
def test_full_analysis_layered_insights():
    ...
    assert len(layers) >= 3
    risk_layer = next((layer_item for layer_item in layers if layer_item['layer'] == 'Portfolio Risk'))
    assert 'what' in risk_layer
    assert 'why' in risk_layer
    assert 'so_what' in risk_layer
    assert 'now_what' in risk_layer
    growth_layer = next((layer_item for layer_item in layers if layer_item['layer'] == 'Growth & Profitability'))
