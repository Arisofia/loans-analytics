...
from backend.loans_analytics.apps.analytics.api.models import CohortAnalyticsResponse, LoanRecord
from backend.loans_analytics.apps.analytics.api.service import KPIService

def _loan_record(**overrides: Any) -> LoanRecord:
    base_payload = {'id': 'L0', 'borrower_id': 'B0', 'loan_amount': 1000.0, 'principal_balance': 900.0, 'interest_rate': 0.2, 'loan_status': 'current', 'days_past_due': 0, 'origination_date': datetime(2026, 1, 1), 'total_scheduled': 200.0, 'last_payment_amount': 190.0}
...
def _cohort_loans() -> list[LoanRecord]:
    ...
    return [_loan_record(id='L1', borrower_id='B1', loan_amount=1000.0, principal_balance=900.0, interest_rate=0.2, loan_status='current', days_past_due=0, origination_date=jan, total_scheduled=200.0, last_payment_amount=190.0), _loan_record(id='L2', borrower_id='B2', loan_amount=1500.0, principal_balance=1200.0, interest_rate=0.25, loan_status='90+ days past due', days_past_due=100, origination_date=jan, total_scheduled=300.0, last_payment_amount=120.0), _loan_record(id='L3', borrower_id='B3', loan_amount=800.0, principal_balance=700.0, interest_rate=0.22, loan_status='default', days_past_due=120, origination_date=feb, total_scheduled=250.0, last_payment_amount=80.0)]
