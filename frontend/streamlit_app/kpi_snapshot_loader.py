...
def _to_snapshot_entry(kpi: KPIResponse) -> dict[str, object]:
    return {'value': kpi.value, 'threshold_status': kpi.threshold_status, 'thresholds': kpi.thresholds, 'unit': kpi.unit, 'name': kpi.name, 'updated_at': kpi.updated_at}

def _kpi_entry_from_dict(kpi: dict[str, object]) -> tuple[str, dict[str, object]]:
    kpi_id = str(kpi.get('id', '')).strip()
    if not kpi_id:
        return ('', {})
    return (kpi_id, {'value': kpi.get('value'), 'threshold_status': kpi.get('threshold_status', 'not_configured'), 'thresholds': kpi.get('thresholds', {}), 'unit': kpi.get('unit', 'unknown'), 'name': kpi.get('name', ''), 'updated_at': kpi.get('updated_at')})
...
def load_kpi_snapshot_from_api(api_url: Optional[str]=None) -> tuple[dict[str, dict[str, object]], Optional[pd.Timestamp], bool]:
