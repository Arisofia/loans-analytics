...
try:
    ...
    def _ssot_calculate_par(  # type: ignore[misc]
        ...
        return (par_amount / total_portfolio).quantize(Decimal("0.0001"), rounding=ROUND_HALF_UP)
...
st.set_page_config(
    page_title="Loans Analytics", page_icon="📊", layout="wide", initial_sidebar_state="expanded"
)
st.markdown(
...
STATUS_ALIAS_TO_CANONICAL = (
    dict.fromkeys(_ACTIVE_ALIASES, "active")
    | dict.fromkeys(_CLOSED_ALIASES, "closed")
    | dict.fromkeys(_DELINQUENT_ALIASES, "delinquent")
    | dict.fromkeys(_DEFAULTED_ALIASES, "defaulted")
)
...
COLUMN_ALIASES = {
    ...
    "borrower_id": ["customer_id", "customer_id_cust", "codcliente", "codcliente1", "cliente_id"],
    "principal_amount": [
        "principal",
        "loan_amount",
        "principal_balance",
    ...
    "kam_hunter": ["cod_kam_hunter", "cod_kam_hunter_", "cod_kam_hunter1", "cod_kam_hunter_1"],
    "kam_farmer": [
        "cod_kam_farmer",
        "cod_kam_farmer_",
        "cod_kam_farmer1",
...
def prepare_uploaded_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    prepared["origination_date"] = pd.to_datetime(
        prepared["origination_date"], errors="coerce", format="mixed"
    )
    if (
...
def _compute_dpd_and_par_metrics(
    portfolio_df: pd.DataFrame, total_loans: int, total_portfolio: float
) -> dict[str, float | int]:
    ...
    par_30_rate = float(
        _ssot_calculate_par(Decimal(str(par_30_amount)), total_portfolio_dec, dpd_threshold=30)
    )
    par_90_rate = float(
        _ssot_calculate_par(Decimal(str(par_90_amount)), total_portfolio_dec, dpd_threshold=90)
    )
    return {
...
def render_metrics_cards(metrics: dict[str, Any]):
    ...
    with col4:
        st.metric(
            ...
            delta=f"{KPI_EXPECTED_LOSS}: {metrics['expected_loss_rate']:.2f}%",
            delta_color="inverse",
        )
...
def _build_local_agent_fallback(df: pd.DataFrame, metrics: dict[str, Any]) -> dict[str, Any]:
    ...
    top_regions_text = (
        ...
        if top_regions_series.empty
        else ", ".join((f"{idx}: €{val:,.0f}" for idx, val in top_regions_series.items()))
    )
...
def _persist_and_notify_agent_results(results: dict[str, Any]) -> None:
    ...
    if emitted_kpi_events > 0:
        ...
        st.caption(
            "Agent comments were generated locally. Set API_BASE_URL to publish them to Monitoring/Grafana."
        )
    if saved_outputs > 0:
        st.caption(
    ...
    with st.spinner("Running multi-agent analysis..."):
        ...
        try:
            ...
            fallback_results = _enrich_agent_results_with_kpis(
                _build_local_agent_fallback(df, metrics), metrics, "local_fallback_after_error"
            )
...
def render_sidebar() -> Any:
    ...
    uploaded_files = st.file_uploader(
        ...
        help=f"You can upload one consolidated file or multiple partial files. Minimum required columns: {', '.join(CORE_REQUIRED_COLUMNS)}",
    )
    st.markdown("---")
...
def _store_uploaded_dataframe(prepared_df: pd.DataFrame, uploaded_files: Any) -> None:
    ...
    st.success(
        f"✅ Data uploaded and validated successfully! Files: {len(uploaded_files)} | Rows: {len(prepared_df):,}"
    )
...
def render_data_format_guide():
    ...
    with col1:
        _render_markdown_items(
            ...
            ["borrower_name", "borrower_email", "borrower_id_number", "region"],
        )
    with col2:
        _render_markdown_items(
            "**Loan Details (optional):**", ["risk_score", "payment_history_json"]
        )
...
def _render_agent_kpi_basis(results: dict[str, Any], metrics: dict[str, Any]) -> None:
    with st.expander("KPI basis used in agent analysis", expanded=False):
        ...
        rows = (
            ...
            else _kpi_methodology_rows(metrics)
        )
...
def _build_segment_row(
    grp: pd.DataFrame, exposure_col: str, dpd_col: str | None, status_col: str | None, seg_val: Any
) -> dict[str, Any] | None:
...
def _render_segment_par_chart(seg_df: pd.DataFrame, dim_label: str) -> None:
    ...
    fig = px.bar(
        seg_df,
        x="Segment",
        y=[SEG_PAR_30, SEG_PAR_60, SEG_PAR_90],
        barmode="group",
        title=f"PAR 30 / 60 / 90 by {dim_label}",
        color_discrete_map={SEG_PAR_30: "#ffc107", SEG_PAR_60: "#ff9800", SEG_PAR_90: "#dc3545"},
        height=380,
    )
...
def render_segment_breakdown(df: pd.DataFrame) -> None:
    ...
    dim_label = st.selectbox(
        "Segment dimension", options=list(available_dims.values()), index=0, key="seg_dim_select"
    )
...
def _empty_temporal_metadata(
    disb_col: str | None,
    pay_col: str | None,
    principal_col: str | None,
    outstanding_col: str | None,
) -> dict[str, Any]:
    return {
        "raw_rows": 0,
        "dedup_rows": 0,
...
def _build_operational_temporal_key_parts(
    df: pd.DataFrame, work: pd.DataFrame, principal_work: pd.Series
    ...
    key_parts = [
        _string_part(work, client_col),
        _string_part(work, disb_num_col),
...
def _build_legacy_temporal_key_parts(
    df: pd.DataFrame, work: pd.DataFrame, principal_work: pd.Series
) -> tuple[list[pd.Series], str]:
...
def _build_temporal_key_data(
    df: pd.DataFrame, work: pd.DataFrame, principal_work: pd.Series, key_strategy: str
) -> tuple[list[pd.Series], str]:
...
def _calculate_temporal_active_set(
    ...
    if work.empty:
        return (
            work,
            _empty_temporal_metadata(disb_col, pay_col, principal_col, outstanding_col),
            None,
        )
    ...
    metadata = {
        ...
        "principal_sum": dedup["_temporal_principal"].sum(),
        "outstanding_sum": dedup["_temporal_outstanding"].sum(),
        "disbursement_col": disb_col,
        "payment_col": pay_col,
        "principal_col": principal_col,
        "outstanding_col": outstanding_col,
...
def render_temporal_active_set(df: pd.DataFrame) -> None:
    ...
    st.caption(
        "Definition: disbursement_date <= cutoff and (payment_date is null or payment_date > cutoff). Use this to reconcile principal disbursed pending payment."
    )
    ...
    if disb_col is None or valid_disb.empty:
        st.warning(
            "Temporal filter unavailable: missing disbursement dates (origination_date/fechadesembolso/fecha_de_desembolso)."
        )
        return
    ...
    with col2:
        key_choice = st.selectbox(
            "Dedup key",
            options=["operational_stable", "loan_legacy"],
    ...
    dedup_df, metadata, error = _calculate_temporal_active_set(
        df, cutoff=pd.Timestamp(cutoff_date), key_strategy=key_choice
    )
    if error:
        st.warning(error)
        return
    ...
    st.caption(
        f"Columns used: disbursement={metadata.get('disbursement_col') or 'NA'} | payment={metadata.get('payment_col') or 'NA'} | principal={metadata.get('principal_col') or 'NA'} | outstanding={metadata.get('outstanding_col') or 'NA'}"
    )
