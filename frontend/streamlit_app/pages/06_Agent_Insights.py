...
def _forecast_df(projections: List[KpiProjection]) -> pd.DataFrame:
    rows = [{'date': item.projection_date, 'predicted_value': item.predicted_value, 'lower_bound': item.lower_bound, 'upper_bound': item.upper_bound, 'method': item.method} for item in projections]
    frame = pd.DataFrame(rows)
    if frame.empty:
        return frame
    frame['date'] = pd.to_datetime(frame['date'])
...
def _seasonality_df(adjustments: dict[int, float]) -> pd.DataFrame:
    ...
    for month_idx in sorted(adjustments):
        ...
        rows.append({'month': month_name[month_int], 'month_number': month_int, 'adjustment_factor': adjustments[month_idx]})
...
try:
    ...
    st.warning(f'REAL mode is not available in the current environment. Reason: {exc}. Falling back to MOCK mode.')
    selected_mode = 'MOCK'
...
if not history_frame.empty:
    chart.add_trace(go.Scatter(x=history_frame['date'], y=history_frame['value'], mode='lines', name='Historical', line={'width': 2}))
if not forecast_frame.empty:
    chart.add_trace(go.Scatter(x=forecast_frame['date'], y=forecast_frame['predicted_value'], mode='lines', name=f'Forecast ({forecast_method})', line={'dash': 'dash', 'width': 2}))
    chart.add_trace(go.Scatter(x=forecast_frame['date'], y=forecast_frame['upper_bound'], mode='lines', line={'width': 0}, showlegend=False, hoverinfo='skip'))
    chart.add_trace(go.Scatter(x=forecast_frame['date'], y=forecast_frame['lower_bound'], mode='lines', line={'width': 0}, fill='tonexty', fillcolor='rgba(31, 119, 180, 0.15)', name='Confidence Band'))
chart.update_layout(title=f'KPI Trajectory: {kpi_id}', xaxis_title='Date', yaxis_title='Value', template='plotly_white', height=460)
...
with left_col:
    ...
    if not seasonality_frame.empty:
        st.dataframe(seasonality_frame[['month', 'adjustment_factor']], width='stretch', hide_index=True)
    else:
