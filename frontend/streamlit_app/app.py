...
from frontend.streamlit_app.components.sales_risk import render_risk_analysis, render_sales_performance
ROOT_DIR = Path(__file__).resolve().parent.parent.parent
LOCAL_EXPORTS_DIR = ROOT_DIR / 'local_exports'
EXPORTS_DIR_CANDIDATES = [ROOT_DIR / 'exports', ROOT_DIR / 'data' / 'archives' / '_exports']
...
def handle_file_uploads() -> dict[str, pd.DataFrame]:
    sidebar_uploaded_files_widget = st.sidebar.file_uploader('Upload CSV Exports', type='csv', accept_multiple_files=True)
    ...
    if sidebar_uploaded_files_widget:
        for uploaded_file_widget_item in sidebar_uploaded_files_widget:
            if upload_widget_key := get_table_type_from_filename(uploaded_file_widget_item.name):
                ...
                st.sidebar.warning(f"Could not map file '{uploaded_file_widget_item.name}' to known data types.")
    return upload_data
...
def _map_dict_sheets(sheets: dict[str, pd.DataFrame], mapped_core_tables_dict: dict[str, pd.DataFrame]) -> None:
...
def fuzzy_map_core_tables(input_dfs: dict[str, pd.DataFrame | dict[str, pd.DataFrame]]) -> dict[str, pd.DataFrame]:
    ...
    for export_dir in export_dirs:
        ...
        if dashboard_path.exists():
            try:
                ...
                logger.warning('Failed to parse KPI dashboard export %s: %s', dashboard_path, exc)
                st.warning(f'⚠️ KPI dashboard file is corrupted or incomplete and could not be loaded ({dashboard_path.name}). Re-run the pipeline to regenerate it.')
    return {}
...
def _normalize_single_export_table(table: pd.DataFrame | None, *, normalize_inputs: bool) -> pd.DataFrame:
    if not isinstance(table, pd.DataFrame) or table.empty:
        return pd.DataFrame()
    return normalize_dataframe_complete(table) if normalize_inputs else table

def _normalize_export_tables(loans_df: pd.DataFrame, customers_df: pd.DataFrame, payments_df: pd.DataFrame, schedule_df: pd.DataFrame | None, *, normalize_inputs: bool) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    normalized_loans = _normalize_single_export_table(loans_df, normalize_inputs=normalize_inputs)
    normalized_customers = _normalize_single_export_table(customers_df, normalize_inputs=normalize_inputs)
    normalized_payments = _normalize_single_export_table(payments_df, normalize_inputs=normalize_inputs)
...
with st.sidebar:
    ...
    if data_source == 'Local artifacts (auto)':
        ...
        main_uploaded_files = st.file_uploader('Upload Loan Tape CSVs and Financial XLSX', accept_multiple_files=True, type=['csv', 'xlsx'])
        if st.button('Ingest Data') or main_uploaded_files:
            ...
            if main_dfs:
                ...
                usage_tracker.track('data_ingestion', 'manual_upload', file_count=len(main_uploaded_files))
                with st.spinner('Generating KPI exports from uploaded data...'):
                    try:
                        output_path = generate_kpi_exports(final_data, normalize_inputs=False)
                        st.cache_data.clear()
...
with st.expander('🔗 Dashboard Links & Strategic Reporting', expanded=True):
    ...
    if st.button('Generate Strategic Report'):
        if global_dashboard_metrics_var:
            ...
            strategic_links = {'streamlit_local': dashboard_local_url, 'grafana_local': grafana_local_url, 'streamlit_prod': deployed_dashboard_url, 'dashboard_docs': 'docs/analytics/dashboards.md'}
            strategic_json_path, strategic_md_path = write_strategic_report(summary=strategic_summary, links=strategic_links, output_dir=ROOT_DIR / 'reports' / 'strategic')
...
if not global_is_api_source and (not global_dashboard_metrics_var) and global_analytics_facts_var.empty:
    st.warning('No KPI exports detected. Generate KPI exports from the sidebar or add complete_kpi_dashboard.json and analytics_facts.csv to the exports directory.')
if global_is_api_source:
    global_kpi_snapshot, global_snapshot_month_var = (global_api_kpi_snapshot, global_api_snapshot_month)
else:
    global_kpi_snapshot, global_snapshot_month_var = build_kpi_snapshot(global_dashboard_metrics_var, global_analytics_facts_var)
...
if not customer_data_df.empty and 'loan_id' in merged_df.columns and ('loan_id' in customer_data_df.columns):
    merged_df = merged_df.merge(customer_data_df, on='loan_id', how='left', suffixes=('', '_cust'))
