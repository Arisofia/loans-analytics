...
class KPIWebSocketClient:
    ...
    def __init__(self, ws_url: Optional[str]=None, connect_timeout: float=5.0, receive_timeout: float=5.0):
        self.ws_url = ws_url or os.getenv('KPI_WS_URL', 'ws://127.0.0.1:8000/analytics/kpis/stream')
        self.connect_timeout = connect_timeout
        self.receive_timeout = receive_timeout

    async def async_get_snapshot(self, kpi_keys: Optional[list[str]]=None, once: bool=True, interval_seconds: float=5.0) -> dict[str, Any]:
        if not websockets:
...
def get_snapshot_once(ws_url: Optional[str]=None, kpi_keys: Optional[list[str]]=None) -> dict[str, Any]:
