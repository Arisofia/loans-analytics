...
def render_cashflow_trends(analytics_facts):
    if not analytics_facts.empty:
        ...
        st.header('💸 Cashflow')
        cash_cols = ['recv_revenue_for_month', 'recv_interest_for_month', 'recv_fee_for_month', 'sched_revenue']
        ...
        if available_cols:
            ...
            fig_cash = px.line(cash_df, x='month', y=available_cols, title='Cashflow Trends', markers=True)
...
def render_category_breakdown(merged, col):
    with col:
        try:
            ...
            elif 'categoria' in merged.columns and 'outstanding_loan_value' not in merged.columns:
