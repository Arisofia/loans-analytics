...
def _render_forecasting_tab(dashboard_metrics):
    ...
    if {'lower_bound_usd', 'upper_bound_usd'}.issubset(forecast_df.columns):
        fig.add_scatter(x=forecast_df['month'], y=forecast_df['upper_bound_usd'], mode='lines', name='Upper Bound', line={'dash': 'dot'})
        fig.add_scatter(x=forecast_df['month'], y=forecast_df['lower_bound_usd'], mode='lines', name='Lower Bound', line={'dash': 'dot'}, fill='tonexty')
...
def _render_opportunity_tab(dashboard_metrics):
    ...
    opportunity_rows = dashboard_metrics.get('extended_kpis', {}).get('opportunity_prioritization', [])
    ...
    if {'client_segment', 'priority_score'}.issubset(opportunity_df.columns):
        fig = px.bar(opportunity_df, x='client_segment', y='priority_score', color='Delinquency_Rate' if 'Delinquency_Rate' in opportunity_df.columns else None, title='Priority Score by Segment')
        st.plotly_chart(apply_theme(fig), width='stretch')
...
def _render_unit_economics_tab(dashboard_metrics):
    ...
    if 'month' in unit_df.columns:
        ...
        if 'ltv_cac_ratio' in unit_df.columns:
            ...
            fig_ratio.add_hline(y=3.0, line_dash='dash', line_color='green', annotation_text='Target (3.0x)')
            st.plotly_chart(apply_theme(fig_ratio), width='stretch')
        if 'gross_margin_pct' in unit_df.columns:
            fig_margin = px.line(unit_df, x='month', y='gross_margin_pct', title='Gross Margin Trend', markers=True)
...
def _render_pricing_governance_tab(dashboard_metrics):
    ...
    pc3.metric('Weighted Effective Rate', f"{current_pricing.get('weighted_effective_rate', 0.0):.2%}")
    ...
    if governance:
        ...
        if completeness:
            st.dataframe(styled_df(pd.DataFrame(list(completeness.items()), columns=['Field', 'Completeness'])), width='stretch')
...
def render_advanced_intelligence(dashboard_metrics):
    ...
    if strategic:
        ...
        c4.metric('Revenue Forecast', '✅' if strategic.get('revenue_forecast_confirmed') else '❌')
    adv_tabs = st.tabs(['Forecasting', 'Opportunity', 'Churn & Retention', 'Unit Economics', 'Pricing & Governance'])
    with adv_tabs[0]:
