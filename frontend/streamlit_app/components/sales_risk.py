...
def render_sales_performance(merged, load_agent_headcount):
    ...
    if 'sales_agent' in merged.columns:
        ...
        if 'outstanding_loan_value' in merged.columns:
            ...
            fig_sales = px.bar(sales_agg, x='sales_agent', y='Count', title='Sales Agent Loan Count')
        ...
        if not headcount_df.empty and {'month', 'function', 'fte_count'}.issubset(headcount_df.columns):
            ...
            fig_headcount = px.bar(latest_headcount, x='function', y='fte_count', color='team' if 'team' in latest_headcount.columns else None, title='Headcount by Function')
...
def render_risk_analysis(merged):
    ...
    with r_col1:
        if 'days_in_default' in merged.columns:
            merged['dpd_bucket'] = pd.cut(merged['days_in_default'], bins=[-1, 0, 30, 60, 90, float('inf')], labels=['Current', '1-30', '31-60', '61-90', '90+'])
