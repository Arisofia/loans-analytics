...
BORROWER_ID_COLS: tuple[str, ...] = ('borrower_id', 'client_code', 'client_name', 'borrower_name')
ALIAS_MAP: dict[str, list[str]] = {'loan_id': ['id_loan', 'idprestamo', 'prestamo_id', 'new_loan_id', 'old_loan_id', 'application_id', 'numero_desembolso', 'codigo_desembolso', 'correlativo_de_operaciones', 'nrc', 'id'], 'borrower_id': ['customer_id', 'customer_id_cust', 'codcliente', 'codcliente_', 'codcliente_2', 'codcliente1', 'cliente_id', 'borrower_id'], 'amount': ['loan_amount', 'principal_amount', 'principal_balance', 'outstanding_balance', 'current_balance', 'saldo', 'saldo_capital', 'capital_pendiente', 'monto_vigente', 'monto_actual', 'monto_total', 'monto_del_desembolso', 'monto_financiado_real_por_desembolso', 'monto_total_aprobado_por_desembolso', 'monto_financiado_real', 'monto_total_aprobado', 'monto', 'monto_prestamo', 'monto_financiado', 'valor_financiado', 'valor_factura', 'importe', 'capital'], 'outstanding_balance': ['outstanding_balance', 'current_balance', 'principal_balance', 'totalsaldovigente', 'saldo_vigente', 'saldo_pendiente', 'capital_pendiente', 'saldo', 'saldo_capital'], 'current_balance': ['current_balance', 'outstanding_balance', 'principal_balance', 'totalsaldovigente', 'saldo_vigente', 'saldo_pendiente', 'capital_pendiente', 'saldo', 'saldo_capital'], 'principal_amount': ['principal_amount', 'loan_amount', 'monto_del_desembolso', 'monto_financiado_real_por_desembolso', 'monto_total_aprobado_por_desembolso', 'monto_financiado_real', 'monto_total_aprobado', 'montodesembolsado', 'amount'], 'status': ['loan_status', 'current_status', 'estado', 'estado_actual'], 'days_past_due': ['dpd', 'dias_mora', 'dias_de_mora', 'dias_en_mora', 'dias_vencido', 'mora_en_dias', 'diias_mora_m', 'dias_mora_m', 'dias_atraso', 'days_overdue'], 'origination_date': ['disbursement_date', 'fecha_desembolso', 'fechadesembolso', 'fecha_de_desembolso', 'fecha_originacion'], 'interest_rate': ['interest_rate_apr', 'apr', 'rate', 'tasa', 'tasainteres', 'tasa_interes', 'tasa_de_interes', 'tasa_anual'], 'due_date': ['due_date', 'fechapagoprogramado', 'fecha_vencimiento', 'fecha_de_vencimiento', 'fecha_vto'], 'last_payment_date': ['last_payment_date', 'payment_date', 'fecha_de_pago', 'fechacobro', 'fecha_pago'], 'total_scheduled': ['scheduled_amount', 'total_due', 'monto_programado', 'monto_cuota', 'cuota', 'valorcuota'], 'last_payment_amount': ['payment_amount', 'monto_pagado', 'ultimo_pago', 'last_real_payment', '_pagado'], 'recovery_value': ['recovery_value_', 'recovery_value', 'recovery_amount'], 'client_code': ['codcliente', 'codcliente_', 'codcliente_2', 'codcliente1'], 'client_name': ['cliente', 'cliente_', 'cliente1', 'nombre_cliente', 'nombre_del_cliente', 'nombrecliente', 'razon_social'], 'issuer_code': ['codemisor'], 'issuer_name': ['emisor'], 'company': ['company', 'empresa', 'compania'], 'credit_line': ['lineacredito'], 'kam_hunter': ['cod_kam_hunter', 'cod_kam_hunter_', 'cod_kam_hunter1', 'cod_kam_hunter_1', 'kam_hunter'], 'kam_farmer': ['cod_kam_farmer', 'cod_kam_farmer_', 'cod_kam_farmer1', 'cod_kam_farmer_1', 'kam_farmer'], 'advisory_channel': ['asesoriadigital'], 'application_date': ['fechasolicitado'], 'term_months': ['term_months', 'plazo', 'plazo_meses', 'term_max', 'term_ponderado'], 'payment_frequency': ['payment_frequency', 'frecuencia_pago', 'tipo_pago'], 'tpv': ['tpv', 'ingreso_total_por_desembolso', 'montototalabonado'], 'utilization_pct': ['porcentaje_utilizado'], 'collections_eligible': ['procede_a_cobrar'], 'delinquency_definition': ['definicion_m'], 'delinquency_bucket_raw': ['rango_m'], 'credit_line_range': ['rango_de_la_linea'], 'ministry': ['ministerio'], 'government_sector': ['goes'], 'capital_collected': ['capitalcobrado'], 'total_payment_received': ['montototalabonado'], 'mdsc_posted': ['mdscposteado'], 'negotiation_days': ['diasnegociacion', 'dias_negociacion'], 'days_to_pay': ['dias_en_pagar'], 'disbursement_count': ['numerodesembolsos'], 'approved_value': ['valoraprobado'], 'negotiation_income': ['ingreso_total_por_desembolso', 'ingreso_pagadopendiente'], 'commission_code': ['comision_cobrada'], 'pending_income': ['ingresos_pendiente']}
...
def render_csv_upload() -> None:
    ...
    uploaded_files = st.file_uploader('Choose CSV/Excel file(s)', type=['csv', 'xlsx', 'xls'], accept_multiple_files=True, help='You can upload one consolidated file or multiple source tables. The uploader maps aliases automatically.')
    ...
    for idx, uploaded_file in enumerate(uploaded_files, 1):
        ...
        with st.expander(f'📄 {uploaded_file.name} ({_format_file_size(uploaded_file.size)})', expanded=idx == 1):
            ...
            with st.expander('📋 Column Information'):
                col_info = pd.DataFrame({'Column': raw_df.columns, 'Type': raw_df.dtypes.astype(str), 'Non-Null': raw_df.notna().sum(), 'Null': raw_df.isna().sum(), 'Null %': (raw_df.isna().sum() / len(raw_df) * 100).round(2)})
                st.dataframe(_to_arrow_safe_display_df(col_info), width='stretch')
            st.subheader('✅ Validation Checks')
            _render_validation(prepared_df)
            if st.button(f'🚀 Process {uploaded_file.name}', key=f'process_{uploaded_file.name}', type='primary'):
                valid, missing, issues, notices = _validate_for_pipeline(prepared_df)
                if not valid:
                    st.error('Cannot process this file yet. Missing required columns after mapping: ' + ', '.join(missing))
                elif issues:
    ...
    if len(prepared_frames) > 1:
        ...
        if st.button('🚀 Process Consolidated Dataset', key='process_consolidated', type='primary'):
            if not valid:
                st.error('Cannot process consolidated dataset. Missing required columns after mapping: ' + ', '.join(missing))
            else:
                if issues:
                    st.warning('Data quality warnings: ' + ' | '.join(issues))
                elif notices:
...
def _alias_matches(columns: list[str], alias: str) -> list[str]:
    pattern = re.compile(f'^{re.escape(alias)}(?:_?\\d+|_+)?$')
...
def _apply_aliases(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for canonical, aliases in ALIAS_MAP.items():
        ...
        if len(candidates) == 1:
            ...
            continue
        merged_series = _coalesce_series([_nullify_missing_entries(normalized[col]) for col in candidates], index=normalized.index)
...
def _derive_amount(df: pd.DataFrame) -> pd.Series:
    ...
    for fallback_col in ['outstanding_balance', 'current_balance', 'principal_balance', 'totalsaldovigente', 'saldo_vigente', 'saldo_pendiente', 'capital_pendiente', 'saldo', 'saldo_capital', 'principal_amount', 'loan_amount', 'monto_del_desembolso', 'monto_financiado_real_por_desembolso', 'monto_total_aprobado_por_desembolso', 'monto_financiado_real', 'monto_total_aprobado', 'montodesembolsado']:
...
def _prepare_dataframe(raw_df: pd.DataFrame) -> pd.DataFrame:
    ...
    prepared = prepared.assign(amount=_derive_amount(prepared), status=_derive_status(prepared))
    balance_sources: list[pd.Series] = []
    for source_col in ['totalsaldovigente', 'saldo_vigente', 'saldo_pendiente', 'capital_pendiente', 'saldo', 'saldo_capital', 'outstanding_balance', 'current_balance', 'amount']:
    ...
    for source_col in ['principal_amount', 'loan_amount', 'monto_del_desembolso', 'monto_financiado_real_por_desembolso', 'monto_total_aprobado_por_desembolso', 'monto_financiado_real', 'monto_total_aprobado', 'montodesembolsado', 'amount']:
        if source_col in prepared.columns:
    ...
    principal_series = _coalesce_series(principal_sources, index=prepared.index, default=prepared['amount'])
    prepared = prepared.assign(outstanding_balance=_coerce_numeric(balance_series), current_balance=_coerce_numeric(balance_series), principal_amount=_coerce_numeric(principal_series), amount=_coerce_numeric(prepared['amount']))
    ...
    for numeric_col in ['days_past_due', 'interest_rate', 'last_payment_amount', 'total_scheduled', 'recovery_value', 'term_months', 'tpv', 'current_balance', 'outstanding_balance', 'principal_amount', 'balance']:
...
def _collapse_to_loan_level(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if work.empty:
        return work
    date_candidates = ['as_of_date', 'snapshot_date', 'measurement_date', 'reporting_date', 'last_payment_date', 'origination_date', 'application_date', 'maturity_date']
    ...
    work = work.sort_values(['loan_id', '_event_date', '_row_order'], ascending=[True, True, True], na_position='last')
    work = work.copy()
...
def _classify_loan_id_duplicates(df: pd.DataFrame) -> list[tuple[str, str]]:
    ...
    if exact_dup_count:
        messages.append(('warning', f'{exact_dup_count} loan_id(s) have exact duplicate rows (same borrower, same amount) — consider deduplicating before processing'))
    if snapshot_count:
        messages.append(('info', f'{snapshot_count} loan_id(s) appear as historical snapshots (same borrower, different amounts) — treated as balance rollups'))
    if suspicious_count:
        messages.append(('warning', f'{suspicious_count} loan_id(s) are shared across different borrowers (suspicious merge) — verify source data integrity'))
    if generic_count:
        messages.append(('warning', f'{generic_count} loan_id(s) have duplicate rows (borrower/amount columns unavailable for detailed classification)'))
    return messages
        ...
        with col2:
            ...
            if json_path.exists():
                with open(json_path, 'rb') as fh:
                    st.download_button('📄 Download JSON', fh, file_name=f'kpis_{run_id}.json', mime='application/json')
        with col3:
            parquet_path = run_dir / 'kpis_output.parquet'
            if parquet_path.exists():
                with open(parquet_path, 'rb') as fh:
                    st.download_button('📄 Download Parquet', fh, file_name=f'kpis_{run_id}.parquet', mime='application/octet-stream')
...
def _build_upload_metadata(df: pd.DataFrame, filename: str) -> dict[str, Any]:
    ...
    return {'source_file': filename, 'uploaded_at': datetime.now().isoformat(), 'detected_as_of_date': as_of_date, 'detected_as_of_source': as_of_source, 'row_count': int(len(df)), 'column_count': int(len(df.columns)), 'field_non_empty': field_non_empty}

def _detect_as_of_date(df: pd.DataFrame) -> tuple[str | None, str | None]:
    primary_candidates = ['as_of_date', 'snapshot_date', 'measurement_date', 'reporting_date', 'fecha_corte', 'fecha_de_corte', 'cutoff_date', 'data_ingest_ts']
    fallback_candidates = ['last_payment_date', 'origination_date', 'application_date']
