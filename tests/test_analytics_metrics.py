...
def sample_df() -> pd.DataFrame:
    return pd.DataFrame(
        {
            "loan_amount": [100000, 50000, 25000],
            "appraised_value": [120000, 0, 20000],
            "borrower_income": [120000, 0, 60000],
            "monthly_debt": [2000, 1000, 1500],
            "loan_status": ["current", "delinquent", "30-59 days past due"],
            "principal_balance": [90000, 45000, 24000],
...
def test_standardize_numeric_handles_negative_symbols_and_commas():
    series = pd.Series(["-1,200", "-$3,000", "-25%"], dtype=object)
    cleaned = standardize_numeric(series)
    assert cleaned.iloc[0] == -1200.0
...
def test_calculate_quality_score_penalizes_missing_values(sample_df):
    df = sample_df.copy()
...
def test_calculate_quality_score_clamps_at_zero():
...
def test_portfolio_kpis_returns_expected_metrics(sample_df):
    ...
    expected_delinquency_rate = (2 / 3) * 100
    expected_portfolio_yield = (
        df["principal_balance"] * df["interest_rate"]
    ).sum() / df["principal_balance"].sum()
    expected_average_ltv = (df["loan_amount"] / df["appraised_value"]).replace([np.inf, -np.inf], np.nan).mean(skipna=True)
    expected_average_dti = (
        df["monthly_debt"] / (df["borrower_income"] / 12)
    ).replace([np.inf, -np.inf], np.nan).mean(skipna=True)
    assert metrics["delinquency_rate"] == pytest.approx(expected_delinquency_rate, rel=1e-6)
    assert metrics["portfolio_yield"] == pytest.approx(expected_portfolio_yield, rel=1e-6)
    assert metrics["average_ltv"] == pytest.approx(expected_average_ltv, rel=1e-6)
    assert metrics["average_dti"] == pytest.approx(expected_average_dti, rel=1e-6)
...
def test_portfolio_kpis_ltv_nan_when_appraisal_non_positive(sample_df):
    df = sample_df.copy()
...
def test_project_growth_builds_monotonic_path():
    ...
    assert projection["yield"].is_monotonic_increasing
    assert projection["loan_volume"].is_monotonic_increasing
...
def test_project_growth_default_periods():
