...
def test_pipeline_config_load_valid(tmp_path):
    config_data = {'ingestion': {'source': 'csv'}, 'transformation': {'clean': True}, 'calculation': {'metrics': ['par30']}, 'output': {'format': 'parquet'}, 'external_integrations': {'supabase': True}, 'observability': {'enabled': True}}
