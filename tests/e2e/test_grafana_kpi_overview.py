"""Grafana E2E: ABACO KPI Overview should not be fully empty or broken."""
...
pytestmark = [
    ...
    pytest.mark.skipif(
        not RUN_GRAFANA_E2E,
        reason="Set RUN_GRAFANA_E2E=1 to include Grafana E2E checks",
    ),
]
...
def datasource_uid() -> str:
    ...
    ds_resp = requests.get(
        f"{GRAFANA_BASE_URL}/api/datasources/name/{ds_name}",
        auth=AUTH,
        timeout=10,
    )
    assert (
    ...
    ds_health = requests.get(
        f"{GRAFANA_BASE_URL}/api/datasources/uid/{ds_uid}/health",
        auth=AUTH,
        timeout=15,
    )
    assert ds_health.status_code == 200, f"Datasource health endpoint failed: {ds_health.text}"
    assert ds_health.json().get("status") == "OK", (
        f"Datasource unhealthy: {ds_health.text}. "
        "Common causes: missing/expired DB credentials or password decryption issues."
    )
...
def test_abaco_kpi_overview_queries(datasource_uid: str):
    dash_resp = requests.get(
        f"{GRAFANA_BASE_URL}/api/dashboards/uid/{GRAFANA_DASHBOARD_UID}",
        auth=AUTH,
        timeout=15,
    )
    assert (
    ...
    for idx, (panel_id, title, raw_sql) in enumerate(sql_targets, start=1):
        ...
        if result.get("error"):
            failures.append(
                f"panel {panel_id} ({title}) -> {result.get('errorSource')}: {result.get('error')}"
            )
            continue
    ...
    if EXPECT_GRAFANA_DATA:
        assert total_rows > 0, (
            "ABACO KPI Overview returned zero rows across all panel queries. "
            "This indicates global 'No data' (empty tables, wrong schema, or stale ingestion)."
        )
