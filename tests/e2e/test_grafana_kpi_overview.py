...
pytestmark = [
    ...
    pytest.mark.skipif(not RUN_GRAFANA_E2E, reason='Set RUN_GRAFANA_E2E=1 to include Grafana E2E checks'),
    pytest.mark.skipif(not GRAFANA_PASSWORD, reason='Set GRAFANA_PASSWORD (or GRAFANA_ADMIN_PASSWORD) to run Grafana E2E checks'),
]
...
def _collect_sql_targets(panels: list[dict]) -> list[tuple[int, str, str]]:
...
def _execute_dashboard_queries(datasource_uid: str, sql_targets: list[tuple[int, str, str]]) -> tuple[list[str], int]:
    failures: list[str] = []
    total_rows = 0
    for idx, (panel_id, title, raw_sql) in enumerate(sql_targets, start=1):
        ...
        query_payload = {
            'queries': [
                {
                    'refId': ref_id,
                    'datasource': {'uid': datasource_uid, 'type': 'grafana-postgresql-datasource'},
                    'rawSql': raw_sql,
                    'format': 'table',
                }
            ],
