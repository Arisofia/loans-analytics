...
def _dashboard_candidates(frontend_base_url: str) -> list[str]:
    ...
    return [base, f'{base}/Portfolio_Dashboard', f'{base}/3_Portfolio_Dashboard', f'{base}/?page=Portfolio_Dashboard']
...
def _open_dashboard_with_uploader(page, frontend_base_url: str) -> bool:
    ...
    for url in _dashboard_candidates(frontend_base_url):
        ...
        try:
            page.get_by_test_id('stFileUploaderDropzone').locator('input').first.wait_for(timeout=20000)
            return True
...
@pytest.mark.skipif(not _frontend_ready(), reason='Frontend dashboard is not ready')
def test_csv_upload_kpi_agent_flow(frontend_base_url: str, csv_path: Path, clean_agent_outputs: dict[str, set[Path]], agent_outputs_dir: Path):
    with sync_playwright() as p:
        ...
        if not _open_dashboard_with_uploader(page, frontend_base_url):
            browser.close()
            return
        page.get_by_test_id('stFileUploaderDropzone').locator('input').first.set_input_files(str(csv_path), timeout=60000)
        page.get_by_text(re.compile('Data uploaded and validated successfully', re.IGNORECASE)).first.wait_for(timeout=180000)
        page.get_by_text(re.compile('Key Portfolio Metrics', re.IGNORECASE)).first.wait_for(timeout=120000)
        page.get_by_text(re.compile('Total Portfolio Value', re.IGNORECASE)).first.wait_for(timeout=120000)
        page.get_by_role('button', name=re.compile('Run Agent Analysis', re.IGNORECASE)).click()
        page.get_by_text(re.compile('AI Analysis Results', re.IGNORECASE)).first.wait_for(timeout=240000)
    ...
    if expect_fallback:
        for path, payload in latest_payloads:
            assert payload.get('analysis_mode') in {'local_fallback', 'local_fallback_after_error'}, f"Expected fallback mode in {path}, got {payload.get('analysis_mode')}"
