...
def mock_openai_client():
    ...
    client.chat.completions.create.return_value = Mock(
        choices=[Mock(message=Mock(content="Mocked LLM response"))]
    )
    return client
...
def sample_agent_message():
    """Sample message for agent testing."""
    return [
...
def agent_execution_context():
    """Common execution context for agents."""
    return {
...
def performance_metrics():
    """Fixture for tracking performance metrics in tests."""
    metrics = {
        "execution_times": [],
        "token_counts": [],
        "api_calls": [],
