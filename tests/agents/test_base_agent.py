...
class TestBaseAgent:
    ...
    def test_agent_execute_method(self, agent_execution_context):
        agent = MagicMock()
        agent.execute.return_value = {'status': 'success', 'result': 'Task completed'}
    ...
    def test_agent_handle_error_method(self):
        agent = MagicMock()
        agent.handle_error.return_value = {'error_handled': True, 'recovery_action': 'retry'}
    ...
    def test_agent_metrics_collection(self, performance_metrics):
        ...
        agent = MagicMock()
        agent.collect_metrics.return_value = {'execution_time': 100, 'tokens_used': 50}
