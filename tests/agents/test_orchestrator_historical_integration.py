...
class TestOrchestratorHistoricalIntegration(unittest.TestCase):
    ...
    def test_auto_enriches_and_injects_historical_context(self, mock_init_client):
        ...
        provider = MagicMock()
        provider.agent_summary.return_value = {'kpi_id': 'par_30', 'trend': {'direction': 'decreasing', 'strength': 'moderate'}, 'moving_average_30d': 2.4, 'recent_change_point': None}
        ...
        def make_process(role: AgentRole):
            ...
            def _process(request):
                ...
                return self._response(role=role, trace_id=request.trace_id, content=f'{role.value} response')
            return _process
        for role, agent in orchestrator.agents.items():
            agent.process = MagicMock(side_effect=make_process(role))
        orchestrator.run_scenario(scenario_name='loan_risk_review', initial_context={'portfolio_data': 'sample portfolio', 'kpi_id': 'par_30'}, trace_id='trace_hist_auto')
    ...
    def test_preserves_explicit_historical_context(self, mock_init_client):
        ...
        provider = MagicMock()
        orchestrator = MultiAgentOrchestrator(historical_context_provider=provider)
        explicit_historical_context = {'kpi_id': 'par_30', 'trend': {'direction': 'stable', 'strength': 'weak'}}
        ...
        for role, agent in orchestrator.agents.items():
            agent.process = MagicMock(side_effect=make_process(role))
        orchestrator.run_scenario(scenario_name='loan_risk_review', initial_context={'portfolio_data': 'sample portfolio', 'historical_context': explicit_historical_context, 'historical_trends': explicit_historical_trends}, trace_id='trace_hist_explicit')
