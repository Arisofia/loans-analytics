...
class TestTimeoutScenarios:
    ...
    async def test_timeout_fallback_mechanism(self):
        agent = MagicMock()
        agent.execute_with_fallback.return_value = {'status': 'timeout', 'fallback_used': True, 'result': 'default_result'}
    ...
    async def test_long_running_analysis(self):
        agent = MagicMock()
        agent.long_analysis.return_value = {'status': 'success', 'duration_seconds': 90}
