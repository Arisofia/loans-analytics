"""
Test suite for the zero-cost pipeline extensions:

  Test 1 — ETL / normalization
    - LoanTapeLoader produces dim_loan / fact_schedule / fact_real_payment
      with loan_id as the primary key.
    - ControlMoraAdapter produces dim_loan-compatible schema.
    - Both adapters produce the same canonical column set.
    - Crosswalk generates exports/unmatched_records.csv when mapping fails,
      with a non-empty reason_code.

  Test 2 — XIRR (robust)
    - Known cash-flow case validates numerical result.
    - Edge cases: all-negative / all-positive flows raise ValueError.
    - Irregular date spacing is handled correctly.
    - loan_xirr and portfolio_xirr return Series indexed by loan_id.

  Test 3 — ETL pipeline integration
    - PipelineRouter returns "loan_tape" for months <= Jan 2026.
    - PipelineRouter returns "control_mora" for months >= Feb 2026.
    - Exporter writes exports/*.csv and exports/*.parquet.
    - DPDCalculator computes correct DPD from synthetic schedule/payment data.
"""
...
# =============================================================================
# Test 1 — ETL / normalisation
# =============================================================================


class TestLoanTapeLoader:
    """LoanTapeLoader produces canonical dim_loan / fact_schedule / fact_real_payment."""

    def _make_loan_csv(self, tmp_path: Path) -> Path:
        content = (
            "Loan ID,Disbursement Date,Disbursement Amount,Interest Rate,"
            "Term Months,Currency,Product Type,Branch Code,Customer ID\n"
    ...
    def _make_schedule_csv(self, tmp_path: Path) -> Path:
        content = (
            "Loan ID,Payment Date,Principal,Interest,Fee,Total,Installment Number\n"
    ...
    def _make_payment_csv(self, tmp_path: Path) -> Path:
        content = (
            "Loan ID,True Payment Date,True Total,True Principal Payment,"
            "True Interest Payment,True Fee Payment\n"
    ...
    def test_control_mora_and_loan_tape_share_loan_id_column(self, tmp_path):
        ...
        # Both should have 'loan_id' equivalent
        assert "loan_id" in tape_tables["dim_loan"].columns
        # Control-de-Mora uses lend_id; pipeline_router aliases it
        assert "lend_id" in mora_df.columns or "loan_id" in mora_df.columns


class TestCrosswalk:
    """Crosswalk generates unmatched_records.csv with reason_code populated."""
    ...
    def _make_mora_df(self) -> pd.DataFrame:
        return pd.DataFrame(
            {
                "lend_id": ["L-001", "L-002"],
                "numero_desembolso": ["NDE-001", "NDE-002"],
                "client_name": ["Juan Pérez", "Maria Lopez G."],
                "disbursement_date": pd.to_datetime(["2025-01-15", "2025-03-01"]),
            }
        )
...
class TestXIRR:
    """XIRR function with known cases and edge cases."""
    ...
    def test_simple_known_case(self):
        ...
        assert abs(rate - 0.20) < 0.001, f"Expected ~0.20, got {rate}"
    ...
    def test_loan_xirr_returns_float(self):
        ...
        pays = pd.DataFrame(
            {
                "loan_id": ["L-001", "L-001", "L-001"],
                "payment_date": pd.to_datetime(["2025-04-01", "2025-07-01", "2026-01-01"]),
                "paid_total": [3000.0, 3000.0, 5000.0],
            }
        )
    ...
    def test_portfolio_xirr_returns_series(self):
        ...
        disb = pd.DataFrame(
            {
                "loan_id": ["L-001", "L-002"],
                "disbursement_date": pd.to_datetime(["2025-01-01", "2025-06-01"]),
                "original_principal": [10000.0, 5000.0],
            }
        )
        pays = pd.DataFrame(
            {
                "loan_id": ["L-001", "L-001", "L-002"],
                "payment_date": pd.to_datetime(["2025-07-01", "2026-01-01", "2026-06-01"]),
                "paid_total": [5000.0, 6000.0, 6000.0],
            }
        )
...
class TestExporter:
    """Exporter writes CSV and Parquet files to exports/."""

    def _sample_df(self) -> pd.DataFrame:
        return pd.DataFrame(
            {
                "loan_id": ["L-001", "L-002"],
                "snapshot_month": pd.to_datetime(["2026-01-31", "2026-01-31"]),
                "outstanding_principal": [9000.0, 18000.0],
            }
        )
    ...
    def test_export_tables_writes_multiple_files(self, tmp_path):
        ...
        exp = Exporter(output_dir=tmp_path)
        tables = {
            "dim_loan": self._sample_df(),
            "fact_schedule": self._sample_df(),
            "_source": "loan_tape",  # should be skipped
        }
        ...
        unmatched = pd.DataFrame(
            {
                ...
                "reason_code": ["unmatched_loan_tape"],
            }
        )
...
class TestDPDCalculator:
    """DPDCalculator computes correct DPD from base data."""
    ...
    def _make_inputs(self):
        ...
        # L-001: 2 installments of 1000 each, due Jan and Feb
        # L-002: 1 installment of 1000, due Jan
        ...
        # L-001 paid Jan only; L-002 paid in full
        fact_real_payment = pd.DataFrame(
            {
                "loan_id": ["L-001", "L-002"],
                "payment_date": pd.to_datetime(["2026-01-31", "2026-01-31"]),
                "paid_principal": [1000.0, 1000.0],
            }
        )
    ...
    def test_build_snapshots_returns_dataframe(self):
        ...
        assert len(snap) == 2  # 2 loans × 1 month
    ...
    def test_overdue_loan_has_positive_dpd(self):
        ...
        calc = DPDCalculator()
        dim_loan, sched, pays = self._make_inputs()
        # Use a date 15 days after the Feb 28 due date → DPD should be 15
    ...
    def test_par_flags_set_correctly(self):
        ...
        dim_loan, sched, pays = self._make_inputs()
        # Mar 15 → L-001 is 15 days overdue (PAR-1), L-002 is current
        snap = calc.build_snapshots(dim_loan, sched, pays, ["2026-03-15"])
        l1_row = snap[snap["loan_id"] == "L-001"].iloc[0]
        l2_row = snap[snap["loan_id"] == "L-002"].iloc[0]
