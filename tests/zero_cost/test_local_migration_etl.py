...
import pandas as pd
import pytest
from backend.src.zero_cost.local_migration_etl import MAX_NOT_SPECIFIED_LOG_ROWS, build_not_specified_log, reconcile_payments
from backend.src.zero_cost.xirr import xirr

def test_xirr_known_case_precision():
    cashflows = [-1500.0, 200.0, 350.0, 450.0, 700.0]
    dates = ['2025-01-01', '2025-03-10', '2025-06-01', '2025-09-15', '2026-01-01']
    rate = xirr(cashflows, dates)
    assert math.isfinite(rate)
    assert rate == pytest.approx(0.1965354, rel=0.001)
...
def test_reconcile_payments_marks_missing_and_mismatch_reason_codes():
    fact_schedule = pd.DataFrame({'loan_id': ['L1', 'L1', 'L2'], 'scheduled_date': ['2025-02-01', '2025-03-01', '2025-02-15'], 'scheduled_total': [100.0, 100.0, 75.0]})
    fact_real_payment = pd.DataFrame({'loan_id': ['L1', 'L2', 'L3'], 'payment_date': ['2025-02-01', '2025-02-15', '2025-02-20'], 'paid_total': [100.0, 70.0, 50.0]})
    reconciliation, unmatched = reconcile_payments(fact_schedule, fact_real_payment)
    assert 'status' in reconciliation.columns
    assert set(unmatched['reason_code']) == {'scheduled_without_payment', 'amount_mismatch', 'payment_without_schedule'}
...
def test_reconcile_payments_captures_null_loan_id():
    fact_schedule = pd.DataFrame({'loan_id': ['L1', None], 'scheduled_date': ['2025-02-01', '2025-02-01'], 'scheduled_total': [100.0, 50.0]})
    fact_real_payment = pd.DataFrame({'loan_id': ['L1', None], 'payment_date': ['2025-02-01', '2025-02-01'], 'paid_total': [100.0, 50.0]})
...
def test_reconcile_payments_captures_invalid_dates_no_duplicates():
    fact_schedule = pd.DataFrame({'loan_id': ['L1', None, 'L2'], 'scheduled_date': ['not-a-date', 'not-a-date', '2025-02-01'], 'scheduled_total': [100.0, 50.0, 75.0]})
    fact_real_payment = pd.DataFrame({'loan_id': ['L1', 'L2'], 'payment_date': ['2025-02-01', 'not-a-date'], 'paid_total': [100.0, 75.0]})
    _, unmatched = reconcile_payments(fact_schedule, fact_real_payment)
    ...
    null_loan_invalid_sched = unmatched[unmatched['loan_id'].isna() & (unmatched['reason_code'] == 'invalid_scheduled_date')]
    assert len(null_loan_invalid_sched) == 1, 'Overlapping row (null loan_id + invalid date) should be logged exactly once under invalid_scheduled_date'
    null_loan_missing_sched = unmatched[unmatched['loan_id'].isna() & (unmatched['reason_code'] == 'missing_loan_id_schedule')]
    assert len(null_loan_missing_sched) == 0, 'Overlapping row should NOT be double-logged under missing_loan_id_schedule'
