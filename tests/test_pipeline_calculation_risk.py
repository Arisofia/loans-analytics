...
def test_ltv_sintetico_zero_adjusted_value_is_opaque_and_nan():
    df = pd.DataFrame(
        {"capital_desembolsado": [50.0], "valor_nominal_factura": [100.0], "tasa_dilucion": [1.0]}
    )
...
def test_ltv_sintetico_missing_denominator_is_opaque_and_nan():
    df = pd.DataFrame(
        {"capital_desembolsado": [50.0], "valor_nominal_factura": [None], "tasa_dilucion": [0.1]}
    )
    result = CalculationPhase._calculate_ltv_sintetico(df)
    assert pd.isna(result.iloc[0])
    assert df.loc[0, "ltv_sintetico_is_opaque"] == 1
    ...
    df = pd.DataFrame(
        {
            ...
            "valor_nominal_factura": [100.0, -50.0],
            "tasa_dilucion": [1.1, 0.1],
        }
    )
    engine = KPIEngineV2.__new__(KPIEngineV2)
    result = engine._calculate_ltv_sintetico(df)
...
def test_ltv_sintetico_valid_row_not_nan():
    df = pd.DataFrame(
        {"capital_desembolsado": [100.0], "valor_nominal_factura": [200.0], "tasa_dilucion": [0.1]}
    )
    result = CalculationPhase._calculate_ltv_sintetico(df)
...
def test_ltv_sintetico_zero_nominal_returns_nan():
    df = pd.DataFrame(
        {"capital_desembolsado": [50.0], "valor_nominal_factura": [0.0], "tasa_dilucion": [0.0]}
    )
    result = CalculationPhase._calculate_ltv_sintetico(df)
    assert pd.isna(result.iloc[0])
    ...
    df = pd.DataFrame(
        {
            ...
            "status": ["defaulted", "active", "defaulted", "defaulted", "active", "active"],
        }
    )
    ...
    df = pd.DataFrame(
        {
            ...
            "status": ["defaulted", "active", "closed", "defaulted", "active", "closed"],
        }
    )
    result = engine._compute_portfolio_velocity_of_default(df)
    assert result is not None
...
def test_compute_portfolio_velocity_of_default_returns_none_single_period():
    engine = KPIEngineV2.__new__(KPIEngineV2)
    df = pd.DataFrame(
        {"as_of_date": ["2025-01-01", "2025-01-15"], "status": ["active", "defaulted"]}
    )
    assert engine._compute_portfolio_velocity_of_default(df) is None
...
def test_vd_units_percentage_points():
    df = pd.DataFrame(
        {"as_of_date": ["2025-01-01", "2025-02-01"], "status": ["defaulted", "active"]}
    )
    engine = KPIEngineV2.__new__(KPIEngineV2)
    result = engine._compute_portfolio_velocity_of_default(df)
    assert float(result) == -100.0
...
def test_vd_handles_mixed_date_formats():
    df = pd.DataFrame(
        {"as_of_date": ["2025-01-01", "02/01/2025"], "status": ["active", "defaulted"]}
    )
    engine = KPIEngineV2.__new__(KPIEngineV2)
    result = engine._compute_portfolio_velocity_of_default(df)
    assert float(result) == 100.0
...
def test_velocity_of_default_unsorted_input_normalized_by_period_col():
    df_ts = pd.DataFrame(
        {"period": ["2025-03", "2025-01", "2025-02"], "default_rate": [2.0, 1.0, 2.5]}
    )
...
def test_velocity_of_default_no_period_col_preserves_row_order():
    df_ts = pd.DataFrame(
        {"period": ["2025-03", "2025-02", "2025-01"], "default_rate": [2.0, 2.5, 1.0]}
    )
    vd = CalculationPhase._calculate_velocity_of_default(df_ts)
    assert pd.isna(vd.iloc[0])
    ...
    df = pd.DataFrame(
        {
            ...
            "status": ["active", "defaulted", "defaulted", "active", "active", "defaulted"],
        }
    )
...
def test_compute_portfolio_velocity_of_default_units_are_percentage_points_v2():
    phase = CalculationPhase.__new__(CalculationPhase)
    df = pd.DataFrame(
        {"as_of_date": ["2025-01-01", "2025-02-01"], "status": ["active", "defaulted"]}
    )
    result = phase._compute_portfolio_velocity_of_default(df)
    assert result is not None
...
def test_par_metrics_uses_canonical_dpd_adjusted():
    grp = pd.DataFrame(
        {"outstanding_balance": [100.0, 200.0], "dpd": [0.0, 0.0], "dpd_adjusted": [0.0, 90.0]}
    )
    result = CalculationPhase._calculate_segment_par_metrics(
...
def test_dimension_segment_kpis_are_sorted_deterministically():
    ...
    result = phase._calculate_dimension_segment_kpis(
        work, dim="company", balance_col="outstanding_balance", dpd_col="dpd", status_col="status"
    )
...
def test_calculate_ltv_result_matches_pure_decimal_division():
    ...
    expected = (loan / collateral * Decimal("100")).quantize(
