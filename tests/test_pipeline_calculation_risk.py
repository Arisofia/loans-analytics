...
def test_ltv_sintetico_zero_adjusted_value_returns_nan():
    """When the adjusted invoice value is zero, LTV should be np.nan (epistemic opacity).

    Returning 0.0 would encode a false low-risk value; np.nan signals that the
    loan's LTV cannot be computed and must not silently enter portfolio aggregations.
    """
    df = pd.DataFrame(
        {
    ...
    result = CalculationPhase._calculate_ltv_sintetico(df)
    assert pd.isna(result.iloc[0]), "Fully-diluted invoice must yield np.nan, not a numeric value"
