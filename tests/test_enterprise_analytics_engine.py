...
"""
Unit tests for enterprise analytics engine KPIs and risk calculations.
"""
...
def test_monthly_payment_matches_finance_formula():
    """
    Test monthly payment calculation matches standard finance formula.
    """
    loan = LoanPosition(
        principal=100_000, annual_interest_rate=0.12, term_months=36
    )
    payment = calculate_monthly_payment(loan)

    monthly_rate = loan.annual_interest_rate / 12
    expected_payment = (
        (monthly_rate * loan.principal)
...
def test_portfolio_interest_and_risk_tracks_defaults():
    """
    Test portfolio interest and risk calculations track loan defaults.
    """
    prime = LoanPosition(
        principal=50_000,
...
def test_invalid_inputs_raise_value_errors():
    with pytest.raises(ValueError):
        LoanPosition(
            principal=0, annual_interest_rate=0.05, term_months=12
        )

    with pytest.raises(ValueError):
        LoanPosition(
            principal=10_000, annual_interest_rate=-0.01, term_months=12
        )

    with pytest.raises(ValueError):
        LoanPosition(
            principal=10_000, annual_interest_rate=0.05, term_months=0
        )
...
def test_portfolio_kpis_surfaces_weighted_metrics():
    loans = [
        LoanPosition(
            principal=100_000, annual_interest_rate=0.09, term_months=24,
            default_probability=0.02
        ),
        LoanPosition(
            principal=50_000, annual_interest_rate=0.12, term_months=36,
    ...
    expected_loss_value = sum(
        expected_loss(loan, 0.4) for loan in loans
    )
    ...
    assert kpis.expected_monthly_payment == pytest.approx(
        calculate_monthly_payment(loans[0])
        + calculate_monthly_payment(loans[1])
    )
