...
def sample_kpi_response():
    return {'kpis': [{'id': 'avg_apr', 'name': 'Average APR', 'value': 12.5, 'unit': '%', 'threshold_status': 'normal', 'thresholds': {'warning': 14.0, 'critical': 16.0}, 'updated_at': '2025-02-26T10:00:00Z'}, {'id': 'par_30', 'name': 'PAR 30', 'value': 8.2, 'unit': '%', 'threshold_status': 'critical', 'thresholds': {'warning': 5.0, 'critical': 7.0}, 'updated_at': '2025-02-26T10:00:00Z'}, {'id': 'cac_usd', 'name': 'CAC', 'value': 45.0, 'unit': 'USD', 'threshold_status': 'not_configured', 'thresholds': {}, 'updated_at': '2025-02-26T10:00:00Z'}], 'timestamp': '2025-02-26T10:00:00Z', 'metrics_published': True}
...
class TestKPIAPIClient:
    ...
    def test_get_latest_kpis_success(self, mock_httpx, api_client, sample_kpi_response):
        mock_response = Mock()
    ...
    @patch('frontend.streamlit_app.kpi_api_client.httpx')
    def test_get_latest_kpis_with_filter(self, mock_httpx, api_client, sample_kpi_response):
        mock_response = Mock()
        mock_response.json.return_value = sample_kpi_response
        mock_response.raise_for_status = Mock()
        mock_client = Mock()
        mock_client.get.return_value = mock_response
        api_client._httpx_client = mock_client
        api_client.get_latest_kpis(kpi_keys=['avg_apr', 'par_30'], portfolio_id='test-portfolio')
    ...
    @patch('frontend.streamlit_app.kpi_api_client.httpx')
    def test_cache_expiration(self, mock_httpx, api_client, sample_kpi_response):
        mock_response = Mock()
        mock_response.json.return_value = sample_kpi_response
        mock_response.raise_for_status = Mock()
        mock_client = Mock()
    ...
    def test_get_single_kpi(self, mock_httpx, api_client):
        kpi_response = {'id': 'avg_apr', 'name': 'Average APR', 'value': 12.5, 'unit': '%', 'threshold_status': 'normal', 'thresholds': {'warning': 14.0, 'critical': 16.0}, 'updated_at': '2025-02-26T10:00:00Z'}
        mock_response = Mock()
...
class TestKPIResponse:
    ...
    def test_status_checks_normal(self):
        ...
        kpi = KPIResponse(id='test', name='Test KPI', value=50.0, unit='unit', threshold_status='normal')
        assert kpi.is_normal() is True
    ...
    def test_status_checks_critical(self):
        from frontend.streamlit_app.kpi_api_client import KPIResponse
        kpi = KPIResponse(id='test', name='Test KPI', value=99.0, unit='unit', threshold_status='critical')
    ...
    def test_status_checks_not_configured(self):
        from frontend.streamlit_app.kpi_api_client import KPIResponse
        kpi = KPIResponse(id='test', name='Test KPI', value=50.0, unit='unit', threshold_status='not_configured')
