...
def sample_df() -> pd.DataFrame:
    return pd.DataFrame({'loan_amount': [12000, 8000, 16000], 'appraised_value': [15000, 10000, 20000], 'borrower_income': [60000, 45000, 80000], 'monthly_debt': [500, 400, 300], 'principal_balance': [10000, 5000, 15000], 'interest_rate': [0.05, 0.07, 0.06], 'loan_status': ['current', 'delinquent', 'current']})
...
class TestMYPEHighRiskClassification:
    ...
    def test_at_thresholds_triggers_flags(self):
        metrics = {'dpd': MYPEBusinessRules.NPL_DAYS_THRESHOLD, 'utilization': MYPEBusinessRules.HIGH_RISK_CRITERIA['utilization'], 'npl_ratio': MYPEBusinessRules.HIGH_RISK_CRITERIA['npl_ratio'], 'collection_rate': MYPEBusinessRules.HIGH_RISK_CRITERIA['collection_rate'] - 0.01}
        is_high, reasons = MYPEBusinessRules.classify_high_risk(metrics)
        assert is_high is True
        assert len(reasons) >= 3
...
class TestMYPERotation:
    ...
    def test_meets_target(self):
        ...
        assert pytest.approx(rotation, rel=0.001) == 5
    ...
    def test_boundary_condition(self):
        rotation, meets, message = MYPEBusinessRules.check_rotation_target(MYPEBusinessRules.TARGET_ROTATION * 100000, 100000)
        assert pytest.approx(rotation, rel=0.001) == MYPEBusinessRules.TARGET_ROTATION
        assert meets is True
...
class TestMYPEFacilityApproval:
    ...
    def test_approves_low_risk(self):
        metrics = {'dpd': 5, 'utilization': 0.35, 'npl_ratio': 0.01, 'collection_rate': 0.97, 'revenue': 500000, 'avg_balance': 80000, 'industry': IndustryType.MANUFACTURING}
    ...
    def test_reduces_recommendation_when_rotation_low(self):
        metrics = {'dpd': 10, 'utilization': 0.4, 'npl_ratio': 0.02, 'collection_rate': 0.95, 'revenue': 150000, 'avg_balance': 60000, 'industry': IndustryType.SERVICES}
        dec = MYPEBusinessRules.evaluate_facility_approval(150000, metrics, 50000)
        assert dec.approved is False
        assert any(('Rotation' in r for r in dec.reasons))
        assert dec.recommended_amount == pytest.approx(120000, rel=0.001)
...
class TestSettings2026Targets:
    ...
    def test_jan_target(self):
        assert settings.portfolio_targets_2026['2026-01'] == 8500000
    ...
    def test_required_growth_achievable(self):
        ...
        assert required_net <= 500000, 'Monthly net growth requirement is unrealistically high'
    ...
    def test_loader_ignores_helper_keys_and_parses_underscore_values(self, tmp_path: Path):
        ...
        assert loaded.portfolio_targets_2026 == {'2026-01': 8500000, '2026-12': 12000000}
...
class TestCalculateQualityScore:
    ...
    def test_all_null_returns_zero(self, sample_df):
        # pandas 2.x raises TypeError when assigning None to integer columns via
        # df[:] = None; use per-column np.nan assignment to safely coerce int64 → float64.
        df = sample_df.copy()
...
class TestProjectGrowth:
    ...
    def test_formats_month_labels(self):
        ...
        assert proj['month'].str.match('^[A-Z][a-z]{2} \\d{4}$').all()
