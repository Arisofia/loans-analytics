"""Hypothesis tests for guardrails input sanitization."""
...
def test_sanitize_for_logging_neutralizes_control_chars(user_input: str) -> None:
    """Sanitized log values must not contain raw control characters."""
...
@given(
    local=st.text(alphabet=st.characters(min_codepoint=97, max_codepoint=122), min_size=1),
    domain=st.text(alphabet=st.characters(min_codepoint=97, max_codepoint=122), min_size=1),
)
def test_redact_pii_replaces_emails(local: str, domain: str) -> None:
    """Generated email-like strings should be redacted."""
