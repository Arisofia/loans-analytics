import pandas as pd
import pytest
from python.validation import validate_dataframe, validate_numeric_bounds, find_column, REQUIRED_ANALYTICS_COLUMNS, ANALYTICS_NUMERIC_COLUMNS, NUMERIC_COLUMNS
...
def test_validate_dataframe_missing_multiple_columns():
    # Coderabbit: Test that multiple missing columns raise ValueError with plural message
...
def test_validate_percentage_bounds():
    ...
    results = validate_percentage_bounds(df, columns=["valid_pct", "invalid_pct", "rate", "bad_rate"])
...
def test_validate_iso8601_dates():
    ...
    results = validate_iso8601_dates(df, columns=["good_date", "bad_date", "iso_datetime", "mixed", "nulls"])
