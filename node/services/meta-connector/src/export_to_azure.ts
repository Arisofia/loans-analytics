...
async function exportMetaDataToAzure() {
  ...
  const insightsRaw = await fetchInsights(
    client,
    META_AD_ACCOUNT_ID,
    today,
    today
  ...
  // Upload campaigns
  const campaignsBlob = containerClient.getBlockBlobClient(
    `campaigns_${today}.json`
  )
  await campaignsBlob.upload(
    JSON.stringify(campaigns, null, 2),
    Buffer.byteLength(JSON.stringify(campaigns))
  )
  // Upload insights
  const insightsBlob = containerClient.getBlockBlobClient(
    `insights_${today}.json`
  )
  await insightsBlob.upload(
