...
MIGRATION_PATTERN = re.compile('^(\\d{2,14})_[a-z0-9_]+\\.sql$')
INIT_MIGRATION_PATTERN = re.compile('^000?_init')
...
def validate_migration_format(migrations: List[str]) -> Tuple[bool, List[str]]:
    ...
    for migration in migrations:
        if not MIGRATION_PATTERN.match(migration):
            errors.append(f'  Invalid migration name format: {migration}\n    Expected format: NN_description.sql or YYYYMMDDhhmmss_description.sql')
...
def validate_init_first(migrations: List[str]) -> Tuple[bool, List[str]]:
    ...
    if first_migration and (not INIT_MIGRATION_PATTERN.match(first_migration)):
        errors.append(f'  Init migration must execute first.\n    Expected first: {init_migrations[0]}\n    Found first: {first_migration}')
    return (len(errors) == 0, errors)
...
def main():
    ...
    all_valid = True
    all_errors = []
    validations = [('Migration naming convention', validate_migration_format), ('Monotonic ordering', validate_monotonic_order), ('No duplicate migrations', validate_no_duplicates), ('Init migration first', validate_init_first)]
