#!/usr/bin/env python3
"""Validate local secrets and check live external connectivity."""
...
SUPABASE_OPTIONAL = {
    "SUPABASE_PROJECT_REF": "Project ref slug in the Supabase URL",
    ...
    "SUPABASE_HISTORICAL_KPI_TABLE": "Table override, default historical_kpis",
}
...
GOOGLE_SHEETS = {
    ...
    "GOOGLE_SHEETS_SPREADSHEET_ID": "Google Sheets share URL -> spreadsheet ID",
}

SMTP_ALERTS = {
    "SMTP_HOST": "SMTP server, for example smtp.gmail.com:587",
...
API_CONFIG = {
    "ABACO_API_KEY": "Internal API authentication secret",
    "API_JWT_SECRET": "JWT secret when API_JWT_ENABLED=1",
    "SENTRY_DSN": "Sentry DSN",
}

GITHUB_CI_SECRETS = {
    "SUPABASE_URL": "tests.yml uses secrets.SUPABASE_URL",
    "SUPABASE_ANON_KEY": "tests.yml uses secrets.SUPABASE_ANON_KEY",
    "SUPABASE_SERVICE_ROLE_KEY": "Needed by backend server-side operations",
    "OPENAI_API_KEY": "tests.yml and pr-checks.yml use secrets.OPENAI_API_KEY",
...
def main() -> None:
    ...
    groups = [
        ("SECTION 1 - Core Supabase credentials", SUPABASE_REQUIRED, True),
        ("SECTION 2 - Extended Supabase settings", SUPABASE_OPTIONAL, False),
        ("SECTION 3 - Required LLM keys", LLM_REQUIRED, True),
        ("SECTION 3b - Optional LLM keys", LLM_OPTIONAL, False),
        ("SECTION 4 - Google Sheets integration", GOOGLE_SHEETS, False),
        ("SECTION 5 - Monitoring and SMTP alerts", SMTP_ALERTS, False),
        ("SECTION 6 - API and observability keys", API_CONFIG, False),
    ]
