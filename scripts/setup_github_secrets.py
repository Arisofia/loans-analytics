...
def bootstrap_google_sheets_json_secret() -> None:
    if os.environ.get('GOOGLE_SHEETS_CREDENTIALS_JSON'):
        return
    configured_path = os.environ.get('GOOGLE_SHEETS_CREDENTIALS_PATH', '').strip()
    ...
    if not candidate.exists() or not candidate.is_file():
        return
    try:
        os.environ['GOOGLE_SHEETS_CREDENTIALS_JSON'] = candidate.read_text(encoding='utf-8')
    except OSError:
        return
...
def upload_actions_secrets(token: str, dry_run: bool) -> None:
    bootstrap_google_sheets_json_secret()
