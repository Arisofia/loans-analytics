...
def run_pipeline(loan_path: Path, payment_path: Optional[Path]=None, customer_path: Optional[Path]=None, model_dir: Path=Path('models/risk'), threshold_auc: float=0.7, iv_threshold: float=0.02) -> bool:
    ...
    try:
        scorecard.fit(loan_df=loan_df, payment_df=payment_df, customer_df=customer_df, iv_threshold=iv_threshold)
    ...
    metrics = model_helper.train(df=loan_df, payment_df=payment_df, customer_df=customer_df, selected_features=selected_features)
...
def main():
    ...
    success = run_pipeline(loan_path=args.loans, payment_path=args.payments, customer_path=args.customers, model_dir=args.output_dir, threshold_auc=args.threshold, iv_threshold=args.iv_threshold)
