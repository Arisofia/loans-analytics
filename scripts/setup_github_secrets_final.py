...
def api_call(method: str, url: str, token: str, payload: dict=None) -> tuple[int, dict]:
    body = None
    headers = {'Accept': 'application/vnd.github+json', 'Authorization': f'Bearer {token}', 'X-GitHub-Api-Version': '2022-11-28', 'User-Agent': 'abaco-secrets-bootstrap'}
...
def upload_secrets(env_path: Path, token: str) -> None:
    ...
    try:
        git_cmd = os.path.join(os.environ.get('LOCALAPPDATA', ''), 'Programs', 'Git', 'cmd', 'git.exe')
    ...
    match = re.search('github\\.com[:/](?P<owner>[^/]+)/(?P<repo>[^/.]+)(?:\\.git)?$', origin)
    if not match:
    ...
    status, key_data = api_call('GET', f'https://api.github.com/repos/{owner}/{repo}/actions/secrets/public-key', token)
    if status != 200:
        raise RuntimeError(f'Failed to fetch public key: {key_data}')
    public_key_str = key_data['key']
    key_id = key_data['key_id']
    public_key_obj = public.PublicKey(public_key_str.encode('utf-8'), encoding.Base64Encoder())
    ...
    for secret_name in to_upload:
        ...
        try:
            api_call('PUT', f'https://api.github.com/repos/{owner}/{repo}/actions/secrets/{secret_name}', token, {'encrypted_value': encrypted_b64, 'key_id': key_id})
            print(f'  ✓ {secret_name}')
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='Upload secrets to GitHub Actions', formatter_class=argparse.RawDescriptionHelpFormatter)
    parser.add_argument('--pat', default=os.environ.get('GITHUB_PAT'), help='GitHub Personal Access Token (or set GITHUB_PAT env var)')
    parser.add_argument('--env-file', default='.env.local', help='Path to .env.local file (default: .env.local)')
