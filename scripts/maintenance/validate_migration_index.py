"""Validate DB migration ordering and index completeness.

Policy:
- Every SQL migration under db/migrations must be listed exactly once in migration_index.yml.
- migration_index.yml defines the authoritative execution order.
- New migration files must follow either legacy numeric prefix (00_/001_/002_)
  or timestamp prefix (YYYYMMDDHHMMSS_).
"""
...
_ALLOWED_PREFIX = re.compile(r"^(?:\d{14}|\d{8}|\d{2,3})_[a-z0-9_]+\.sql$")
...
def main() -> int:
    ...
    # Validate naming conventions for SQL migration files
    ...
    # Enforce deterministic ordering contract for execution
