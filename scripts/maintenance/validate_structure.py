...
class AbacoValidator:
    ...
    def _load_manifest(self) -> Dict:
        ...
        try:
            ...
            print(
                f"{Colors.RED}CRITICAL ERROR: Failed to parse {self.structure_file}: {e}{Colors.END}"
            )
            sys.exit(1)
    ...
    def _check_path(self, relative_path: str, purpose: str = "N/A") -> bool:
        ...
        if full_path.exists():
            ...
            if self.verbose:
                print(
                    f"{Colors.GREEN}✅{Colors.END} {relative_path} {Colors.CYAN}({purpose}){Colors.END}"
                )
            return True
        else:
            self.missing.append(relative_path)
            print(
                f"{Colors.RED}❌ MISSING:{Colors.END} {relative_path} {Colors.YELLOW}({purpose}){Colors.END}"
            )
            return False
    ...
    def validate(self):
        ...
        for tool in integrations:
            ...
            configs = [
                c.strip() for c in config_str.split(",") if c.strip() and not c.startswith(".env")
            ]
            for cfg in configs:
