...
def _append_required_actions(lines: list[str], results: dict[str, dict[str, Any]]) -> None:
    failed_checks = [result.get('name', key.title()) for key, result in results.items() if not result.get('success', False)]
    if failed_checks:
...
class ServiceStatusChecker:
    ...
    def check_git_status(self) -> dict[str, Any]:
        status: dict[str, Any] = {'name': 'Git Repository', 'success': True, 'details': {}}
        ...
        if success:
            filtered_lines = [line for line in stdout.strip().split('\n') if line and (not any((exclude in line for exclude in [REPORT_MD_NAME, REPORT_JSON_NAME])))]
    ...
    def check_python_environment(self) -> dict[str, Any]:
        status: dict[str, Any] = {'name': 'Python Environment', 'success': True, 'details': {}}
    ...
    def check_tests(self) -> dict[str, Any]:
        status: dict[str, Any] = {'name': 'Test Suite', 'success': False, 'details': {}}
        tests_dir = self.repo_root / 'tests'
        status['details']['tests_directory_exists'] = tests_dir.exists()
        success, stdout, stderr = self.run_command([sys.executable, '-m', 'pytest', '-v', '--tb=short', '--maxfail=5'], timeout=120)
    ...
    def check_linting(self) -> dict[str, Any]:
        status: dict[str, Any] = {'name': 'Code Quality (Linting)', 'success': True, 'details': {}}
        success, stdout, stderr = self.run_command([sys.executable, '-m', 'ruff', 'check', '.', '--quiet'], timeout=60)
                    ...
                    for py_file in search_dir.rglob('*.py'):
                        try:
                            with open(py_file, 'r', encoding='utf-8') as f:
                                todo_count += sum(('TODO:' in line for line in f))
    ...
    def check_supabase_configuration(self) -> dict[str, Any]:
        status: dict[str, Any] = {'name': 'Supabase', 'success': False, 'details': {}}
        ...
        if not supabase_url:
            ...
            return status
        if not supabase_key:
            status['details']['message'] = 'Supabase URL configured but key missing; connectivity not available'
            return status
        status['success'] = True
    ...
    def check_pipeline(self) -> dict[str, Any]:
        status: dict[str, Any] = {'name': 'Data Pipeline', 'success': True, 'details': {}}
    ...
    def check_multi_agent_system(self) -> dict[str, Any]:
        status: dict[str, Any] = {'name': 'Multi-Agent System', 'success': True, 'details': {}}
        ...
        llm_configured = bool(os.getenv('OPENAI_API_KEY') or os.getenv('ANTHROPIC_API_KEY') or os.getenv('GEMINI_API_KEY'))
        status['details']['llm_provider_configured'] = llm_configured
        return status

    def check_documentation(self) -> dict[str, Any]:
        status: dict[str, Any] = {'name': 'Documentation', 'success': True, 'details': {}}
    ...
    def check_ci_cd(self) -> dict[str, Any]:
        status: dict[str, Any] = {'name': 'CI/CD', 'success': True, 'details': {}}
    ...
    def run_all_checks(self) -> dict[str, dict[str, Any]]:
        ...
        checks = [('git', self.check_git_status), ('python', self.check_python_environment), ('tests', self.check_tests), ('linting', self.check_linting), ('supabase', self.check_supabase_configuration), ('pipeline', self.check_pipeline), ('agents', self.check_multi_agent_system), ('docs', self.check_documentation), ('cicd', self.check_ci_cd)]
        for key, check_func in checks:
            try:
                ...
                self.results[key] = {'name': key.title(), 'success': False, 'details': {'error': str(e)}}
...
def generate_markdown_report(results: dict[str, dict[str, Any]]) -> str:
    ...
    passed_checks = sum((r.get('success', False) for r in results.values()))
    lines = ['# Service Status Report', '', f'**Generated:** {timestamp}', f'**Status:** {passed_checks}/{total_checks} checks passed', '', '## Executive Summary', '']
    ...
    lines.extend(['---', '', '## About This Report', '', 'This report is automatically generated by `scripts/maintenance/generate_service_status_report.py`.', 'It performs comprehensive health checks across all system components and continues', 'checking even if individual components fail, ensuring complete visibility into', 'system status.', '', 'To regenerate this report:', '```bash', 'python scripts/maintenance/generate_service_status_report.py', '```', '', f'*Last updated: {timestamp}*'])
    return '\n'.join(lines)
...
def main() -> None:
    ...
    try:
        ...
        print()
        total = len(results)
        passed = sum((r.get('success', False) for r in results.values()))
        print('=' * 70)
        ...
        try:
            json_file = checker.repo_root / REPORT_JSON_NAME
            with open(json_file, 'w', encoding='utf-8') as f:
                json.dump({'timestamp': datetime.now(timezone.utc).isoformat(), 'results': {}, 'error': error_message}, f, indent=2)
        except Exception as artifact_exc:
            print(f'Warning: failed to write error artifact JSON: {artifact_exc}', file=sys.stderr)
    finally:
        sys.exit(0)
