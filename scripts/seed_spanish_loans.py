...
SPANISH_FIRST_NAMES = [
    ...
    "Carlos", "Miguel", "Alejandro", "Pedro", "Jesús", "Rafael", "Pablo", "Sergio",
    "Fernando", "Ángel", "Luis", "Jorge", "Alberto", "Raúl", "Roberto", "Andrés",
    # Female names
    "María", "Carmen", "Dolores", "Isabel", "Ana", "Josefa", "Francisca", "Pilar",
    "Teresa", "Rosa", "Antonia", "Lucía", "Marta", "Elena", "Laura", "Sara",
    "Cristina", "Patricia", "Beatriz", "Natalia", "Silvia", "Raquel", "Mónica", "Eva"
]
...
SPANISH_SURNAMES = [
    ...
    "Molina", "Castro", "Ortega", "Delgado", "Ortiz", "Morales", "Iglesias", "Santos",
    "Guerrero", "Medina", "Garrido", "Cortés", "Castillo", "Rubio", "Moya", "Pardo"
]
...
def generate_email(name: str, surname: str) -> str:
    ...
    surname_clean = surname.lower().replace('á', 'a').replace('é', 'e').replace('í', 'i').replace('ó', 'o').replace('ú', 'u').replace('ñ', 'n')
    ...
    if status == "current":
        ...
        for i in range(payments_made):
            ...
            history.append({
                "payment_number": i + 1,
                "due_date": current_date.strftime("%Y-%m-%d"),
                "paid_date": (current_date + timedelta(days=delay_days)).strftime("%Y-%m-%d") if delay_days < 180 else None,
                "amount_due": round(monthly_payment, 2),
                "amount_paid": round(monthly_payment * random.uniform(0.3, 1.0), 2) if delay_days < 180 else 0,
                "days_late": delay_days,
                "status": "paid" if delay_days < 30 else "late_paid" if delay_days < 180 else "defaulted"
            })
...
def generate_loan_record(loan_id: int) -> dict[str, Any]:
    ...
    if monthly_rate > 0:
        monthly_payment = principal_amount * (monthly_rate * (1 + monthly_rate) ** term_months) / \
                         ((1 + monthly_rate) ** term_months - 1)
    else:
        monthly_payment = principal_amount / term_months
    
    # Generate payment history
    payment_history = generate_payment_history(status, term_months, origination_date, monthly_payment)
...
def main():
    ...
    # Generate dataset
    records = generate_loan_dataset(num_records=850)  # Generate 850 to have 800+ after any filtering
