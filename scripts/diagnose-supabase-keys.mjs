...
console.log(
  "║       SUPABASE CONNECTION & KEY DIAGNOSTICS                    ║",
);
console.log(
  "╚════════════════════════════════════════════════════════════════╝\n",
);
...
function validateJWT(key) {
  ...
  if (parts.length !== 3) {
    return {
      valid: false,
      error: `Invalid JWT format (${parts.length} parts, expected 3)`,
    };
  }
  ...
  if (key.includes("_KEY") && value) {
    ...
    if (validation.valid) {
      ...
      if (validation.payload.exp) {
        ...
        console.log(
          `  Expires: ${validation.payload.exp} ${expired ? "⚠️  EXPIRED" : "✓"}`,
        );
      }
    } else {
    ...
    try {
      ...
      if (error) {
        if (error.code === "42P01") {
          ...
          console.log(
            `⚠️  Unexpected error: ${error.message} (code: ${error.code})`,
          );
        }
      } else {
  ...
  try {
    ...
    try {
      ...
      if (error) {
        ...
        console.log(
          `✓ Service role bypassed RLS: ${data?.length || 0} rows returned`,
        );
      }
    } catch (err) {
...
console.log(
  "  SUPABASE_DB_URL: PostgreSQL connection string with pooler suffix\n",
);
...
if (requiredVars.SUPABASE_ANON_KEY) {
  ...
  if (anonValidation.valid && anonValidation.payload.role !== "anon") {
    console.log(
      `⚠️  WARNING: SUPABASE_ANON_KEY has role="${anonValidation.payload.role}" (expected "anon")`,
    );
  }
}
...
if (requiredVars.SUPABASE_SERVICE_ROLE_KEY) {
  ...
  if (
    ...
    serviceValidation.payload.role !== "service_role"
  ) {
    console.log(
      `⚠️  WARNING: SUPABASE_SERVICE_ROLE_KEY has role="${serviceValidation.payload.role}" (expected "service_role")`,
    );
  }
}
...
if (!requiredVars.SUPABASE_SERVICE_ROLE_KEY) {
  issues.push(
    "⚠️  SUPABASE_SERVICE_ROLE_KEY is not set (optional but recommended)",
  );
}

if (
  requiredVars.DATABASE_URL &&
  !requiredVars.DATABASE_URL.includes("sslmode")
) {
  issues.push(
    "⚠️  DATABASE_URL missing SSL configuration (add ?sslmode=require)",
  );
}
...
if (
  anonVal.valid &&
  serviceVal.valid &&
  anonVal.payload.iss !== serviceVal.payload.iss
) {
  issues.push(
    "⚠️  ANON_KEY and SERVICE_ROLE_KEY have different issuers (possible project mismatch)",
  );
}
...
console.log(
  "  2. Run: node scripts/diagnose-rls.sql for database-level RLS checks",
);
console.log(
  '  3. If "No suitable key" error persists, verify JWT signature algorithm',
);
