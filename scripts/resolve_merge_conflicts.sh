...
check_and_clean_duplicates() {
  ...
  if [ -n "$dups" ]; then
    ...
    if [ "$DRY_RUN" = true ]; then
      ...
      log_info "Removing duplicate files..."
      find . -name ".git" -prune -o -type f \( -name "* 2" -o -name "* 2.*" -o -name "* 3.*" \) -exec rm "{}" +
      log_success "Duplicate files removed"
    else
      log_warning "Run scripts/cleanup_duplicates.sh to remove them, or delete manually."
    fi
  fi
}
