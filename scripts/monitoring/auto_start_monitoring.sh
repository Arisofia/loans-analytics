...
# Refresh Supabase PostgreSQL datasource with temporary login-role credentials
if [[ -x "$SCRIPT_DIR/refresh_grafana_supabase_datasource.sh" ]]; then
	echo -e "${YELLOW}Refreshing Supabase PostgreSQL datasource credentials...${NC}"
	if "$SCRIPT_DIR/refresh_grafana_supabase_datasource.sh" >/dev/null 2>&1; then
		echo -e "${GREEN}✓ Supabase datasource refreshed${NC}"
	else
		echo -e "${YELLOW}⚠ Could not refresh Supabase datasource automatically${NC}"
	fi
	echo ""
fi
