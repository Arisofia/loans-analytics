...
class MetricsRegistry:
    ...
    async def _collect_connection_pool_metrics(self) -> None:
        try:
            ...
            self.register_counter(
                ...
                metrics.get("failed_connections", 0),
            )
            ...
            if result_file.exists():
                ...
                for kpi_name, kpi_data in kpi_results.items():
                    ...
                    self.register_counter(
                        ...
                        1,
                        {"kpi_name": kpi_name, "status": status},
                    )
                    if status == "error":
                        self.register_counter(
                            "kpi_calculation_failures_total",
                            1,
                            {"kpi_name": kpi_name},
                        )
        except Exception as e:
    ...
    def render_prometheus_format(self) -> str:
        lines = [
            "# Abaco Loans Analytics Pipeline Metrics",
    ...
    def _get_help_text(self, metric_name: str) -> str:
        help_texts = {
            ...
            "fact_loans_row_count": "Number of rows in fact_loans",
            "connection_pool_size": "Connection pool size by state",
            "connection_pool_queries_total": "Total pool queries",
            "connection_pool_failures_total": "Total pool failures",
            "connection_pool_health_check": "Pool health (1=ok, 0=bad)",
            "idempotency_cache_hits_total": "Total cache hits",
            "idempotency_cache_misses_total": "Total cache misses",
            "kpi_calculations_total": "Total KPI calculations by status",
            "kpi_calculation_failures_total": "Total KPI calc failures",
        }
