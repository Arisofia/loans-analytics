...
def get_cascade_credentials() -> tuple[Optional[str], Optional[str], str]:
    ...
    if not username or not password:
        logger.warning(
            "CASCADE_USERNAME and CASCADE_PASSWORD not set. " "Manual download required."
        )
...
def download_loan_tape(pid: str, output_dir: Path) -> Optional[Path]:
    ...
    logger.info(f"Please download from: {CASCADE_LOAN_TAPE_URL}?pid={pid}")
...
def process_loan_tape(file_path: Path, output_dir: Path) -> pd.DataFrame:
    ...
    ingestion = CascadeIngestion(data_dir=str(file_path.parent), strict_validation=False)

    # Set context
    ingestion.set_context(source="cascade_debt", portfolio="abaco", data_type="loan_tape")
    ...
    # Save processed data
    output_file = (
        output_dir / f"loan_tape_processed_{datetime.now().strftime('%Y%m%d_%H%M%S')}.parquet"
    )
...
def process_financials(data_type: str, file_path: Path, output_dir: Path) -> pd.DataFrame:
    ...
    # Save processed data
    output_file = (
        output_dir / f"{data_type}_processed_{datetime.now().strftime('%Y%m%d_%H%M%S')}.parquet"
    )
    df.to_parquet(output_file, index=False)
    logger.info(f"Saved processed data to: {output_file}")
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description="Ingest data from Cascade Debt platform")
    parser.add_argument("--loan-tape", action="store_true", help="Process loan tape export")
    parser.add_argument("--financials", action="store_true", help="Process financial statements")
    parser.add_argument("--all", action="store_true", help="Process all data sources")
    parser.add_argument(
        "--output-dir", type=str, default="data/cascade", help="Output directory for processed data"
    )
    parser.add_argument("--input-file", type=str, help="Input CSV file to process (skips download)")
    ...
    # Get credentials
    _, _, pid = get_cascade_credentials()
    ...
    if args.input_file:
        ...
        if "loan_tape" in input_path.name.lower():
            process_loan_tape(input_path, output_dir)
        else:
            process_financials("financial_data", input_path, output_dir)
    ...
    if args.loan_tape or process_all:
        ...
        logger.info(
            f"4. Run: python scripts/ingest_cascade.py --loan-tape --input-file {output_dir}/loan_tape_YYYYMMDD.csv"
        )
        logger.info("")
