...
def _build_kam_maps(desembolsos_df: pd.DataFrame) -> tuple[pd.Series, pd.Series]:
    ...
    kam_farmer = _first_non_empty(desembolsos_df, ['Cod_Kam_farmer', 'Cod_Kam_farmer_', 'CL'])
    kam = kam_hunter.fillna(kam_farmer)
    client_id = _first_non_empty(desembolsos_df, ['CodCliente', 'CodCliente_']).astype(str).str.strip()
    client_name = _first_non_empty(desembolsos_df, ['Cliente_', 'Cliente', 'CJ']).astype(str).str.strip()
    ...
    map_by_code_df = mapped[~mapped['cod_cliente'].str.lower().isin({'', 'nan', 'none', 'null'})].drop_duplicates(subset=['cod_cliente'], keep='first')
    map_by_name_df = mapped[~mapped['client_name_norm'].str.lower().isin({'', 'nan', 'none', 'null'})].drop_duplicates(subset=['client_name_norm'], keep='first')
    map_by_code = pd.Series(map_by_code_df['kam'].values, index=map_by_code_df['cod_cliente'])
    map_by_name = pd.Series(map_by_name_df['kam'].values, index=map_by_name_df['client_name_norm'])
...
def _build_overrides_from_intermedia(intermedia_df: pd.DataFrame) -> pd.DataFrame:
    ...
    base = base[~base['cod_cliente'].str.lower().isin({'', 'nan', 'none', 'null'})].drop_duplicates(subset=['cod_cliente'], keep='first')
    base['industry'] = ''
    base['kam'] = ''
    base['lt_customer_id'] = ''
...
def run(args: argparse.Namespace) -> None:
    ...
    if not creds or not spreadsheet_id:
        raise ValueError('Google Sheets credentials missing. Set --credentials-path/--spreadsheet-id or GOOGLE_SHEETS_CREDENTIALS_PATH/GOOGLE_SHEETS_SPREADSHEET_ID.')
    ...
    if 'cliente' not in overrides_df.columns:
        ...
        inter_map_df = inter_map_df[~inter_map_df['cod_cliente'].str.lower().isin({'', 'nan', 'none', 'null'})].drop_duplicates(subset=['cod_cliente'], keep='first')
    ...
    overrides_df.loc[empty_mask, 'kam'] = by_code_values.where(by_code_values.astype(str).str.strip().ne(''), by_name_values)
    ...
    print(f'KAM backfill complete from {source}. Filled={filled}, still_missing={int(after_missing)}, total={len(overrides_df)}')

def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description='Fill missing KAM in manual overrides from DESEMBOLSOS Cod_Kam_farmer (CL)')
    parser.add_argument('--overrides-csv', default='data/raw/client_manual_overrides.csv', help='Path to manual overrides CSV with cod_cliente and kam columns')
    parser.add_argument('--credentials-path', default=None, help='Path to Google service account JSON (optional if env var is set)')
    parser.add_argument('--spreadsheet-id', default=None, help='Google Spreadsheet ID (optional if env var is set)')
    return parser
