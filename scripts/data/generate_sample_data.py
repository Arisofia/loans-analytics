...
def generate_loan(loan_number: int | None = None, start_date: date | None = None) -> Loan:
    ...
    loan_id = f"LN-{loan_number or random.randint(100000, 999999)}"  # nosec
    customer_id = f"CUST-{random.randint(10000, 99999)}"  # nosec
    year = start_date.year
    month = random.randint(1, 12)  # nosec
    day = random.randint(1, 28)  # nosec

    principal = Decimal(str(round(random.uniform(1_000, 50_000), 2)))  # nosec
    rate = Decimal(str(round(random.uniform(0.10, 0.45), 4)))  # nosec
    term = random.choice([6, 9, 12, 18, 24])  # nosec
