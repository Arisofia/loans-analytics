#!/usr/bin/env python3
"""Generate executive strategic reporting artifacts with dashboard links."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        default="reports/strategic",
        help="Directory where report files will be written.",
    )
    parser.add_argument(
        "--streamlit-local",
        default="http://localhost:8501",
        help="Local Streamlit dashboard URL.",
    )
    parser.add_argument(
        "--grafana-local",
        default="http://localhost:3001/dashboards",
        help="Local Grafana dashboards URL.",
    )
    parser.add_argument(
        "--streamlit-prod",
        default="",
    ...
    parser.add_argument(
        "--dashboard-docs",
        default="docs/analytics/dashboards.md",
        help="Path or URL to dashboard documentation.",
    )
    return parser.parse_args()


def main() -> int:
    """Generate and write strategic report artifacts."""
    args = parse_args()

    metrics = load_dashboard_metrics(REPO_ROOT)
    summary = build_strategic_summary(metrics)
    links = {
        "streamlit_local": args.streamlit_local,
        "grafana_local": args.grafana_local,
        "streamlit_prod": args.streamlit_prod,
        "dashboard_docs": args.dashboard_docs,
    }
