...
def check_env_local(root: Path) -> dict:
    ...
    REQUIRED = ['SUPABASE_URL', 'SUPABASE_ANON_KEY', 'SUPABASE_SERVICE_ROLE_KEY', 'OPENAI_API_KEY', 'API_JWT_SECRET']
    OPTIONAL = ['SNYK_TOKEN']
    missing = [k for k in REQUIRED if not env.get(k)]
    if missing:
        return {'status': 'FAIL', 'message': f"Missing required secrets: {', '.join(missing)}"}
    empty_required = [k for k in REQUIRED if env.get(k) in ['', 'xxxxxxxxxxxx', '<<PASTE_GITHUB_PAT>>']]
    if empty_required:
        return {'status': 'FAIL', 'message': f"Secrets have placeholder/empty values: {', '.join(empty_required)}"}
    present = {'required': REQUIRED, 'optional': [k for k in OPTIONAL if env.get(k)], 'total': len(REQUIRED) + len([k for k in OPTIONAL if env.get(k)])}
    return {'status': 'PASS', 'secrets': present, 'message': f"All {present['total']} secrets present"}

def check_no_hardcoded_secrets(root: Path) -> dict:
    dangerous_patterns = ['OPENAI_API_KEY\\s*=\\s*["\\\']sk-[A-Za-z0-9_-]{20,}', 'SUPABASE_ANON_KEY\\s*=\\s*["\\\']eyJ[A-Za-z0-9_-]+', 'SNYK_TOKEN\\s*=\\s*["\\\'][A-Za-z0-9_-]{40,}', 'API_JWT_SECRET\\s*=\\s*["\\\'][A-Za-z0-9_-]{32,}']
    found_secrets = []
    ...
    if found_secrets:
        return {'status': 'WARN', 'message': f"Potential hardcoded secrets found in: {', '.join(found_secrets[:3])}"}
...
def check_workflows(root: Path) -> dict:
    ...
    required_in_workflows = {'SUPABASE_URL', 'SUPABASE_ANON_KEY', 'SUPABASE_SERVICE_ROLE_KEY', 'OPENAI_API_KEY', 'SNYK_TOKEN', 'API_JWT_SECRET'}
    found_secrets = set()
    workflow_count = 0
    ...
    return {'status': 'PASS', 'message': f'{len(found_secrets)} distinct CI secrets referenced across {workflow_count} workflows', 'secrets_used': sorted(found_secrets)}
...
def main():
    ...
    print('=' * 70)
    print()
    checks = [('Secrets in .env.local', check_env_local), ('No hardcoded secrets', check_no_hardcoded_secrets), ('Workflows configuration', check_workflows), ('Upload script', check_upload_script)]
    results = []
    ...
    print('=' * 70)
    passed = sum((r['status'] == 'PASS' for _, r in results))
    warned = sum((r['status'] == 'WARN' for _, r in results))
    failed = sum((r['status'] in ('FAIL', 'ERROR') for _, r in results))
    total = len(results)
