...
files = {k: [f for p in v for f in glob.glob(f'data/input/{p}.csv') + glob.glob(f'data/input/{p}.xlsx')] for k, v in patterns.items()}
...
def clean_file(file, is_excel=False):
    if is_excel:
        ...
        if key: df = df.drop_duplicates(subset=[key])
...
for f in config_files:
    with open(f, 'r') as yf: data = yaml.safe_load(yf)
    merged.update(data)
with open('config/unified.yml', 'w') as out: yaml.dump(merged, out)
[os.remove(f) for f in config_files if f != 'config/unified.yml']

# Post-Clean KPIs (Sample; Extend for Full)
loan_df = pd.read_csv(next(f for f in cleaned_files if 'Loan_Data' in f or 'loan' in f.lower()), low_memory=False)
kpis = {
    'total_outstanding': loan_df['Outstanding Loan Value'].sum() if 'Outstanding Loan Value' in loan_df.columns else 0,
    'unique_clients': loan_df['Customer ID'].nunique() if 'Customer ID' in loan_df.columns else 0,
    'avg_amount': loan_df['Disbursement Amount'].mean() if 'Disbursement Amount' in loan_df.columns else 0
}
with open('logs/kpis.yaml', 'w') as kf: yaml.dump(kpis, kf)
